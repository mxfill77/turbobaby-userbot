...
def substitute(text, ph):
    """Реплика корпуса + даты окна → строка, которую УВИДИТ ГОЛОВА. ЕДИНСТВЕННАЯ подстановка полосы.

    Вынесена из `build_transcript` не ради красоты. Строку клиента готовит не только прогон: её же
    показывает владельцу витрина экзамена (`exam_show.freeze` кладёт вопрос в снимок, снимок идёт в
    карточку). Вторая копия `str(x).format(**ph)` разошлась бы МОЛЧА — и владелец судил бы ответ по
    вопросу, которого голова не видела. Ровно это и замерено 12.09.2026 на кейсе 1: в показанной
    карточке стоял шаблон `{when_sloppy}`, а в ответе бота — сами даты «с 6 по 11 октября».
    Формулировки клиента при этом остаются ДОСЛОВНЫМИ: подставляются только числа."""
...
def quote_models(j_line):
    """Имена моделей из QUOTE-БЛОКА записки, В ПОРЯДКЕ БЛОКА. → list[str].

    Предмет — то, что ПОСЧИТАЛ И ПОДАЛ КОД, а не то, что пересказала голова. Формат строки задаёт
    сам вставляющий код: «МЕТКА — цена…» у точечной цены и подбора по классу, «- метка: цена…» у
    многомодельной ветки. Строка без обеих разделителей моделью не считается (в блок попадает,
    например, строка «цена и депозит ЗА КАЖДЫЙ юнит» — она не модель и в счёт не идёт)."""
    out = []
    for raw in (j_line or "").split("\n"):
...
def case_checks(case, draft, exp):
    ...
    # 9б. СПРОШЕНА ОДНА МОДЕЛЬ — СЛОВО ВЛАДЕЛЬЦА 12.09.2026, дословно: «только про спрошенную
    # модель и одну-две альтернативы». Ожидание: моделей в quote-блоке НЕ БОЛЬШЕ ТРЁХ (спрошенная +
    # максимум две) и ПЕРВАЯ — та, которую назвал клиент.
    #   ПОЧЕМУ СУДИМ ЗАПИСКУ, А НЕ ТЕКСТ ГОЛОВЫ. Строки в quote-блок вставляет КОД, и предмет
    #   правила — сколько моделей код посчитал и подал. Голова вправе пересказать блок своими
    #   словами и склеить строки — счёт по её прозе краснел бы на здоровом пересказе и зеленел бы
    #   на молчании (тот же класс, что закрыт узлом «молчащая голова = НЕИЗВЕСТНО»).
    #   ЧЕМ ЭТО БЫЛО ДО 12.09: чеков на ЧИСЛО моделей не было ни одного, поэтому снимок кейса 1 от
    #   11.09 с ДЕСЯТЬЮ моделями в блоке (7 из них мотоциклы на спрошенный макси-скутер) прошёл
    #   набор 12 из 12 — корпус не судил ровно то, из-за чего владелец и завёл правило.
    asked_model = want.get("one_model")
    if asked_model:
        names = quote_models(exp.get("j_line"))
        key = suggest._bike_key(asked_model)
        first_ok = bool(names) and key and key in suggest._bike_key(names[0])
        chk = _chk("подбор: спрошенная первая, альтернатив не больше двух",
                   bool(names) and len(names) <= 3 and bool(first_ok),
                   "в quote-блоке записки не больше 3 моделей (спрошенная + максимум две "
                   "альтернативы) и первая из них — «%s»" % asked_model,
                   ("моделей %d: %s%s" % (len(names), ", ".join(names[:6]),
                                          "" if first_ok else " · ПЕРВАЯ НЕ СПРОШЕННАЯ"))
                   if names else "quote-блока в записке нет")
        if not names:
            chk["unknown"] = suggest.no_basis("quote-блока в ценовой записке нет — считать модели "
                                              "не в чем")
        out.append(chk)
