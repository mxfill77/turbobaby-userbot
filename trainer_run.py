...
def _pair(a, b):
    """Два значения ОДНОГО поля для человека, ОБЯЗАТЕЛЬНО РАЗЛИЧИМЫЕ на глаз. → (строка, строка).

    ЗАЧЕМ (живой замер 07.09.2026, сброс точки на `--runs 3`). Обрезка по длине молча схлопывает
    значения, расходящиеся в ХВОСТЕ: текст критерия на 2 и на 3 круга совпадает первыми 46
    символами, и причина печаталась как «критерий (правило): F: кейсам класса … против F: кейсам
    класса …» — то есть прибор называл расхождение и тут же показывал два одинаковых значения.
    Читатель обязан заключить, что прибор врёт. Поэтому при совпавших обрезках показываем ОКНО
    ВОКРУГ ПЕРВОГО РАСХОЖДЕНИЯ, а не голову строки."""
    sa, sb = str(a), str(b)
    va, vb = _val(sa), _val(sb)
    if va != vb:
        return va, vb
    n = min(len(sa), len(sb))
    i = next((k for k in range(n) if sa[k] != sb[k]), n)
    lo = max(0, i - 12)
    return ("…" + sa[lo:i + 26] + "…", "…" + sb[lo:i + 26] + "…")
...
def basis_diff(old, new):
    ...
    for k in sorted(set(old or {}) | set(new or {})):
        ...
        if a != b:
            va, vb = _pair(a, b)
            out.append("%s: %s против %s" % (word, va, vb))
    return out
