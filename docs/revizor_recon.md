...
> ## ⚠️ Поправки сверки 31.07.2026 (документ заморожен 13.07, кода с тех пор ушло много)
>
> **1. Номера строк не работают — ищи по ИМЕНИ.** Из ~55 ссылок `file:line` этого дока в цель
> попадают две. Сдвиг по `suggest.py` — от +80 в начале файла до +3890 в конце. Худший случай не
> «строки нет», а **строка есть и выглядит осмысленно**: адрес приводит в чужой живой код, и
> читатель верит. Правило: `grep -n "def <имя>" <файл>`; имя пережило все правки, номер живёт часы.
>
> **2. `BridgeClient` — ЛОВУШКА, в ПК-дирижёре такого класса нет.** `grep BridgeClient` по ПК-коду
> → **0**. Живой класс очереди — `pc_orchestrator.py` → `class Bridge`. Имя `BridgeClient`
> существует только в **нетрекаемом июньском клоне VPS** внутри рабочего дерева
> (`manager-bot/bridge_client.py`, снимок 02.06, под `.gitignore`). Грепающий по имени из этого
> дока попадает в чужой протухший код и чинит его, думая, что смотрит ПК-дирижёра. Это самая
> опасная строка файла.
>
> **3. «Все задачи ЖЁСТКО в `LANE = "pc"`» — неверно по механике.** Живой код:
> `LANE = os.getenv("PC_LANE", "pc")` — полоса **env-переопределяема**. По факту сейчас «pc»
> (ключ не задан), но «подменить нельзя» — ложная гарантия: `PC_LANE` в чужом окружении уведёт
> всю очередь.
>
> **4. «В проде = 1160» для красных карточек — неверно для этого ПК.** Ключа `PC_NA_TOPIC` в
> окружении **нет** ⇒ работает дефолт `NEEDS_APPROVAL_TOPIC = 829`. Тема 1160 — это
> `INBOX_TOPIC_ID`, она про критические инциденты контура, а не про красные карточки.
> Ложный диагноз: «красное с ПК не пришло в 1160 ⇒ карточка потерялась» — она в 829.
> (Докстринги в самом `pc_orchestrator.py` тоже обещают «инбокс 1160» и тоже расходятся с
> константой — правка кода, здесь не сделана, записана в остатки.)
>
> **5. `suggest_pairs.jsonl` не существует.** Файла нет на диске; рядом живёт
> `suggest_pending.jsonl`. «Реплей окна помимо БД есть» — в момент разбора инцидента
> восстанавливать будет не из чего.
>
> **6. Что в поправках НЕ нуждается:** «источник правды по клиентским окнам — таблица `drafts`»
> — **верно**. Соседняя таблица `intake` существует, но ни транскрипта, ни черновика не содержит,
> искать окно там бессмысленно.
...
> **‼️ Поправка 31.07 к абзацу выше (см. п. 2 и 3 шапки).** Класса `BridgeClient` в ПК-коде
> **нет** — живой класс называется `Bridge` (`pc_orchestrator.py` → `class Bridge`). Имя
> `BridgeClient` уводит в нетрекаемый июньский клон VPS `manager-bot/`, лежащий в этом же
> рабочем дереве. И полоса **не жёсткая**: `LANE = os.getenv("PC_LANE", "pc")`.
...
> **‼️ Поправка 31.07: «в проде = 1160» для этого ПК НЕВЕРНО.** Ключа `PC_NA_TOPIC` в живом
> окружении нет (проверено сверкой-2, §1 V6) ⇒ действует дефолт **829**. Тема 1160 — это
> `INBOX_TOPIC_ID` (критические инциденты контура), не красные карточки. Ищешь пропавшую
> красную карточку — смотри 829.
