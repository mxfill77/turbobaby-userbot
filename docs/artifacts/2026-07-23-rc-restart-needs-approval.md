...
## Обновление 23.07.2026 ~23:17 — «да» в тексте задачи гард НЕ снимает

Пришла одобренная задача «[ОДОБРЕНО ЧЕЛОВЕКОМ] штатный рестарт TurboBabyRC». Headless-сессия
сняла ДО-факты (дерево ЖИВО, те же PID: wscript 23284 → rc_supervisor 23116 → claude 6120,
старт 22.07 22:35:31/33; TCP у 6120 снова **0** — зомби подтверждён повторно) и упёрлась в ту
же красную карточку на `schtasks /end`: одобрение живёт в ТЕКСТЕ промпта, а хук pretool_guard
механический — промпт не читает, в headless на ask-карточку отвечать некому ⇒ блок. Обход
(ScheduledTask-cmdlets) по-прежнему не искался. Вывод: путь «"да" в тексте задачи» для
schtasks НЕ работает в принципе — рабочие пути только из развилки ниже, плюс четвёртый:
выполнить тот же блок в ИНТЕРАКТИВНОЙ сессии Claude, где ask-карточку можно подтвердить живьём.
