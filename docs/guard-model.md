...
> карточек, красное/зелёное и `decide()` — пришёл не туда, и не найдёт: перечни классов гарда
> разрешений не сведены **ни в одном** документе репозитория. Это отдельный остаток сверки.
>
...
>
> **3. «Полный гейт 1754 OK» в конце файла — протухший якорь зелени.** Сегодня `def test_` во
> всех `test_*.py` = **2398** (замер 31.07). Сверять «гейт зелёный» по числу 1754 нельзя.
>
