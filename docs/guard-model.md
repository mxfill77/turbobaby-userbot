...
- `suggest.py:compose_quote_draft` (3124) — точечный quote-блок Bridge в финал КОДОМ; fail-safe: потерянные LLM денежные числа приклеиваются хвостом.
- `suggest.py:compose_delivery_draft` (3161) — приклейка блока цены доставки кодом (LLM блок не видит).
...
- Шаг 7/7 (обязательный смоук `runLiveSmoke`) — прогнан как у демона
  ...
  выполняется. НЕ регрессия #274. Развилка у владельца:
