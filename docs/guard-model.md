...
> ## ⚠️ Читать первым (сверка 31.07.2026)
>
> **1. Это НЕ карта гарда разрешений.** Несмотря на имя файла, `pretool_guard` здесь не
> упоминается ни разу: «guard-модель» тут — про **гвард выбора модели техники** в клиентском
> контуре (чтобы в прайс-блок не попала чужая карточка). Кто ищет по имени файла классы
> карточек, красное/зелёное и `decide()` — **пришёл не туда, ему в
> [`guard-classes.md`](guard-classes.md)** (заведён 31.07: три перечня классов разной длины,
> точное совпадение при одобрении, отсутствие `deny` на ПК). Раньше их не было ни в одном
> документе — остаток закрыт.
>
> **2. Номера строк мертвы — ищи по ИМЕНИ.** Все адреса заморожены на 23.07; `suggest.py` с тех
> пор уехал на +1000…+1400 строк (например `on_client_message` 5634 → ~6899,
> `generate_draft` 4932 → ~6169, `price_sheet` 2783 → ~3446). Худший случай — адрес существует
> и ведёт в осмысленный ЧУЖОЙ код. Правило: `grep -n "def <имя>" suggest.py`.
>
> **3. «Полный гейт 1754 OK» в конце файла — протухший якорь зелени.** Число тестов растёт
> каждый день (31.07 замерено дважды: 2398 утром, **2418** вечером), поэтому **не сверяй зелень
> по числу вообще** — ни по 1754, ни по свежему. Мерить командой:
> `grep -cE "^\s*def test_" test_*.py | awk -F: '{s+=$2} END {print s}'`, а зелень — прогоном
> гейта.
>
> **4. Три расхождения по существу** — в тексте ниже, помечены «Поправка 31.07».
...
- `suggest.py:transcript_from` (1023) — сборка транскрипта «Клиент:/Менеджер:» из сообщений; канонический источник текста для всех детектов.
  ...
  > **‼️ Поправка 31.07 — формат транскрипта записан НЕВЕРНО, и это молчаливый убийца тестов.**
  > Живой код собирает строки как `[клиент]: …` / `[менеджер]: …` (`transcript_from` →
  > `lines.append(f"[{who}]: {body}")`, где `who` = `клиент`/`менеджер`), а **не**
  > «Клиент:/Менеджер:». Все фильтры завязаны на эту форму дословно
  > (`_client_text` → `l.startswith("[клиент]:")`, `_manager_text` → `"[менеджер]:"`).
  > Цена ошибки: фикстура, собранная «по карте», даёт `_client_text() == ""` **молча** —
  > детекты вернут False, тест останется зелёным, а прод будет мёртв. Это ровно класс
  > «мок ≠ реальность» из `CLAUDE.md`: формат мока обязан копировать формат источника.
...
- `suggest.py:parse_approval` (918) — разбор ответа менеджера: «+»=approve, «-»/«нет»/«no»/«отклонить»=reject, иной текст=edit.
  ...
  > **‼️ Поправка 31.07 — «иной текст = edit» неверно для ПУСТОЙ реплики.** Живая функция:
  > approve ловит шире (`+`, `да`, `ок`, `ok`, `yes`, `+1` и префикс `«да …»`), а **пустой текст
  > даёт `reject`**, не `edit`. Тот же дефект живёт в докстринге самой функции — то есть
  > читатель кода получит ту же неверную картину, что и читатель этого дока.
  > Ложный диагноз: «менеджер отправил пусто ⇒ уйдёт как правка» / «черновик потерялся в IPC» —
  > он штатно **отклонён**.
...
- `moderation_ipc.py:set_decision` (239) / `mark` (249) — запись финального статуса черновика `ready | test_held | rejected` в SQLite-очередь; `rejected` = забракован, отправки не будет.
  ...
  > **‼️ Поправка 31.07 — это ДВЕ разные функции с РАЗНЫМИ алфавитами, а док слил их в одну.**
  > `set_decision` принимает `ready | test_held | rejected` и защищён `assert`.
  > `mark` принимает **другое**: `sent | failed | test_held`, и **ассерта у неё нет**.
  > При этом `rejected` пишется именно через `mark` (`moderation_bot.py` → `_apply`) —
  > то есть мимо всякой проверки.
  > Ложный диагноз: «`set_decision` пропустил мусорный статус» → пойдут править ассерт,
