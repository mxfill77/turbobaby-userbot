...
# ---------- ЧТО ВИДИТ ОПОЗДАВШИЙ (единственный текст на весь контур, 06.09.2026) ----------
# До этой даты второй нажавший получал обычное «✅ Принято — userbot отправит клиенту»: контур
# не различал «ты решил» и «за тебя уже решили», и человек уходил уверенный, что отправил он.
# Текст живёт ЗДЕСЬ, в чистом модуле, который импортирует ЖИВОЙ бот, — и отсюда же его берёт
# `moderation_card.render_closed`. Двух копий нет СОЗНАТЕЛЬНО: разъехавшись, они соврали бы
# по-разному в двух местах одного контура.
STATUS_RU = {
    "ready": "отправляем клиенту",
    "sending": "отправляется прямо сейчас",
...
def render_closed(row):
    ...
    who = _sv(row, "decided_by") or "кем-то из модераторов"
    when = _sv(row, "updated_ts")
    out = "🔒 Карточка уже закрыта: " + STATUS_RU.get(status, status) + " · решил " + who
    if when:
        out += " · " + when
