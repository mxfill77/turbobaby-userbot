...
def _emit_ask(card, kind=""):
    ...
    if mk:
        _write_marker(mk, card, kind)     # канал 2 (развод боя и пробы — внутри писателя)
    _push(card)                           # канал 1 (глушение — внутри отправителя)
    # ТЕКСТ РЕШЕНИЯ читает ИСПОЛНИТЕЛЬ, а не владелец: в пробе показываем перехват на границе
    # (дословную карточку, которая собиралась уйти), в бою — саму карточку, как было.
    print(json.dumps({"hookSpecificOutput": {
        "hookEventName": "PreToolUse",
        "permissionDecision": "ask",
        "permissionDecisionReason": _probe_intercept(card, mk or "") if isolated() else card,
    }}, ensure_ascii=False))
    sys.exit(0)
