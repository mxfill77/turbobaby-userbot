...
_ACTION_CHECK = {
    ...
    "_RE_SQL_WRITE": "оператор ЗАПИСИ SQL (update/insert/drop…) + имя .db — действие, а не слово",
...
def kinds_from_card(text):
    ...
    if not (out & set(_KIND_VOCAB)):
        heads = "\n".join(s for s in (x.strip() for x in lines) if _RE_CARD_HEAD.match(s))
