...
ДВЕ ПРАВКИ 30.07.2026 (оба дефекта — про ЦЕНУ подтверждения, детали у самих функций):
  ...
  • НАЛИЧИЕ ФАЙЛА ≠ ЕГО СОДЕРЖИМОЕ. `Test-Path .env` (в команде дословно «not reading contents»)
    ...
    остаётся в hard-блоке. См. `_env_reach` / `_py_env_probe_only`.
...
ДЕВЯТАЯ ГРУППА ТОГО ЖЕ КЛАССА (04.08.2026) — ПРАВИЛО ИСПОЛНЯЮЩЕЙ ПОЗИЦИИ, зеркало полосы VPS
(`5ca761d`/`b30e9b8`, артефакт `docs/artifacts/2026-08-04-exec-position.md`). ОДНА формулировка
вместо разряда за разрядом: КРАСНИТ ИМЯ, СТОЯЩЕЕ ТАМ, ГДЕ ЕГО ИСПОЛНИТ РАЗБОРЩИК. У имён боевых
операций исполняющую позицию называет форма вызова (`_py_write_call`, `_py_write_call_ast`), у
команды оболочки — командная позиция (`_verb_acts`), у SQL — вызов курсора (`_sql_is_executed`).
Из двух течей, закрытых на сервере, на ПК ЖИВОЙ ОКАЗАЛАСЬ ОДНА, и сказано это замером, а не
пересказом ТЗ:
  • ИМЯ ОПЕРАЦИИ В ИМЕНИ ФАЙЛА (`python create_booking_report.py` с ЧИСТЫМ телом) на ПК красного
    не рождало УЖЕ — здесь это закрыла пятая группа 31.07, переведя `py_write` с подстроки на
    форму вызова. Повторно не чинено; замок от повторного открытия — секция 2 голденов.
  • SQL-ГЛАГОЛ В ПРОЗЕ ТЕЛА — течь была ЖИВАЯ. `_scan_python` искал `_RE_SQL_WRITE` ГОЛОЙ
    ПОДСТРОКОЙ по сырому тексту скрипта, поэтому `print('UPDATE rides SET price=1 в other.db')`,
    тот же текст в докстринге и в комментарии давали карточку `sqlite`, а ОБЪЕКТ её тянулся из
    ТОЙ ЖЕ прозы («other.db», «таблица rides»): карточка про запись, за которой записи нет.
    Замер на 6000 живых команд из транскриптов сессий: изменилось 18 решений, ВСЕ красное→зелёное,
    карточек владельцу 206 → 188. Ни в одном из 18 скриптов нет ни одного вызова `execute`.
    Заодно это делает ПРАВДОЙ утверждение, которое шапка этого модуля уже делала («SQL-запись
    ловится `_RE_SQL_WRITE` в `_scan_python` — по ФОРМЕ ВЫЗОВА»): до правки формы вызова там не
    было вовсе.
ЧТО НЕ ОСЛАБЛЕНО: ТЕКСТ САМОЙ КОМАНДЫ (включая инлайн `-c`, чей argv не режется) судится ПОЛНЫМ
сканом — печатаемый SQL внутри `-c` красный как был; разбора нет (тело не питон, битый синтаксис,
heredoc, нечитаемый .py) → СЛЕПОЙ режим, прежняя подстрока БАЙТ-В-БАЙТ, то есть краснее;
склейка и f-строка, отданные курсору, красные; `executemany`/`executescript` наравне с `execute`.
ПОРЯДОК ПРОВЕРОК НЕ СДВИНУТ ни на строку (на сервере его пришлось восстанавливать — там правка
сняла быстрый греп и подняла обобщённый признак; здесь снимать было нечего): внутри
`_scan_python` конкретное по-прежнему раньше обобщённого, и это ЗАМЕРЕНО голденом, а не обещано.
ОСТАТКИ НАЗВАНЫ АДРЕСОМ в артефакте `docs/artifacts/2026-08-04-exec-position-pc.md` §6.
Регресс — `test_exec_position.py`.
...
# ГЛАГОЛ ЗЕРКАЛА РОВНО ОДИН — `Edit(путь)`. Движок разрешений сверяет только его и правил вида
# `Write(путь)` не смотрит вовсе (замер 22.08.2026); на этом основании владелец перевёл все 12
# таких записей в `Edit(...)` — коммит `1a21072` от 23.08.2026, пути в скобках не менялись.
# Заводить `Write(путь)` обратно НЕ НУЖНО: это правило, которого никто не читает.
_GUARD_SOURCES = (
...
_ACTION_CHECK = {
    ...
    "_RE_PY_TARGET": "вида НЕ назначает: достаёт .py-цель из СЫРОЙ команды, чтобы прочитать её "
                     "тело (`_resolve_py_target`). Проверка действия остаётся за тем, что найдено "
                     "В ТЕЛЕ; сам путь решения не принимает",
...
# ── ФУНКЦИЯ БЕЗ ССЫЛОК ВНУТРИ МОДУЛЯ ОБЯЗАНА НАЗВАТЬ ПОТРЕБИТЕЛЯ (02.08.2026, класс Д-5) ──────
# Та же форма, что у `_ACTION_CHECK` выше, и заведена она по той же причине: 02.08 машинный счёт
# по AST нашёл в этом модуле ШЕСТЬ функций, на чьё имя внутри модуля нет ни одной ссылки. Четыре
# из них живые (их зовёт демон или тесты), а две держались СОБСТВЕННЫМИ доводами в докстрингах,
# которые к тому дню были уже отменены другими правками, — и обе успели стоить чужого времени:
# комментарии решающих веток ссылались на них как на живой механизм. Мёртвая функция дороже
# лишней: читающий разбор приходит НЕ В ТУ точку и чинит то, что не исполняется.
#
# Реестр не запрещает внешнее API — он требует НАЗВАТЬ его потребителя ФАЙЛОМ, а сторож
# (`test_functions_without_callers_declare_their_consumer`) выводит список сирот САМ, по AST, и
# падает и на незаявленной сироте, и на протухшем имени, и на потребителе, который имя не
# упоминает. То есть «мёртвая функция» перестаёт быть находкой одного прохода и становится
# проверяемым свойством: ноль ссылок внутри + ноль заявленных снаружи = не проходит гейт.
_EXTERNAL_API = {
    "kinds_from_card": "pc_orchestrator.py",     # класс одобрения из карточки — читает демон
    "object_from_card": "pc_orchestrator.py",    # объект из карточки — читает демон
    "card_or_journal": "test_pretool_guard.py",  # помощник тестов (см. его докстринг: он СЛЕПОЙ)
    "card_from_decision": "test_pretool_guard.py",
}
...
# Наличие файла, окружение и база уже разведены по ДЕЙСТВИЮ (`_env_reach`,
...
# ЧЕСТНЫЕ ПОМЕТКИ «ГАРД КОДА НЕ ВИДЕЛ» — они же объекты, которые `_scan_python` отдаёт вместо
# имени операции. С 02.08.2026 (класс Д-4) каждая из них КРАСНАЯ в `_stays_red` и НАЗВАННЫМ
# ОБЪЕКТОМ не считается (`_UNNAMED_OBJ`), то есть у высшего вида даёт `deny`, а не подтверждаемую
# карточку «разрешить неизвестно что». Список ровно из тех случаев, где код ЕСТЬ, но разбор его
# не получил: тело подаётся трубой, файл не открылся, строку не удалось разобрать на токены.
#
# ЧЕГО ЗДЕСЬ НЕТ И ПОЧЕМУ (остаток назван, а не спрятан):
#   • `-m <модуль>` — код модуля лежит установленным на диске, он не ad-hoc; честный фикс —
#     резолвить модуль и читать его тело либо вести реестр зелёных модулей (`_GREEN_MODULES`
#     уже есть), а не отказывать вслепую. Точка касания: ветка `t == "-m"` в `_scan_python`.
#   • «без внятной цели» — питону не передали НИ кода, ни файла (`python`, `python --version`).
#     Не видеть тут нечего: тела нет вовсе. Но цель, спрятанная в ПЕРЕМЕННОЙ (`python $S`),
#     попадает сюда же и остаётся тихой — это остаток той же ветки, замер по нему в §4 артефакта.
#   • «кривое квотирование» — пометки БОЛЬШЕ НЕ СУЩЕСТВУЕТ: сбой `shlex` теперь не обрывает
#     разбор, а переводит его на сырой текст (см. `_scan_python`). Красить было НЕЧЕГО, и первый
#     заход этой же правки на ней и споткнулся, уронив штатный многострочный `-c`.
_PY_UNSEEN = ("код из stdin", "скрипт не прочитан")
...
# ══ ПРАВИЛО ИСПОЛНЯЮЩЕЙ ПОЗИЦИИ ДЛЯ SQL (зеркало VPS 5ca761d, 04.08.2026) ════════════════════
# Краснит имя, стоящее там, где его ИСПОЛНИТ разборщик. Для имён боевых операций это правило на
# ПК уже действует с 31.07 (`_py_write_call` судит ФОРМУ ВЫЗОВА, а не подстроку) — и замер это
# подтвердил: имя операции в ИМЕНИ ФАЙЛА (`create_booking_report.py` с чистым телом) красного не
# рождает. А SQL-глагол в теле судился ГОЛОЙ ПОДСТРОКОЙ по сырому тексту, и течь была живой:
#     python s8.py   # тело: print('UPDATE rides SET price=1 в other.db')
#     → ask sqlite, ОБЪЕКТ карточки «other.db» вытянут из ТОЙ ЖЕ печатаемой строки
# Карточка про запись, за которой никакой записи нет, — и объект в ней тоже из прозы. То же на
# докстринге и на комментарии (там объектом вставало «таблица rides»).
#
# ЭТО ЖЕ ЗАКРЫВАЕТ ЛОЖЬ В СОБСТВЕННОМ ДОКСТРИНГЕ МОДУЛЯ: шапка (§ «отдельно про python») уже
# УТВЕРЖДАЛА, что SQL-запись ловится «`_RE_SQL_WRITE` в `_scan_python` — по ФОРМЕ ВЫЗОВА».
# До этой правки это было неправдой: формы вызова там не было вовсе, была подстрока.
#
# ГРАНИЦЫ, каждая сознательная:
#   • ТЕКСТ САМОЙ КОМАНДЫ судится по-прежнему ПОЛНЫМ сканом (`cmd_scan` в `_scan_python`):
#     `python нет_такого.py "UPDATE …"` обязан краснеть, как краснел;
#   • РАЗБОРА НЕТ — СЛЕПОЙ РЕЖИМ: тело не питон, обрывок, heredoc с чужим синтаксисом → прежняя
#     подстрока БАЙТ-В-БАЙТ. Направление сомнения прежнее — краснее (fail-closed). Тем и
#     отличается от `_py_write_call_ast`, где сбой разбора уходит в `continue`: там подстрочная
#     форма отработала ПЕРВОЙ, здесь заменять нечем;
#   • строковые константы берутся ИЗ ВСЕГО ПОДДЕРЕВА аргумента, а не только с верхнего узла:
#     склейка (`"UPDATE " + t`) и f-строка обязаны остаться красными.
_SQL_EXEC_CALLS = frozenset(("execute", "executemany", "executescript"))


def _py_sql_exec_args(sources):
    """Строковые константы, отданные ИСПОЛНЯЮЩЕМУ вызову курсора (execute/executemany/
    executescript) в разобранных телах → список (возможно пустой). Разбор не удался хоть на
    одном непустом теле → None: судить нечем, и вызывающий возвращается к прежней подстроке."""
    out = []
    for src in sources or ():
        if not (src or "").strip():
            continue
        try:
            ...
            return None               # слепой режим: пусть решает подстрока по сырому тексту
        for n in ast.walk(tree):
            if not isinstance(n, ast.Call):
                continue
            fn = n.func
            name = fn.attr if isinstance(fn, ast.Attribute) else getattr(fn, "id", "")
            if name not in _SQL_EXEC_CALLS:
                continue
...
def _sql_is_executed(text, sources):
    """SQL-ЗАПИСЬ в телах скриптов стоит в ИСПОЛНЯЮЩЕЙ позиции? Разбора нет → прежняя подстрока
    по всему тексту (fail-closed, байт-в-байт). Разбор есть → судим ТОЛЬКО строки, отданные
    курсору: печать, докстринг и комментарий SQL не исполняют."""
    args = _py_sql_exec_args(sources)
    if args is None:
        return bool(_RE_SQL_WRITE.search(text or ""))
    return any(_RE_SQL_WRITE.search(s or "") for s in args)
...
# наличия секрета (`_env_reach`): имя обязано стоять в КОМАНДНОЙ ПОЗИЦИИ сегмента.
...
# ── ГАРД ОБЯЗАН УВИДЕТЬ КОД, ПРЕЖДЕ ЧЕМ ЕГО ПРОПУСТИТЬ (02.08.2026, класс Д-4) ─────────────────
# Два канала подачи кода питону разбор НЕ ВИДЕЛ ВОВСЕ, и оба давали `py_write` с честной
# пометкой «не смог разобрать», которую `_stays_red` тут же превращала в тихий проход. Замер
# фикстурами через полный конвейер (`decide_for_role` → `card_decision`):
...
# Последняя строка и есть весь класс: одна операция, два канала, разный цвет — не по цене, а по
# способу подачи. В живом логе гарда за 22.07–02.08 таких проходов 40.
#
# ПРИЧИНЫ РАЗНЫЕ, И ОБЕ МЕХАНИЧЕСКИЕ, А НЕ ДОКТРИНАЛЬНЫЕ:
#  1) `-` (stdin). Разбор возвращал «код из stdin» ДО того, как собрать скан-текст, — хотя при
#     heredoc тело лежит В САМОЙ КОМАНДЕ и `_strip_heredoc` его СОХРАНЯЕТ ровно для этого случая
#     («телом подают КОД — вырезать нечего, всё под сканом»). То есть механизм видеть тело уже
#     был, а ранний выход до него не доходил.
#  2) Windows-путь. `shlex.split` в posix-режиме СЪЕДАЕТ обратные слэши: `tmp\x\wipe.py` даёт
#     токен `tmpxwipe.py`, файла с таким именем нет → «скрипт не прочитан». То же с MSYS-формой
#     `/d/turbobaby-bot/…`, которую Windows открыть не может. Соседний `_RE_PY_TARGET` умеет
#     доставать .py-цель ЦЕЛИКОМ из сырой команды — здесь он и применён.
def _heredoc_code(cmd):
    """Тело heredoc, которое команда САМА ИСПОЛНИТ как код → текст | ''.

    Граница та же, что у `_strip_heredoc`: телом считается код, только если в строке-заголовке
    стоит интерпретатор (`_line_runs_code`). Тело heredoc, которое уходит ДАННЫМИ в файл или в
    журнальную запись, кодом не считается и здесь не возвращается — иначе журнальная строка со
    словом `os.remove` в тексте покраснела бы как операция (класс, уже оплаченный задачей 189)."""
    if not cmd or "<<" not in cmd:
        return ""
    lines = cmd.split("\n")
    out, i = [], 0
    while i < len(lines):
        line = lines[i]
        i += 1
        m = _RE_HEREDOC.search(line)
        if not m or not _line_runs_code(line):
            continue
        delim = m.group(1) or m.group(2) or m.group(3)
        j = i
...
def _resolve_py_target(tok, cmd, cwd):
    """ТОКЕН .py-ЦЕЛИ → путь, который РЕАЛЬНО открывается. Не нашёлся — возвращаем токен как был.

    Сначала пробуем сам токен (как было), затем — .py-цели, вынутые ИЗ СЫРОЙ КОМАНДЫ
    (`_RE_PY_TARGET`, отдаёт путь целиком) и их MSYS-форму `/d/…` → `D:\\…`. Берём ту цель, чьё
    ИМЯ ФАЙЛА совпадает с именем из токена: posix-разбор мог склеить путь, но имя файла в
    склейке сохраняется хвостом.

    ПОЧЕМУ ОДНА ФУНКЦИЯ НА ДВА ВОПРОСА («доверять ли» и «что читать»). Оба спрашивают про ОДИН
    файл, и разъехаться им нельзя: если резолвить путь только для чтения, то `.py` ПОД GIT,
    запущенный Windows-путём, перестал бы опознаваться доверенным (`_is_repo_tracked` смотрит
    склеенный токен, находит False) — и его тело пошло бы под скан. Это не дыра, но лишние
    карточки на штатном запуске отслеживаемых скриптов, то есть ровно та цена, из-за которой
    послабления и заводят. Один резолв, один ответ."""
    if _read_file(tok, cwd) is not None:
        return tok
    want = os.path.basename((tok or "").replace("\\", "/")).lower()
    for raw in _RE_PY_TARGET.findall(cmd or ""):
        cands = [raw]
        m = re.match(r"^/([A-Za-z])/(.*)$", raw)          # MSYS: /d/turbobaby-bot/x.py
        if m:
            cands.append(m.group(1).upper() + ":\\" + m.group(2).replace("/", "\\"))
        for cand in cands:
            base = os.path.basename(cand.replace("\\", "/")).lower()
            # `want` мог схлопнуться (posix-разбор съел слэши) — тогда сверяем вхождением имени
            if base != want and not want.endswith(base) and base not in want:
                continue
            if _read_file(cand, cwd) is not None:
                return cand
    return tok
...
def _scan_python(cmd, cwd, env_probe=False):
    ...
    наличия», см. `_env_reach`): он снимает красное ТОЛЬКО с упоминания В КОМАНДЕ, тело
    скрипта проверяется отдельно и своей проверкой."""
    try:
        toks = shlex.split(cmd)
    except Exception:
        # КРИВОЕ КВОТИРОВАНИЕ — НЕ ПОВОД НИ СДАТЬСЯ, НИ ОТКАЗАТЬ (02.08.2026, класс Д-4).
        # Здесь стоял ранний выход с пометкой «кривое квотирование», которую `_stays_red`
        # обращала в тихий проход. Первый заход правки сделал пометку красной — и уронил
        # ШТАТНУЮ работу: многострочный `-c "…"` шлём мы сами, и `shlex` на нём падает
        # регулярно (живой пример — фикстура чтения sqlite в `mode=ro`).
        # Правильный ответ — третий: НЕ угадывать по токенам, а разобрать СЫРОЙ ТЕКСТ. Код при
        # `-c`/heredoc лежит в самой команде, поэтому `blob` ниже видит его целиком, а .py-цели
        # достаются из текста регуляркой. Тот же приём уже принят в `_scan_text` («кривое
        # квотирование → сегмент как есть, краснее»). Сила детекта не падает, зелёное не краснеет.
        toks = [t for t in _RE_PY_TARGET.findall(cmd or "") if t.endswith(".py")]
    content = ""
    ...
    while i < len(toks):
        ...
        if t == "-c":
            ...
            continue
        if t == "-":
            # КОД ИЗ STDIN. Тело heredoc лежит в самой команде — тогда мы его ВИДИМ и судим по
            # нему, как по любому другому телу. Настоящая труба (`cat x.py | python -`) кода не
            # показывает: там разбирать нечего, и это остаётся честной пометкой.
            here = _heredoc_code(cmd)
            if not here:
        ...
        if t.endswith(".py"):
            t = _resolve_py_target(t, cmd, cwd)   # Windows-/MSYS-путь → путь, который открывается
    ...
    if env_hit:
        ...
        if not (cmd_ok and body_ok):
            # ОБЪЕКТ — ФАЙЛ, К КОТОРОМУ ОПЕРАЦИЯ ОБРАТИТСЯ (правка 03.09.2026, блок правила у
            # `_secret_object`). Здесь стояло дословное совпадение `_RE_ENV`, и на живом случае
            # задачи 20 оно давало объектом расширение `.session`. Не назвать имя — законный
            # исход: метка «объект не установлен» кончается отказом, а не догадкой.
            return ("ask", "env", _secret_object(cmd_scan, py_srcs))
    ...
    # 04.08.2026 (зеркало VPS 5ca761d): оператор судится ПО ИСПОЛНЯЮЩЕЙ ПОЗИЦИИ — строка, отданная
    # курсору, а не напечатанная (`_sql_is_executed`). ТЕКСТ САМОЙ КОМАНДЫ остался под ПОЛНЫМ
    # сканом отдельным условием и не ослаблен; разбора нет → подстрока байт-в-байт (fail-closed).
...
# ══ ОБЪЕКТ КАРТОЧКИ СЕКРЕТА — ТО, К ЧЕМУ ОПЕРАЦИЯ ОБРАТИТСЯ, А НЕ ТО, ЧТО ОНА УПОМЯНУЛА ══════
# ЖИВОЙ ПОВОД, ЗАМЕРЕН 03.09.2026 (задача 20, лог гарда 02:58:18 `headless|PowerShell|ask|env`).
# Команда `venv\Scripts\python.exe _scratch_contracts_0903\read_contracts.py`, объект карточки —
# `.session`. Это РАСШИРЕНИЕ, а не файл: подтвердить «да .session» нечем, а строка отката
# («файл сессии Telethon … вернуть только повторной авторизацией») описывала ОРИГИНАЛ, к
# которому операция не прикасается — скрипт открывает КОПИЮ в `%TEMP%`.
#
# ПРИЧИНА БЫЛА ОДНА СТРОКА: `_scan_python` отдавал объектом ДОСЛОВНОЕ совпадение `_RE_ENV`, а
# `_RE_ENV` ловит хвост `.session`. Правка 05.08 («объект — ПУТЬ, а не расширение») чинила
# ТОЛЬКО шелловую дорогу (`_env_object` по тексту команды) — дорога «имя лежит в ТЕЛЕ скрипта»
# осталась на дословном совпадении.
#
# ПОЧЕМУ ИМЯ ИЗ ТЕЛА НЕЛЬЗЯ БРАТЬ ТЕМ ЖЕ ЛЕВЫМ ПРОХОДОМ. В том же файле ПЕРВОЕ вхождение
# `turbobaby_session.session` стоит в ДОКСТРИНГЕ — во фразе «работает с КОПИЕЙ … оригинал
# `turbobaby_session.session` держит живой userbot». Левый проход по тексту вернул бы оттуда
# ОРИГИНАЛ, то есть карточка назвала бы объектом файл, который операция не трогает, — ошибка
# ХУЖЕ прежней: прежняя была бессмысленной, эта была бы убедительной и ложной. Поэтому имя
# берётся ТОЛЬКО из ИСПОЛНЯЕМЫХ литералов (`ast`, тот же приём, что у `_py_env_only_in_prose`).
#
# ТРЕТИЙ ИСХОД ОБЯЗАТЕЛЕН И ЗДЕСЬ. У живого случая исполняемые литералы — `".session"` и
# `".session"` (имя собирается в коде: `SESSION + ".session"`, `COPY_BASE + ".session"`), и
# назвать по ним файл НЕЛЬЗЯ. Тогда объект — НАЗВАННАЯ метка «объект не установлен», а не
# догадка: она стои́т в `_UNNAMED_OBJ`, то есть карточка не выписывается вовсе (`card_decision`
# → `deny`). Правило владельца 03.09: «отличить нельзя — сказать, что объект не установлен;
# это повод для отказа, а не место для догадки».
ENV_OBJ_UNSET = "объект не установлен"
ENV_OBJ_UNSET_CODE = ENV_OBJ_UNSET + ": имя секрета собирается в коде"
ENV_OBJ_UNSET_EXT = ENV_OBJ_UNSET + ": в команде названо только расширение"
# Вторая природа вида `env` — ВЫНОС ЗНАЧЕНИЯ в файл (`_decide_write`). Метка общая на два места,
# чтобы разбор объекта и фраза последствия читали её с ОДНОГО места, а не двумя литералами.
ENV_LEAK_MARK = " → файл"


def _secret_name_named(obj):
    """Имя секрета НАЗВАНО ⇔ владелец может по нему опознать файл и повторить откат.

    Два признака, и оба положительные: имя начинается с `.env` (полное имя файла по соглашению
    проекта — `.env`, `.env.local`) либо перед точкой стои́т имя (`turbobaby_session.session`,
    `id_rsa.key`) — ровно то, что считает названным файлом замок отката `_named_files`.
    Голое расширение (`.session`) не проходит НИ ПО ОДНОМУ: файла с таким именем не существует,
    и подтверждать в такой карточке нечего.

    СПРАШИВАЕМ ПО БАЗОВОМУ ИМЕНИ, А НЕ ПО ПУТИ ЦЕЛИКОМ, и это не косметика: `_RE_CARD_FILE`
    сознательно не начинает совпадение после `/` или `\\` (отрицательный просмотр назад), поэтому
    на АБСОЛЮТНОМ пути (`/tmp/x/read_copy.session`, `D:/repo/bot.session`) он не находит ничего —
    и полный путь читался бы как «имя не названо». Базовое имя тот же прибор разбирает верно."""
    ...
    if b.startswith(".env"):
        return True
    return bool(_named_files(b))


def _py_secret_literals(sources):
    """Имена секретов из ИСПОЛНЯЕМЫХ строковых литералов python-тел → list[str].

    Комментарии и докстринги ИСКЛЮЧЕНЫ структурно (`ast`), а не эвристикой: комментария для
    парсера не существует вовсе, докстринг — первая инструкция-строка модуля/класса/функции.
    Ни то, ни другое передать в `open()` нечем, значит объектом операции они быть не могут.
    Код не разобрался — тело пропускаем (fail-closed: объект останется неустановленным)."""
    out = []
    for src in sources or ():
        try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Constant) and isinstance(node.value, str) \
                    and id(node) not in docs and _RE_ENV.search(node.value):
                out.append(node.value.strip())
    return out


def _secret_object(cmd_scan, py_srcs):
    """ОБЪЕКТ карточки секрета: путь/имя файла либо метка «объект не установлен».

    Порядок источников — от того, что владелец видит глазом, к тому, что лежит глубже: сначала
    САМА КОМАНДА (`_env_object`), затем исполняемые литералы тел скриптов. Ни один не дал
    названного имени → метка, а не первое попавшееся совпадение."""
    o = _env_object(cmd_scan)
    if _secret_name_named(o):
        return o
    for lit in _py_secret_literals(py_srcs):
        if _secret_name_named(lit):
            return lit
    return ENV_OBJ_UNSET_CODE


# СНЯТА 02.08.2026 (класс Д-5): здесь стояла обёртка-однострочник над `_env_reach` (возвращала
# «разбор по действию смягчил упоминание секрета»), и держал её СОБСТВЕННЫЙ довод в докстринге:
# «имя сохранено: под ним признак `_RE_ENV` стоит в замке `_ACTION_CHECK`». Довод был ложным уже
# в момент написания и остаётся ложным: в реестре (`_ACTION_CHECK["_RE_ENV"]`) названы
# `_env_reach` и `_py_env_readonly`, а снятое имя не названо нигде. Потребителей у обёртки не
# было ни одного (счёт по AST: ноль ссылок в проде и в тестах) — она пережила `f4c3cff` как шим
# ради довода, который тот же коммит и отменил. Цена была не в мёртвом коде: комментарии
# решающих веток ссылались на неё как на живой механизм, и читающий разбор приходил НЕ В ТУ
# функцию — поэтому здесь снята и функция, и все восемь ссылок на неё. Живой разбор —
# `_env_reach` (шелл) и `_py_env_probe_only` (тело питона); это РАЗНЫЕ функции, не путать.
...
def _decide_write(ti, cwd):
    ...
    if not _is_temp_zone(path):
        ...
        if hit:
            # Метка — КОНСТАНТА (`ENV_LEAK_MARK`), а не литерал: по ней фраза последствия узнаёт
            # ВТОРУЮ природу вида `env` (вынос ЗНАЧЕНИЯ в файл, а не обращение к файлу секрета).
            return ("ask", "env", hit + ENV_LEAK_MARK)
...
def _decide_bash_body(cmd, cwd, scan, env_probe=False, skip_kinds=frozenset()):
    ...
    if sheet is not None and "live_sheet" not in skip_kinds:
        return ("ask", "live_sheet", sheet)
    # Секрет упомянут — но ПРОБА НАЛИЧИЯ секретом не делится (`_env_reach`). Признак пробы
...
def _stays_red(kind, obj, cmd):
    ...
    if kind == "py_write":
        # БОЕВОЙ ТОКЕН = КРАСНОЕ. Плюс с 02.08.2026 (класс Д-4) — НЕИЗВЕСТНОСТЬ ТОЖЕ КРАСНАЯ,
        # если гард ПРИЗНАЁТСЯ, что кода не видел (`_PY_UNSEEN`).
        #
        # Здесь стояло `return obj in _RED_PY_TOKENS`, и комментарий рядом честно называл вторую
        # половину: «„скрипт не прочитан“/„-m X“ = неизвестность». Неизвестность отдавалась
        # `defer`, то есть тихим проходом, — и это противоречило доктрине, которую тот же гард
        # объявил у `card_decision`: «не удаётся решить достоверно — операция не проходит».
        # Симметрия, которая делает это дефектом, а не вкусовщиной: сбой разбора в `main` даёт
        # `ask|unknown` (hard-блок), а сбой разбора в `_scan_python` давал тихий проход. ОДНО
        # событие — «гард не смог разобрать» — и два противоположных ответа, по МЕСТУ сбоя.
        #
        # Гейт стоит ЗДЕСЬ, а не в `card_decision`, потому что до карточки эти случаи не
        # доезжали вовсе: `_stays_red`=False → `decide_for_role` отдаёт `defer` и разговор
        # кончается. Новая fail-closed граница высшего яруса для них была недостижима.
        return obj in _RED_PY_TOKENS or obj in _PY_UNSEEN
...
# ПОПОЛНЕН 02.08.2026 (класс Д-4, он же латентный разрыв Д-6). Пять честных пометок `py_write`
# («скрипт не прочитан», «код из stdin», …) заглушками БЫЛИ, а в списке не стояли — и разрыв не
# стрелял только потому, что до `_object_named` они не доезжали: `_stays_red` отсекала их раньше
# и пропускала молча. Две находки маскировали друг друга, и снятие одной обязано было включить
...
# ПОПОЛНЕН 03.09.2026 меткой `ENV_OBJ_UNSET` (живой случай задачи 20). Секретный вид называл
# объектом расширение `.session` — и карточка БЫЛА ПОДТВЕРЖДАЕМА: «да .session» подтверждало
# операцию, о цели которой гард не мог сказать ничего. Метка стои́т ПРЕФИКСОМ (сравнение в
# `_object_named` — по началу строки), поэтому обе её редакции — «…собирается в коде» и
# «…названо только расширение» — читаются как одна незаполненность.
...
#
# СТАТУС СПИСКА ИЗМЕНИЛСЯ 02.08.2026 (класс Д-3), и это надо читать буквально: гейт объекта снят
# для ВСЕХ видов, поэтому «исключением из гейта» эти семь больше не являются — исключать стало не
# из чего. Список остаётся НАЗВАННЫМ ЯДРОМ доктрины и живёт не украшением: его читают голдены
# (`test_hard_block_and_money_always_card` и соседи). То есть потребитель у списка есть и он
# назван — ровно то, чего требует `_EXTERNAL_API` от функций. Ослаблять список нельзя: если
# правило карточки когда-нибудь снова начнут смягчать, эти семь обязаны остаться безусловными.
# ЧТО ИМЕННО ТРЕБУЮТ ГОЛДЕНЫ — ПОПРАВЛЕНО 17.08.2026, и разница не косметическая. Требовали
# «карточки при пустом объекте»; теперь требуют, чтобы вид отсюда при пустом объекте НЕ МОЛЧАЛ
# и НЕ ИСПОЛНЯЛСЯ, то есть `deny` — а карточки требуют там, где объект НАЗВАН. Прежняя редакция
# пиннила бы ровно ту подтверждаемую вслепую карточку, ради снятия которой заведён `deny`:
# «разрешить операцию, о цели которой гард не может сказать ничего».
...
def _card_fields(kind, obj="", raw_cmd=""):
    ...
    if kind == "delete":
        ...
        # ПРОВЕРЯЕМ ТОЛЬКО ТО, ЧТО ЗАПОЛНИЛИ САМИ (03.09.2026). Объект, пришедший от вызывающего,
        # трогать нельзя: у второй природы вида (`ENV_LEAK_MARK` — вынос ЗНАЧЕНИЯ в файл) он по
        # построению не файл, и требование «имя перед точкой» отправило бы в отказ живую карточку
        # утечки значения. Дорога тела скрипта названа честной меткой ещё в `_scan_python`.
        if not o:
            o = _env_object(cmd)
            if o and not _secret_name_named(o):
                o = ENV_OBJ_UNSET_EXT
...
# ══ ПОСЛЕДСТВИЕ «ДА» — ОДНОЙ ФРАЗОЙ (03.09.2026, решение владельца) ══════════════════════════
# ЧЕГО НЕ ХВАТАЛО. Карточка говорила, ЧЕГО ХОЧЕТ операция («Хочу обратиться к .env / секретам»),
# у КАКОГО объекта и КАК откатить. Не сказано было ровно то, ради чего владелец жмёт кнопку:
# ЧТО ПРОИЗОЙДЁТ СРАЗУ ПОСЛЕ «да». Владелец достраивал это сам — то есть шёл поднимать код, и
# ровно тогда карточка перестаёт быть решением за три секунды.
#
# ФРАЗА СОБИРАЕТСЯ ИЗ КЛАССА И ОБЪЕКТА, А НЕ ИЗ ОБЩЕГО ШАБЛОНА. Общий шаблон («операция будет
# выполнена с %s») читается как служебный шум и не несёт ни одного бита: у сноса файла, у выхода
# в сеть и у чтения секрета последствия РАЗНЫЕ по природе, и именно эта разница — предмет
# решения. Поэтому у каждого вида своя фраза, а объект в неё вклеивается.
#
# СОБРАТЬ НЕ УДАЛОСЬ — КАРТОЧКА НЕ ВЫПИСЫВАЕТСЯ ВОВСЕ (`card_decision` → `deny`, причина в лог).
# Направление ошибки прежнее: `deny` строже `ask`, новых разрешений это не выдаёт ни одного.
# Сторож от протухания — `test_every_kind_has_a_consequence`: новый вид в `_KIND_VOCAB` без
# фразы роняет набор, а не тихо теряет карточку в проде.
CONSEQ_LINE_PREFIX = "Если «да»: "
_CONSEQ = {
    "delete": "команда исполнится, и %s исчезнет с диска сейчас же",
    "kill": "команда исполнится, и %s перестанет работать сейчас же",
    "schtasks": "команда исполнится, и задача Планировщика %s изменится на этой машине",
    "git_force": "команда исполнится, и история git будет переписана: %s",
    "sqlite": "команда исполнится, и запись ляжет в базу %s",
    "clasp": "команда исполнится в проекте Apps Script живых таблиц (%s)",
    "clasp_push": "команда исполнится, и код %s заедет в Apps Script живых таблиц",
    "clasp_deploy": "команда исполнится, и ПРОД Apps Script переедет: %s",
    "clasp_run": "команда исполнится, и функция %s отработает в ЖИВЫХ таблицах",
    "live_sheet": "команда исполнится, и обращение уйдёт в живой лист %s",
    "network": "команда исполнится, и запрос уйдёт на %s — отправленное не вернуть",
    "env": "команда исполнится, и содержимое %s окажется в контексте этой сессии",
    # У `read_secret` фраза СВОЯ, а не общая с `env`: одинаковый текст на два вида означал бы,
    # что фраза собрана не из класса (голден `test_consequence_is_built_from_the_class…`).
    # Разница настоящая: `env` — это обращение любой природы, здесь же файл ЧИТАЕТСЯ ЦЕЛИКОМ.
    "outside": "команда исполнится, и %s изменится ЗА пределами проекта",
    "write_outside": "команда исполнится, и %s будет записан ЗА пределами проекта",
    "py_write": "команда исполнится, и python сделает боевую запись: %s",
    ...
    "unknown": "команда исполнится как написана (%s), а гард не берётся сказать, что она сделает",
    KIND_UNKNOWN_TOOL: "операция уйдёт каналом %s, о котором гард не знает даже, читает он или пишет",
}
# Вынос ЗНАЧЕНИЯ секрета в файл — ВТОРАЯ природа вида `env` (`_decide_write` → `ENV_LEAK_MARK`).
# Одна фраза на две природы была бы враньём ровно того же рода, что одна строка отката на две
# природы `py_write` (карточка 136): там «обратной проводкой Bridge» предлагалось снимать уборку
# файла, здесь «окажется в контексте сессии» описывало бы запись секрета НА ДИСК.
_CONSEQ_ENV_LEAK = "команда исполнится, и значение секрета (%s) ляжет файлом на диск"


# Объект во ФРАЗЕ — обрезанный, и это не экономия места ради красоты. Целиком он стои́т строкой
# ВЫШЕ (`Объект:`), а предельный законный путь Windows (260 символов) во второй раз вытеснял бы
# из маркера штамп класса: карточка `read_secret` с таким объектом переросла `MARKER_CARD_MAX`
# ровно на длину повтора. Режем с ХВОСТА, оставляя начало: у пути значащее начало — корень.
CONSEQ_OBJ_MAX = 60


def _conseq_obj(o):
    s = " ".join(str(o or "").split())
    return s if len(s) <= CONSEQ_OBJ_MAX else s[:CONSEQ_OBJ_MAX] + "…"


def _consequence(kind, obj=""):
    """Одна фраза «что будет сразу после «да»» → str, либо "" (собрать не из чего).

    Пустая строка — не «нечего сказать», а РЕШЕНИЕ: `card_decision` обращает её в отказ. Обе
    половины обязательны, и обе доказуемы: вид обязан быть в `_CONSEQ`, объект — быть названным
    (`_object_named`). Без объекта фраза выродилась бы в «команда исполнится» — это не
    последствие, а пересказ слова «да»."""
    o = " ".join(str(obj or "").split())
    if not _object_named(o):
        return ""
    if (kind or "") == "env" and o.endswith(ENV_LEAK_MARK):
        return _CONSEQ_ENV_LEAK % _conseq_obj(o[:-len(ENV_LEAK_MARK)].strip())
    tpl = _CONSEQ.get(kind or "")
    return (tpl % _conseq_obj(o)) if tpl else ""


# ══ СПРАВКА О СТОЯЩИХ ЗАЩИТАХ (03.09.2026, решение владельца) ════════════════════════════════
# ПОВОД. По карточке задачи 20 владелец не мог узнать того, что решает дело: у полосы УЖЕ стои́т
# страж исходящего (`review_send.outbound_violations`), и он ловит ник Telegram, почту и телефон
# ДО отправки наружу. Без этой строки владелец либо отказывает лишний раз, либо соглашается, не
# зная, что именно прикрыто, — и оба исхода стоят круга.
#
# СПИСОК БЕРЁТСЯ У САМОГО СТРАЖА, ЛИТЕРАЛОМ НЕ НАБИРАЕТСЯ. Набранный литералом список разошёлся
# бы с кодом МОЛЧА при первом же новом правиле — и карточка авторитетно обещала бы защиту,
# которой нет (или молчала бы о той, что есть). Сторож — `test_protection_kinds_come_from_sender`.
#
# ЧЕСТНЫЙ ТРЕТИЙ ИСХОД: справку не собрали (модуля нет, импорт упал) — так и написано. «Ничего не
# сказать» здесь читается как «защит нет», а это не то же самое, что «не знаем».
_PROTECTED_KINDS = ("env", "read_secret", "edit_secret", "network")
# ЧТО ИМЕННО ПЕЧАТАТЬ. Перечислить в карточке все виды стража целиком нельзя: их дюжина, и
# строка вырастает на 285 символов — карточка `read_secret` с предельным путём при этом
# перерастает потолок маркера (замер 03.09: 1557 при потолке 1400), то есть справка вытесняла бы
# ШТАМП КЛАССА. Печатать первые сколько влезет — хуже: в порядке самого стража первыми стоят пути
# и токены, а решает дело как раз ХВОСТ перечня (почта, телефон, ник). Поэтому карточка называет
# ЧИСЛО видов и тот их РАЗРЕЗ, ради которого справка заведена, — контакты клиента.
#
# РАЗРЕЗ ВЫНИМАЕТСЯ ЖИВОЙ ПРОБОЙ САМОГО СТРАЖА, а не списком имён: гард показывает ему образец
# контактов и печатает то, что страж на нём назвал. Литерала имён нет вовсе — значит и расходиться
# с кодом стража нечему. Тем же способом проверяется утверждение «а вот этого не ловит»: образец
# с ИМЕНЕМ человека прогоняется через того же стража, и слово «НЕ ловит» печатается ТОЛЬКО если
# страж на нём действительно промолчал.
PROTECT_PROBE_CONTACTS = "@nickname_probe probe@example.com +70000000000"
PROTECT_PROBE_NAME = "Иван Петров"
PROTECT_MISS = "имя человека простым текстом"
PROTECT_UNKNOWN = ("Защита: справку о страже исходящего собрать не удалось — считай, что наружу "
                   "уедет всё, что операция прочитает")


def _outbound_guard():
    """Модуль стража исходящего либо None.

    Импорт ЛЕНИВЫЙ и в try: гард — процесс на КАЖДЫЙ вызов инструмента, и платить импортом
    отправщика за карточку, которой в 99% случаев нет, незачем. Отказ импорта решения гарда не
    меняет ни на йоту — он меняет только текст справки."""
    try:
        import review_send                      # noqa: PLC0415 — лениво и намеренно
        return review_send
    except Exception:
        return None


def outbound_guard_kinds():
    """Все виды, которые ловит страж исходящего полосы, → tuple (пусто = спросить не удалось)."""
    mod = _outbound_guard()
    try:
        return tuple(k for k, _rx in mod._OUTBOUND_RULES)
    except Exception:
        return ()


def outbound_guard_catches(text):
    """Что страж исходящего НАЗОВЁТ на этом образце → tuple видов (пусто = промолчал)."""
    mod = _outbound_guard()
    try:
        return tuple(sorted({h["kind"] for h in mod.outbound_violations(text)}))
    except Exception:
        return ()


def _protection_line(kind):
    """Строка справки о защитах для вида, либо "" (виду справка не положена)."""
    if (kind or "") not in _PROTECTED_KINDS:
        return ""
    kinds = outbound_guard_kinds()
    if not kinds:
        return PROTECT_UNKNOWN
    contacts = outbound_guard_catches(PROTECT_PROBE_CONTACTS)
    misses = not outbound_guard_catches(PROTECT_PROBE_NAME)
    return ("Защита: страж исходящего ловит %d форм; из контактов — %s; %s %s"
            % (len(kinds), ", ".join(contacts) or "НИ ОДНОГО",
               PROTECT_MISS, "НЕ ловит" if misses else "ловит тоже"))


def _card_blocked(kind, o, raw_cmd="", obj=""):
    """ПРИЧИНА, по которой карточка не выписывается вовсе, либо "" (выписывается). → str

    Два условия, оба — решения владельца 03.09.2026, и оба про ОДНО: карточка, которая не может
    сказать правду, не спрашивает, а отказывает.
      • ПОСЛЕДСТВИЕ НЕ СОБРАЛОСЬ (`_consequence`) — нечего показать в самой решающей строке;
      • ОТКАТ ПРО ДРУГОЙ ОБЪЕКТ (`_rollback_conflicts`) — владелец, выполнивший строку буквально,
        тронет ЧУЖОЙ файл, а названный объект не изменится вовсе (живой брак задачи 189).
    ПОЧЕМУ ВТОРОЕ УСЛОВИЕ СТАЛО ОТКАЗОМ ТОЛЬКО СЕЙЧАС. 05.08 тот же случай кончался ПОДМЕНОЙ
    строки на честный прочерк, и довод стоял прямо в комментарии: «снятие карточки было бы дырой,
    а не строгостью» — потому что тогда «не выписывать карточку» означало `defer`, то есть ТИХОЕ
    ИСПОЛНЕНИЕ операции. С 17.08 у гарда есть `deny`: операция НЕ исполняется и НЕ подтверждается.
    Довод отменён не мнением, а появлением третьего исхода. Подмена строки в `_card` остаётся
    ВТОРЫМ ПОЯСОМ: прямой вызов `_card` (тесты, будущий читатель) не должен печатать чужой файл."""
    if not _consequence(kind, o):
        return "последствие «да» не собралось: вид %r, объект %r" % (kind or "—", o or "—")
    rb = _rollback(kind, raw_cmd, obj)
    if _rollback_conflicts(rb, o):
        return "откат называет не тот объект: %r против объекта %r" % (rb, o or "—")
    return ""
...
def _card(kind, obj="", raw_cmd=""):
    ...
    # ПОСЛЕДСТВИЕ «ДА» — СРАЗУ ПОД ОБЪЕКТОМ: это самая решающая строка карточки, а глаз читает
    # сверху. Фраза строится из ПЕЧАТАЕМОГО объекта (`o`), а не из сырого `obj`: владелец решает
    # по тому, что видит. Пустой фразы здесь не бывает — `card_decision` до карточки не доводит.
    cons = _consequence(kind, o)
    if cons:
        lines.append(CONSEQ_LINE_PREFIX + cons)
    lines.append("Число: " + (n or "—"))
    ...
    lines.append(rb)
    # СПРАВКА О ЗАЩИТАХ — только там, где данные операции уезжают наружу или ложатся в контекст
    # сессии, откуда их выносит отправщик. Обычная карточка от неё не растёт ни на байт.
    prot = _protection_line(kind)
    if prot:
        lines.append(prot)
    if raw_cmd:
...
def _deny_text(kind, obj="", raw_cmd="", reason=""):
    """Отказ вместо карточки без названного объекта — с 17.08.2026 у ЛЮБОГО вида, не только у
    высшего. Формулировка ОБУЧАЮЩАЯ: она говорит, что именно сделать, чтобы операция стала
    подтверждаемой, — иначе отказ читается как поломка гарда и его начинают обходить.

    ЦЕНА НАЗЫВАЕТСЯ, НО ОТКАЗ ОТ НЕЁ НЕ ЗАВИСИТ: у высшего вида первая строка несёт «высшая
    цена» (владелец и модель обязаны видеть разницу необратимого), у обычного — не несёт, потому
    что врать про цену нельзя. Решение при этом одно и то же: объекта нет — карточки нет.

    `reason` (03.09.2026) — ВТОРОЙ повод отказа: объект назван, но карточка всё равно не может
    сказать правду (`_card_blocked` — последствие не собралось либо откат про другой объект).
    Тогда шапка не врёт про неназванный объект, а называет ПРИЧИНУ; совет остаётся обучающим."""
    head = ("⛔ ОТКАЗ: " + ("высшая цена, а " if is_top_tier(kind) else "")
            + ("КАРТОЧКА НЕ СОБРАЛАСЬ — " if reason else "ОБЪЕКТ НЕ НАЗВАН — ") + _human(kind, ""))
    lines = [head,
             ("Карточка выписывается только целой: без последствия «да» и без отката, который "
              "описывает ТОТ ЖЕ объект, подтверждать в ней нечего." if reason else
              "Карточка без названного объекта не выписывается: подтверждать в ней нечего, "
              "а подтвердить её можно (так 31.07 прошла карточка «лист не определён»)."),
             ("Что делать: доложи владельцу причину ниже — её чинит правка гарда, а не "
              "переформулировка команды. Операция НЕ выполнена." if reason else
    ...
    if reason:
        lines.append("Причина: " + " ".join(str(reason).split()))
    flat = " ".join(str(obj or "").split())
    if flat in _PY_UNSEEN:
        # ОТКАЗ ОБЯЗАН УЧИТЬ ТОМУ, ЧТО ЗДЕСЬ ДЕЙСТВИТЕЛЬНО ПОМОЖЕТ (02.08.2026, класс Д-4).
        # У этих пометок совет «назови объект» неверен: объект не назвать, потому что гард не
        # смог ПРОЧИТАТЬ КОД. Совет обязан говорить про канал подачи кода, а не про имя цели.
        lines[2] = ("Что делать: подай код так, чтобы гард его ВИДЕЛ — запусти скрипт путём, "
                    "который открывается из корня репозитория (`venv/Scripts/python.exe "
                    "tmp/…/x.py`), либо телом heredoc вместо трубы в stdin. Операция НЕ "
                    "выполнена. Если скрипта нет вовсе — это опечатка в пути.")
    elif flat.startswith(ENV_OBJ_UNSET):
        # ТО ЖЕ ПРАВИЛО ДЛЯ СЕКРЕТА (03.09.2026): совет «назови объект» верен, но недостаточен —
        # у живого случая задачи 20 операция работала с КОПИЕЙ, и назвать оригинал было бы
        # неправдой. Поэтому совет называет ОБА законных ответа: путь настоящей цели либо копию.
        lines[2] = ("Что делать: назови ПУТЬ той цели, к которой операция обратится — если "
                    "работаешь с копией, объектом обязана стоять КОПИЯ, а не оригинал. Путь "
                    "должен стоять в самой команде или исполняемым литералом в коде (имя, "
                    "склеенное из переменных, гард прочитать не может). Операция НЕ выполнена.")
...
def card_gate(kind, obj, num=""):
    ...
    СЕГОДНЯ ФУНКЦИЯ ВОЗВРАЩАЕТ True БЕЗУСЛОВНО, и сказано это первой строкой намеренно: читатель
    не должен выводить поведение из списка ниже, который описывает, КАК правило дошло до такого
    вида. Тихой карточки не бывает — всё, что доходит до `card_decision`, уже доктринально красное
    (`_stays_red`) и уже прошло проверку командной позиции (`_verb_acts`).

        • ОБЪЕКТ карточку БОЛЬШЕ НЕ ГЕЙТИТ (02.08.2026, класс Д-3 — разбор и числа ниже). Было:
          «нет объекта — признак сработал на ПОДСТРОКЕ, показывать нечего, идёт строка в журнал».
        ...
        • Hard-блок и секреты (`_HARD_CARD`) НЕ МОЛЧАТ никогда — как и было. Деньги тоже. Теперь
          это не исключение из гейта, а названное ядро доктрины (см. комментарий у списка).
          ПОПРАВКА 17.08.2026: «не молчат» — это не «спрашивают». С НАЗВАННЫМ объектом они
    ...
    правила, и имя зеркалит полосу сервера, где правило пока прежнее. `card_decision` спрашивает
    его, а не зашивает ответ у себя, — чтобы правка правила (в любую сторону, включая обратную)
    была правкой одной функции. Текущее значение названо прямо, поэтому мёртвой посылки здесь
    нет: ветка `journal` в `card_decision` сегодня недостижима, и об этом сказано и там.
    ...
    ОСТАТОК ЗАКРЫТ 02.08.2026 ЗАМЕРОМ (класс Д-3). Здесь стояло: «у ОБЫЧНОГО вида та же посылка
    мертва ровно так же, но правило журнала оставлено как было… замер частоты ложных срабатываний
    для них не делался. Это остаток, а не доказанная безопасность». Замер сделан — по ПОЛЮ строки
    журнала гарда, а не подстрокой (подстрочный счёт врёт: строка лога несёт текст команды, и
    собственная команда `grep journal` попадает в счёт как срабатывание):

        22.07–01.08, 10673 решений: journal — 22 события, И ВСЕ ДВАДЦАТЬ ДВА на ДВУХ видах —
                                    `kill` ×11 и `schtasks` ×11;
        01.08–02.08, 2813 решений:  journal — НОЛЬ.

    Оба числа говорят одно. Все 22 исторических события были НАСТОЯЩИМИ подстроками (`git add …
    .claude/settings.json`, `printf 'ASK …красные классы kill+schtasks…'`) — то есть посылка была
    верна для той популяции. `_verb_acts` (31.07) перенёс отсев подстроки на слой раньше и забрал
    у ветки ВЕСЬ её входной поток: с тех пор она не сработала ни разу. Значит ветка не просто
    лишилась посылки — она лишилась входа, и всё, что дошло бы до неё сегодня, по построению уже
    НЕ подстрока, а «действие разобрано, цель не извлеклась». Это ровно тот случай, ради которого
    заведён отказ высшего вида, и молчать о нём нельзя ни у какого вида.
    `kill` из тех 22 уже закрыт (`cbed09d`) — второй половиной был `schtasks`, обычный вид.

    ЦЕНА ПРАВКИ ИЗМЕРЕНА, А НЕ ОЦЕНЕНА: при сегодняшнем потоке она равна нулю лишних карточек в
    сутки, потому что журнальная ветка не срабатывает вовсе. Направление ошибки прежнее и
    fail-closed: лишняя карточка стоит одного круга, тихо исполненная операция Планировщика или
    выхода в сеть не стоит ничего вернуть.

    ЧТО НЕ ЗАКРЫТО И НАЗВАНО ЧЕСТНО: полоса СЕРВЕРА в этот проход не тронута (на VPS не ходили) —
    там правило объекта осталось прежним. Точки касания для зеркального фикса: `_ALWAYS_CARD` и
    место, где серверный гард гасит карточку при пустом объекте."""
    return True
...
def card_decision(kind, obj="", raw_cmd=""):
    ...
    ОБЫЧНЫЙ ВИД ДОБРАН 02.08.2026 (класс Д-3): у него та же посылка была мертва ровно так же, и
    остаток закрыт замером — журнальная ветка не срабатывала ни разу с 31.07, а все 22 её
    исторических события пришлись на `kill` и `schtasks`. Теперь молчания нет ни у какого вида.
    Любой сбой разбора → ask (FAIL-SAFE, как было).

    ЗАМОК РАСПРОСТРАНЁН НА ВСЕ 20 ВИДОВ 17.08.2026 (решение владельца «пустой объект — жёсткий
    отказ, а не карточка»). Было `is_top_tier(kind) and not _object_named(o)` — то есть замок
    накрывал 8 видов `_TOP_TIER` из 20 `_KIND_VOCAB`, и `env`/`sqlite`/`schtasks`/`network`/
    `unknown` в восьмёрку не входили. Живой повод назван цензом 16.08
    (`docs/artifacts/2026-08-16-guard-census-pc.md` §3.3): карточка `sqlite` 15.08 несла объект
    «цель записи не определена», механизм ЗНАЛ, что объекта нет (`_UNNAMED_OBJ` эту пометку
    опознаёт), и всё равно выписал ПОДТВЕРЖДАЕМУЮ карточку — потому что вид не высший. Замок
    был, входа в него у этого вида не было.

    ЧТО ЭТО НЕ ЕСТЬ — и это ИЗМЕРЕНО, а не обещано (артефакт 2026-08-17-guard-empty-object.md).
    Правка живёт ПОСЛЕ обнаружения и меняет только СУДЬБУ обнаруженного:
      • обнаружений признаком .... 2779 → 2779   (корпус 11966 строк лога гарда)
      • обнаружений доктриной .... 168 → 168     (`decide_for_role`, слой не тронут вовсе)
      • карточек владельцу ....... 39 → 33       (−6 = ровно число безобъектных не-высших)
      • отказов на месте ......... 129 → 135     (+6, те же шесть)
      • РАЗРЕШЕНО (исполняется) .. 11798 → 11798 (+0: ни одна операция не стала проходить)
    Направление ошибки прежнее и fail-closed: `deny` строже `ask`, новых разрешений правка не
    выдаёт ни одного. Разрешённое проходит как проходило: с НАЗВАННЫМ объектом карточка
    рождается у всех 20 видов из 20 (замер тем же прогоном).

    ЧЕМ ПЛАТИМ, ЧЕСТНО. У видов, где объект пуст ПО ПРИРОДЕ, операция теперь не подтверждаема
    вовсе — она обязана назвать цель в самой команде и повториться. Это сознательный размен:
    подтвердить вслепую нельзя (ровно так прошла карточка «лист не определён» 31.07), а
    молчаливого исполнения не было и нет. `_HARD_CARD` от этого не ослаб — он требует, чтобы
    вид НЕ МОЛЧАЛ, и `deny` это требование удовлетворяет строже, чем `ask`."""
    try:
        o, n = _card_fields(kind, obj, raw_cmd)
        # ПОРЯДОК ЗНАЧИМ И ПЕРЕСТАВЛЕН 17.08.2026 вместе с расширением: отказ считается ДО
        # `card_gate`. Сегодня это ничего не меняет (гейт безусловен, замер: 0 строк из 11966),
        # но снимает латентный тихий путь — если журнальную ветку когда-нибудь оживят, БЕЗ
        # объекта операция обязана кончиться отказом, а не молчаливым исполнением. Ровно та
        # склейка «две верные правки дали неверное поведение», которой был класс A-54.
        if not _object_named(o):
        ...
        blocked = _card_blocked(kind, o, raw_cmd, obj)
        if blocked:
            _log("guard", "card", "deny", kind, blocked)
            return ("deny", _deny_text(kind, o, raw_cmd, blocked))
...
def card_or_journal(kind, obj="", raw_cmd=""):
    """→ текст карточки либо None. ПОМОЩНИК ТЕСТОВ, а не рабочий путь — и это МЕРЯЕТСЯ, а не
    обещается (`test_card_or_journal_is_test_only_and_blind`, 02.08.2026, класс Д-5).

    ЧТО ЗДЕСЬ БЫЛО НАПИСАНО НЕПРАВДОЙ. Докстринг говорил «оставлена прежней по сигнатуре: ЕЮ
    ПОЛЬЗУЮТСЯ ВЫЗЫВАЮЩИЕ». Замер: 28 вызовов, ВСЕ в `test_pretool_guard.py`, в проде ноль.
    То есть совместимость сохранялась ни с кем, а фраза заставляла читателя думать, что через
    эту обёртку идёт боевое решение.

    ЧЕМ ЭТО ОПАСНО, ДОСЛОВНО. Обёртка СХЛОПЫВАЕТ ТРИ РАЗНЫХ ИСХОДА В ДВА: `journal` (операция
    ИСПОЛНЯЕТСЯ молча) и `deny` (операция НЕ исполняется) дают одинаковый `None`. Ровно неверное
    чтение этого бита и было дефектом A-54: тест, написанный через `assertIsNone`, зелен и до
    правки, и после — он не различает «прошло молча» и «отказано». Поэтому дефект нашли не
    тесты, а замер ПОЛНЫМ конвейером (`decide` → `decide_for_role` → `card_decision`).

    ПРАВИЛО ДЛЯ НОВЫХ ТЕСТОВ: про ВЛАСТЬ (пройдёт операция или нет) спрашивать `card_decision` и
    сверять решение поимённо; эта обёртка отвечает только на вопрос «каков ТЕКСТ карточки»."""
...
#     значениями (объект длиной с MAX_PATH Windows 260, команда, обрезаемая до 200): было 1182
#     символа (`py_write`), стало 1402 (`read_secret`) — карточка выросла на ПОСЛЕДСТВИЕ «да» и
#     справку о защитах (03.09.2026). Тест `test_marker_card_max_fits_real_cards` меряет это
#     заново и падает, если формат карточки перерастёт порог: тогда порог пересчитывают, а не
#     режут живые карточки молча. Ровно это здесь и сделано: 1400 → 1460;
...
#     (`test_marker_card_max_leaves_room_for_three_blocks` в тестах демона). Верхний предел
#     считается оттуда: 3 × (N + 31) ≤ 4500 даёт N ≤ 1469, поэтому 1460 — с запасом в 9 символов
#     под клапаном и в 58 над самой длинной законной карточкой.
