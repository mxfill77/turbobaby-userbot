...
def _deny_text(kind, obj="", raw_cmd=""):
    ...
    lines = ["⛔ ОТКАЗ: " + ("высшая цена, а " if is_top_tier(kind) else "")
             + "ОБЪЕКТ НЕ НАЗВАН — " + _human(kind, ""),
    ...
    if " ".join(str(obj or "").split()) in _PY_UNSEEN:
        ...
        lines[-1] = ("Что делать: подай код так, чтобы гард его ВИДЕЛ — запусти скрипт путём, "
