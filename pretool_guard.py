...
# (`test_hard_block_and_money_always_card` и соседи), которые требуют от каждого вида отсюда
# карточки при пустом объекте. То есть потребитель у списка есть и он назван — ровно то, чего
# требует `_EXTERNAL_API` от функций. Ослаблять список нельзя: если правило карточки когда-нибудь
# снова начнут смягчать, эти семь обязаны остаться безусловными.
...
def card_gate(kind, obj, num=""):
    ...
    (`_stays_red`) и уже прошло проверку командной позиции (`_verb_acts`).
        ...
        • Hard-блок и секреты (`_HARD_CARD`) спрашивают всегда — как и было. Деньги тоже. Теперь
          это не исключение из гейта, а названное ядро доктрины (см. комментарий у списка).
        • ВЫСШИЙ ВИД (`_TOP_TIER`): решать, что с ним делать, обязан `card_decision`, а не эта
          функция. Без объекта он получит там `deny`. Разбор — ниже.
