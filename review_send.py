...
def classify_codex(
    *,
    ...
    """
    # Цену ищем в ОБОИХ потоках. Замер 01.09 на живом канале: когда stdout —
    # труба (а у отправщика он всегда труба), Codex кладёт в stdout ТОЛЬКО текст
    # ответа, а весь свой протокол вместе со строкой «tokens used» уводит в
    # stderr. Разбор одного stdout давал «цена неизвестна» при живой и
    # напечатанной цене — то есть отчёт о бюджете врал бы, ничего не сломав.
