...
class TestClass0ModelResolveAndTerm(unittest.TestCase):
    ...
    def test_live_xsr155_bare_day_gives_xsr_price_not_mt03(self):
        ...
        for ph in phrases:
            ...
            if not lines:
                ...
                self.assertNotIn("723", note, f"чужая цена MT-03 в ответе на: {ph}\n{note}")
                continue
            ...
            # ПЕРЕСЧИТАНЫ ВТОРОЙ РАЗ 06.09.2026 — решение владельца 06.09.2026-1 сдвинуло границу
            # корзины срока с 13 на 14, и весь этот класс кейсов («2 недели» = РОВНО 14 суток)
            # переехал из скидочной корзины в опорную: множитель 0.82 → 1.000, и только он.
            #   XSR 155   = 444 x 1.0 (P1) x 1.000 (корзина 7-14, 14 сут) = 444 ฿/день;
            #   MT-03 300 = 723 x 1.0 x 1.000 = 723 ฿/день.
            # Совпадение цены с базой здесь НЕ ошибка списывания: множитель опорной корзины равен
            # 1.000 по построению, а период P1 — низкий сезон с множителем 1.0.
            ...
            self.assertIn("444", lines[0], ph)                  # цена XSR155 — своя
            self.assertNotIn("723", lines[0], f"чужой тариф MT-03 в карточке XSR на: {ph}")
            for ln in lines[1:]:                                # дополнительные — только тот же класс
                self.assertTrue(ln.upper().startswith("MT-03"), f"чужой класс в подборе: {ln}")
                self.assertNotIn("444", ln, "цена XSR подставлена в карточку MT-03")
    ...
    def test_live_xsr155_resolved_quote_present(self):
        ...
        # ПЕРЕСЧИТАН ВТОРОЙ РАЗ 06.09.2026: граница корзины 13 → 14 (решение владельца 06.09.2026-1),
        # срок 14 сут лёг в опорную корзину 7-14 → 444 x 1.0 x 1.000 = 444 ฿/день.
        self.assertIn("444", note)
        self.assertNotIn("не удалось", note.lower())    # не свалились в «уточни модель/даты»
        lines = self._quote_lines(note)
        self.assertIn("444", lines[0])                  # своя цена в СВОЕЙ карточке (первой)
        self.assertNotIn("723", lines[0])               # подмены чужой карточкой нет
...
class TestStep2ModelTermQuoteDepositPercent(unittest.TestCase):
    ...
    def test_golden_xsr155_two_weeks_quote_with_deposit(self):
        ...
        # ПЕРЕСЧИТАНЫ ВТОРОЙ РАЗ 06.09.2026: решение владельца 06.09.2026-1 сдвинуло границу
        # корзины срока 13 → 14, и «2 недели» (РОВНО 14 сут) переехали в опорную корзину 7-14:
        # 444 x 1.0 (P1) x 1.000 = 444 ฿/день, итого 444 x 14 = 6216 ฿. Депозит не тронут.
        self.assertIn("444", note)                       # суточный тариф XSR155
        self.assertIn("6216", note)                      # итог за 14 дней = 444*14
        ...
        self.assertNotIn("723", line)                    # чужой тариф MT-03 не подставлен в её карточку
    ...
    def test_golden_paraphrases_model_term_carry_deposit(self):
        ...
        for ph in ("Здравствуйте! XSR 155 с 20 июля на 2 недели, посчитайте.",
            ...
            # ПЕРЕСЧИТАНЫ ВТОРОЙ РАЗ 06.09.2026 (граница корзины 13 → 14, решение владельца
            # 06.09.2026-1): 444 x 1.0 x 1.000 (7-14) = 444 ฿/день, итого 6216 ฿.
            self.assertIn("444", note, ph)
            self.assertIn("6216", note, ph)
    ...
    def test_golden_percent_of_calculation(self):
        ...
        # ПЕРЕСЧИТАНЫ ВТОРОЙ РАЗ 06.09.2026 (граница корзины 13 → 14): база 6216 ฿ (444 x 14),
        # процент от неё: 6216 x 0.10 = 621.6 → 622 ฿. Округление то же, менялась только база.
        self.assertIn("6216", note)                      # база расчёта — итог XSR155 на 14 дней
        self.assertIn("622 ฿", note)                     # 10% от 6216 = 621.6 → 622 (КОД посчитал)
    ...
    def test_golden_percent_paraphrases(self):
        for ph in ("10% это какая сумма",
            ...
            note = self._note(tr)
            self.assertIn("622 ฿", note, ph)      # 10% от итога правила 6216 ฿ (пересчёт 06.09.2026)
    ...
    def test_foreign_model_card_blocked_by_postcheck(self):
        ...
        # ПЕРЕСЧИТАНЫ ВТОРОЙ РАЗ 06.09.2026 (граница корзины 13 → 14, решение владельца
        # 06.09.2026-1): своя сумма = 444 x 14 = 6216 ฿; чужая = 723 x 14 = 10122 ฿.
        draft = ("XSR 155 — 6216 ฿ за 14 дней, депозит 7000 ฿. "
                 "А MT-03 — 10122 ฿ за 14 дней, депозит 15000 ฿.")
        out = suggest.postcheck_draft(draft, "ru", pricing_note=note)
        client = out.split("[уточнить", 1)[0]
        self.assertIn("6216", client)                    # своя сумма (из quote) цела
        self.assertIn("7000", client)                    # свой депозит цел
        self.assertNotIn("10122", client)                # чужая сумма MT-03 вырезана
    ...
    def test_class_offer_whitelists_only_computed_numbers(self):
        ...
        # ПЕРЕСЧИТАНЫ ВТОРОЙ РАЗ 06.09.2026 (граница корзины 13 → 14, решение владельца
        # 06.09.2026-1): XSR 155 = 444 x 14 = 6216 ฿, MT-03 300 = 723 x 14 = 10122 ฿.
        self.assertIn("10122", note)                     # MT-03 посчитана КОДОМ (та же дверь quote)
        # NMAX 155 классом НИЖЕ — в подбор не идёт, значит НЕ посчитана, значит её цифры выдуманы.
        # ВЫДУМАННОЕ ЧИСЛО НАРОЧНО ОСТАВЛЕНО 44444: после пересчёта белый список держит 444
        # (цена суток XSR), и 44444 — проверка того, что режется ЧИСЛО ЦЕЛИКОМ, а не подстрока.
        draft = ("XSR 155 — 6216 ฿ за 14 дней, депозит 7000 ฿. "
                 "А MT-03 — 10122 ฿ за 14 дней, депозит 15000 ฿. "
                 "А NMAX 155 — 44444 ฿ за 14 дней, депозит 3000 ฿.")
        out = suggest.postcheck_draft(draft, "ru", pricing_note=note)
        client = out.split("[уточнить", 1)[0]
        self.assertIn("6216", client)                    # своя сумма цела
        self.assertIn("10122", client)                   # посчитанная сумма соседа по классу цела
    ...
    def test_own_deposit_and_total_kept_by_postcheck(self):
        ...
        # 6216 ฿ = он же после сдвига границы корзины 13 → 14 (444 x 14), пересчёт 06.09.2026
        draft = "XSR 155: 6216 ฿ за 14 дней, депозит 7000 ฿."
