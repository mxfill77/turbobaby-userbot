...
def list_folder(prefix="", limit=0, env=None, get=None):
    """ДЕТИ ПАПКИ Brain (имена и file id) — GET list_brain_folder. ТОЛЬКО ЧТЕНИЕ.

    ЧЕМ ЭТО ОТЛИЧАЕТСЯ ОТ :func:`list_brain` — И ЗАЧЕМ НУЖНЫ ОБА. `list_brain`
    отдаёт РЕЕСТР (ключ → file id), то есть список того, что кто-то зарегистрировал;
    эта функция отдаёт СОДЕРЖИМОЕ ПАПКИ, то есть то, что в ней лежит. Разница не
    теоретическая: замер 03.09.2026 на живом мосту — детей 49, ключей в реестре 37,
    БЕЗ КЛЮЧА 15 документов. Документ, созданный прямо в папке (`shtab_box_probe`),
    реестру не виден вовсе, и его file id взять было неоткуда — читать его по имени
    мост отказывается дословно: «ИМЯ НЕ РАЗРЕШЕНО … в живом реестре такого ключа
    нет». Эта дверь закрывает ровно эту дыру.

    ЗАЧЕМ ЗДЕСЬ, А НЕ В РАЗОВОМ СКРИПТЕ — та же причина, что у :func:`list_brain`:
    адрес и токен моста берёт САМ доверенный писатель, и скрипт, читающий конфиг
    ради похода в мозг, — красный канал по гарду (класс 328).

    ОТДАЁТСЯ ОТВЕТ ЦЕЛИКОМ, А НЕ ОДИН СПИСОК ФАЙЛОВ, и это не лень. В ответе есть
    поле ``truncated`` — «упёрлись в limit», — и вот чего оно стоит на самом деле
    (замер 03.09, ``limit=3`` на папке из 49 файлов): вернулись
    ``KB_QUEUE_STATE``, ``KB_cowork_log``, ``KB_queue_state_pc`` — то есть НЕ ТРИ
    ПЕРВЫХ ПО АЛФАВИТУ. Мост перебирает детей в порядке Drive (произвольном),
    обрывается по лимиту и сортирует по имени УЖЕ ОБРЕЗАННЫЙ КУСОК. Значит при
    ``truncated=True`` список — это ПРОИЗВОЛЬНОЕ ПОДМНОЖЕСТВО, отсортированное так,
    что выглядит началом. Верни мы отсюда голый ``files``, признак усечения потерялся
    бы по дороге, и вызывающий уверенно взял бы «первый» документ, которого в
    настоящем начале нет. Судить об усечении обязан вызывающий, а спрятать признак
    здесь — значит решить за него молча.

    ``prefix`` — отбор НА СТОРОНЕ МОСТА по началу имени, РЕГИСТРОЗАВИСИМЫЙ (замер
    03.09: ``prefix='shtab_box'`` → 2, ``prefix='SHTAB_BOX'`` → 0). Пустой ответ при
    ``ok=True`` — честный ноль, а не отказ.

    → dict ответа моста: ``ok, folder_id, count, files[{name,id,mime}], folders,
    folders_count, prefix, scanned, truncated``.
    """
    url, token = _config(env)
    if not url or not token:
        raise BrainWriterError("нет BRIDGE_URL/BRIDGE_TOKEN в окружении/конфиге", 1)
    params = {"action": "list_brain_folder", "token": token}
    if prefix:
        params["prefix"] = str(prefix)
    if limit:
        params["limit"] = str(int(limit))
    try:
        r = _retry_read(lambda: (get or _get)(url, params))
    except Exception as e:
        raise BrainWriterError("list_brain_folder упал (%d попытки): %s: %s"
                               % (_READ_TRIES, type(e).__name__, e), 2)
    if not (isinstance(r, dict) and r.get("ok") and isinstance(r.get("files"), list)):
        raise BrainWriterError("list_brain_folder не ok: %s"
                               % json.dumps(r, ensure_ascii=False)[:300], 2)
    return r
