...
переписано: чистой признаётся ТОЛЬКО ДОКАЗАННАЯ цепочка. Порядок силы
(`НЕ ДОКАЗАН > НЕИЗВЕСТНО > ДОКАЗАН`) живёт ТАМ, в судье, и здесь не повторён ни одной строкой:
счётчику приходит УЖЕ СЛОЖЕННЫЙ вердикт цепочки, один из трёх слов.

Из трёх слов счётчик знает ДВА — `ДОКАЗАН` (`PROVEN`) и `НЕИЗВЕСТНО` (`UNKNOWN`); третьего ему
знать не нужно, потому что всё, что не первое и не второе, есть обрыв, и перечислять обрывы по
именам значило бы завести второй список, расходящийся с первым. Дословность ОБОИХ слов сторожит
тест сверкой с литералами судьи, а не дисциплина правки: два экземпляра одного слова расходятся
...
«НЕИЗВЕСТНО — ОБРЫВ» ЗВУЧАЛО ЖЁСТКО И БЫЛО СНЯТО РЕШЕНИЕМ ШТАБА 11.09.2026 (п. 2). Правило
стояло на доводе «серия — обещание, а не надежда», и он верен ровно наполовину: цепочка, про
которую нельзя доказать достижение цели, серию действительно не ПРОДОЛЖАЕТ. Но она её и не
ОБРЫВАЕТ — потому что осуждена не за работу, а за то, что прибор её не прочитал. Живой случай,
на котором это стало видно: заход 245 (10.09.2026) сделал работу, положил коммит `22f57a2` и
артефакт по названному адресу, а судья споткнулся о ФОРМУ значений внутри артефакта и выдал
`НЕИЗВЕСТНО`. Обрыв серии здесь мерил бы СУДЬЮ работой полосы.

ПОЭТОМУ ИСХОДОВ У ЦЕПОЧКИ ТРИ, А НЕ ДВА: чистая (`ДОКАЗАН`) · обрыв (`НЕ ДОКАЗАН` и всё
незнакомое) · НЕИЗВЕСТНАЯ — выходит из ЗНАМЕНАТЕЛЯ (`chains` на неё не растёт) и считается
отдельно (`unknowns`). Закон «„проверить невозможно“ не закрывает эпизод» (`CLAUDE.md`, О1–О4,
п. 3) от этого не нарушен ни строкой: неизвестная цепочка не становится ни чистой, ни зачтённой —
она перестаёт быть ЧУЖИМ ПРИГОВОРОМ полосе.

ЗАМОК ОТ ДЫРЫ НАЗВАН ЗДЕСЬ ЖЕ, потому что дыра очевидна: если «неизвестно» ни во что не входит,
им выгодно называть всё. Замков два, и оба обязательны. ПЕРВЫЙ: неизвестность БЕЗ НАЗВАННОЙ
ПРИЧИНЫ льготы не получает — она обрыв (`is_unknown` требует причину; п. 4 решения). ВТОРОЙ:
число неизвестных печатается ВСЕГДА, а доля выше пятой части — ТРЕВОГА ПРО СУДЬЮ, звучащая
наравне с красным (`render`, `UNKNOWN_ALARM_*`).
...
# ═════════════════════════ ПРАВИЛО ЧИСТОТЫ ═══════════════════════════════════════════════════
# ДВА слова из трёх, и оба ЗАИМСТВОВАНЫ, а не заведены: равенство с литералами судьи адреса
# сторожит тест. Третье слово счётчику не нужно — всё, что не `ДОКАЗАН` и не `НЕИЗВЕСТНО`, есть
# обрыв, и перечислять обрывы по именам значило бы завести второй список, расходящийся с первым.
PROVEN = "ДОКАЗАН"
UNKNOWN = "НЕИЗВЕСТНО"        # с 11.09.2026: не засчитывает и не рвёт — см. шапку

# ДОЛЯ НЕИЗВЕСТНОГО, ВЫШЕ КОТОРОЙ ЭТО ТРЕВОГА ПРО СУДЬЮ. Пятая часть — число Штаба (решение
# 11.09.2026, п. 3). Считается от ВСЕХ рассмотренных цепочек (зачтённые + неизвестные), а не от
# знаменателя: знаменатель на неизвестных как раз и уменьшается, и доля от него росла бы сама
# собой — тревога стала бы самосбывающейся, а значит бессмысленной.
...
UNKNOWNS = "unknowns"    # НЕИЗВЕСТНЫХ цепочек — вышли из знаменателя, но не из виду
...
WHY_UNKNOWN = "why_unknown"   # ПРИЧИНА последней неизвестной — СЛОВАМИ СУДЬИ
...
ROOT = "root"
CLOSED_AT = "closed_at"
REASON = "reason"        # ПОЧЕМУ судья сказал это слово. Для `НЕИЗВЕСТНО` — обязательное поле
...
def is_clean(verdict):
    """ЧИСТОЙ признаётся ТОЛЬКО ДОКАЗАННАЯ. Всё прочее серию не продолжает."""
    return verdict == PROVEN


def is_unknown(verdict, reason=None):
    """НЕИЗВЕСТНАЯ цепочка — та, про которую судья СКАЗАЛ ПОЧЕМУ. → bool.

    ПРИЧИНА ОБЯЗАТЕЛЬНА, и это не формальность, а второй замок правила (решение Штаба
    11.09.2026, п. 4). Неизвестность даёт льготу — выход из знаменателя без обрыва, — а льгота,
    которую можно получить молча, перестаёт быть исходом и становится способом не считаться.
    Вердикт `НЕИЗВЕСТНО` без названной причины поэтому НЕ неизвестен: он обрыв, как всё
    недоказанное.
    """
    return verdict == UNKNOWN and isinstance(reason, str) and bool(reason.strip())
...
def make_entry(key, verdict, root, closed_at, reason=None):
    ...
    `reason` — слова судьи. Для `НЕИЗВЕСТНО` без них льготы нет (см. `is_unknown`)."""
    return {KEY: str(key), VERDICT: verdict, ROOT: root, CLOSED_AT: closed_at, REASON: reason}
...
def zero_state(since, frozen=None):
    ...
    return {SINCE: since, UPDATED: since, STREAK: 0, RECORD: 0, CHAINS: 0, BREAKS: 0,
            SERVICE: 0, UNKNOWNS: 0, SEEN: [], LAST: None, WHY: "", WHY_UNKNOWN: "",
            FROZEN: past}
...
def fold(state, entries, now):
    ...
    ШЕСТЬ исходов у одной цепочки, и каждый назван числом в отчёте оборота:
      учтена чистой · учтена обрывом · НЕИЗВЕСТНА (в знаменатель не входит и серию не рвёт)
      · служебная (в счёт не входит) · прошлое (раньше включения) · ждёт (метки закрытия нет
      либо её не прочитать).
    """
    ...
    unknowns = state.get(UNKNOWNS, 0)
    last = state.get(LAST)
    why = state.get(WHY, "")
    why_unknown = state.get(WHY_UNKNOWN, "")

    turn = {"clean": 0, "broken": 0, "unknown": 0, "service": 0, "past": 0, "waiting": 0,
            "again": 0}
    for one in sorted(entries, key=_order):
        ...
        if is_unknown(verdict, one.get(REASON)):
            # НЕ ЗАСЧИТЫВАЕТ И НЕ РВЁТ: `chains` не растёт (выход из знаменателя), `streak` не
            # трогается вовсе. Учтённой цепочка при этом СТАНОВИТСЯ — иначе она вернулась бы
            # следующим оборотом и считалась заново.
            seen.append(key)
            unknowns += 1
            why_unknown = ("последняя неизвестная — цепочка %s: %s"
                           % (key, str(one.get(REASON)).strip()))
            turn["unknown"] += 1
            continue
        seen.append(key)
        chains += 1
    ...
    out = {SINCE: since, UPDATED: now, STREAK: streak, RECORD: record, CHAINS: chains,
           BREAKS: breaks, SERVICE: service, UNKNOWNS: unknowns, SEEN: list(seen), LAST: last,
           WHY: why, WHY_UNKNOWN: why_unknown, FROZEN: state.get(FROZEN)}
    return out, turn


def unknown_alarm(state):
    """Доля неизвестных выше пятой части? → bool. Знаменатель — ВСЕ рассмотренные цепочки.

    Рассмотренные — это зачтённые (`chains`) плюс неизвестные (`unknowns`). Считать от одного
    `chains` нельзя: он на неизвестных не растёт, и доля лезла бы вверх сама собой."""
    got = state if isinstance(state, dict) else {}
    unknowns = got.get(UNKNOWNS, 0)
    looked = got.get(CHAINS, 0) + unknowns
    return unknowns * UNKNOWN_ALARM_DEN > looked * UNKNOWN_ALARM_NUM


def unknown_line(state):
    """Строка про неизвестные — рядом с серией и теми же словами, что красное.

    ТРЕВОГА ЗДЕСЬ ПРО СУДЬЮ, А НЕ ПРО РАБОТУ, и сказано это дословно: доля неизвестного выше
    пятой части означает, что прибор перестал читать продукт полосы, а не что полоса перестала
    его делать. Читающий, который спутает две новости, пойдёт чинить не то."""
    got = state if isinstance(state, dict) else {}
    unknowns = got.get(UNKNOWNS, 0)
    looked = got.get(CHAINS, 0) + unknowns
    words = "НЕИЗВЕСТНЫХ %d из %d рассмотренных (в знаменатель серии не входят)" % (unknowns,
                                                                                    looked)
    if not unknown_alarm(got):
        return words
    return ("ТРЕВОГА: %s — доля выше %d/%d; это про СУДЬЮ, а не про работу "
            "(продукт не прочитан, а не не сделан)"
            % (words, UNKNOWN_ALARM_NUM, UNKNOWN_ALARM_DEN))
...
def render(state, now=None):
    ...
    lines = ["СЧЁТ СЕРИИ полосы ПК (чистая цепочка = ДОКАЗАННАЯ; служебные корни не в счёте)",
             ...
             # ОТДЕЛЬНОЙ СТРОКОЙ И ВСЕГДА, в том числе нулём — там же, где показана серия
             # (решение Штаба 11.09.2026, п. 3). Число, которое печатается только когда оно
             # ненулевое, читается как «такого не бывает».
             "  %s" % unknown_line(state),
    ...
    words = state.get(WHY_UNKNOWN)
    if words:
        # ПРИЧИНА НЕИЗВЕСТНОСТИ — СЛОВАМИ, а не одним числом: «неизвестно» без причины к
        # льготе не допускается вовсе, и показать льготу, не показав её основание, нельзя.
        lines.append("  %s" % words)
