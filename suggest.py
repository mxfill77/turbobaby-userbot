...
def _smoke_checks(draft, exp) -> list:
    ...
    # КОД: «свободен на эти даты» дописывается в quote-блок ТОЛЬКО под `if q.get("available")`
    # (_client_price :3458/:3477), а ветка `ok` выдаётся лишь по СВОБОДНОМУ юниту (pricing:278) —
    # значит наличие в exp["j_line"] и есть то самое q["available"], принесённое ЕДИНСТВЕННЫМ
    # носителем, который переходит границу «печать → проверяющий» (второй котировки не заводим:
    # это было бы ДРУГОЕ чтение склада в другой миг). Нет утверждения в блоке → None, как раньше:
    # fail-closed цел, отрицание/дефицит/спеццены судятся ровно как до правки (22.08.2026).
