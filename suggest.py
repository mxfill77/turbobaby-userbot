...
#   | замер=НЕТ, число круглое — обоснования в коде не было ни строкой. Что о нём ИЗВЕСТНО с
#   живого корпуса 06.09 (10 сборок журнала продукта): 57.6 · 59.4 · 64.3 · 64.5 · 66.6 · 68.4 ·
#   82.5 · 114.3 · 120.0 · 120.0с, медиана 67.5с. Ниже медианы порог НЕ стои́т (в отличие от
#   бюджета сторожа), но 2 сборки из 10 уткнулись в потолок и выбросили 44 и 41 котировку из
#   111. Это САМЫЙ ДОРОГОЙ невыясненный порог ценового пути — и он остаётся невыясненным
#   намеренно: своего замера у него нет, а менять значение без замера запрещено
#   | снят=— | делится=все quote сборки (~111)
#   | предел-единицы=НЕТ. Чем грозит: у quote нет своего потолка стенного времени, а дедлайн
#   абсолютный — значит в момент его наступления ВСЕ ещё не поспевшие quote выбрасываются
#   разом, и цена одного залипания равна не одному юниту, а всему хвосту обхода. Замка вида
#   PRICE_GATE_ONE_SEC здесь нет. Заводить его без своего замера НЕЛЬЗЯ
#   | род=латентность | артефакт=docs/artifacts/2026-09-06-правило-порога-запись-замера-06.09.md
...
def price_sheet_status(ds, getter=None, _now=None, _fleet=None, _gate=None):
    ...
    if cut_by_deadline:
        log.warning("SHEET: ПОРОГ СРАБОТАЛ — дедлайн %sс выбросил %d quote из %d (%.0f%%); "
                    "сетка строится на неполных данных. Первые выброшенные: %s",
