...
def enforce_vehicle_word(draft, vehicle_type, lang="ru"):
    """ПОМЕТКА МЕНЕДЖЕРУ вместо слепой замены слова (28.07.2026).
    ...
    «байкер» → «авто». На смешанном запросе detectVehicleType даёт приоритет 'car', поэтому
    ...
    запросе, черновик уходит модератору С ПОМЕТКОЙ, и решает человек. Идемпотентна: повторный вызов
    второй пометки не добавит. 'bike'/'unknown' → черновик БАЙТ-В-БАЙТ, как и раньше."""
    if not vehicle_mismatch(draft, vehicle_type):
        return draft
    note = _VEHICLE_NOTE["en" if str(lang or "").lower().startswith("en") else "ru"]
    if note in draft:
        return draft
    return draft.rstrip() + "\n" + note
