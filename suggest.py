...
def _resolve_model_price(model, ds, de, hint_days, monthly, getter=None, name_filter=None):
    """Цена для ОДНОЙ модели по датам ds..de. Возвращает (kind, phrase, quote):
      ...
      noprice_no_row / noprice_not_judged — ТОТ ЖЕ фолбэк и ТА ЖЕ фраза, но с ИМЕНЕМ причины:
            ...
            исход «модель без цены» — см. `noprice_gate` и его границы класса.
    ...
    # суммы, маркерный режим) побайтно прежнее — вклад ровно нулевой. Свежесть сюда не доходит:
    # гашение сторожа `price_gate` выше по этой же функции ключа `noprice` не несёт.
    ...
    return (noprice_gate.kind_of(res) or "error", fallback, None)
