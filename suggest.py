...
import price_source  # СЧЁТ ПО ФАЙЛУ price_source.json — с 12.09 только ВНУТРЕННИЕ прикидки
import price_gate  # сторож свежести записанного правила: устарело → цену не называем
import door_price  # ИСТОЧНИК ЦЕНЫ КЛИЕНТУ: число ЖИВОЙ ДВЕРИ (решение владельца 12.09.2026)
...
#
# ПАКЛОК 490, А НЕ 390 — И ЭТО НЕ ОПЕЧАТКА, КОТОРУЮ НАДО «ПОЧИНИТЬ» ОБРАТНО. Источник строки
# выше (`turbobaby_faq_v1.md`) держит Паклок в группе 390 вместе с Кароном; живые зоны моста
# (`delivery_zones_get` → `fixtures/delivery_zones_get.live.json`: `["Паклок", 7.9645, 98.4085,
# 490, 5]`) — 490. Две сетки в одном боте спорили, и спор решён СЛОВОМ ВЛАДЕЛЬЦА 06.09.2026-3
# («паклок — 490», узел `KB_business_rules`): статический текст промпта приведён к живому
# резолверу, а не наоборот. Правка ровно одной зоны: КАРОН остался 390 — решение владельца
# сказано про Паклок и на соседа не растягивается. Сверив эту строку с CRM или с FAQ, легко
# объявить её расхождением — она им и является, и так решено намеренно.
# ОТКАТ: вернуть «Карон/Паклок 390; Ката/Кату 490» — но только новым словом владельца.
#
# СТУПЕНЕЙ СКИДКИ ЗА СРОК ЗДЕСЬ БОЛЬШЕ НЕТ — И ЭТО НЕ ПОТЕРЯ ПРИ СЛИЯНИИ (правка 13.09.2026,
# задание 62-a2). До неё в перечне стояла строка «- Скидки за срок: неделя ~6-15%, 2 недели
# ~15-25%, месяц ~35-50%.», и ОДИН И ТОТ ЖЕ промпт нёс два спорящих утверждения о скидке: голые
# ступени под зонтиком «Прайс-ОРИЕНТИР ДЛЯ МЕНЕДЖЕРА» и правило ниже, прямо запрещающее ступени
# называть. Спор разведён В ПОЛЬЗУ ЗАПРЕТА, и вот цена ошибки наружу, которой выбор обоснован:
#   1) ЧИСЛО. Верх ступеней — 50% за месяц. Самая глубокая скидка, названная владельцем в живой
#      редакции узла `business_rules`, — 25% на скутеры (дно низкого сезона, июнь-октябрь).
#      Утёкшая наружу ступень обещает клиенту ВДВОЕ больше, чем владелец разрешал где бы то ни
#      было в году, и обещает это деньгами.
#   2) ИСТОЧНИК. Ступени пришли из `turbobaby_faq_v1.md` (срез CRM), решением владельца они не
#      являются; узел от 16.08.2026 задаёт скидку МЕСТОМ В КАЛЕНДАРЕ, а не сроком аренды.
#   3) ЗОНТИК НЕ ЗАЩИЩАЛ. «Ориентир для менеджера» — слова в том же тексте, который читает
#      голова: на утечку числа они не влияют никак, а спор с правилом ниже создавали.
# ЖИВОЙ ИСТОЧНИК СКИДКИ ОСТАЛСЯ ОДИН и правкой не тронут: строка J Календаря (quote), которая
# приходит со СВОИМ процентом по конкретным датам — «(Скидка за срок 15%, 480 ฿ в день)» —
# и едет в черновик посимвольно. Прайсовых чисел правка не касается.
# ОТКАТ: вернуть строку ступеней в перечень — но только новым словом владельца и вместе с
# пересмотром записи `no_discount_peak` реестра `rules_decided.py`, иначе спор вернётся.
#
# ПОПРАВКА 62-a3 (13.09.2026) К ПУНКТУ 2 ВЫШЕ: «узел от 16.08 задаёт скидку местом в календаре»
# верно про РАЗДЕЛ 16.08, но опорой записи `no_discount_peak` он быть не может — слова «пик» в
# нём нет ни разу. Самое позднее живое слово владельца про скидку в пик лежит в блоке «ГИБКОСТЬ
# ЦЕНЫ: КОГДА УСТУПАЕМ» (17.08.2026, узел :291-408, устаревшим НЕ помечен): «в пик скидки не
# предоставляются вовсе». На него запись и переставлена, и строка КРИТФАКТОВ ниже — его дословие.
# Вывод про ступени правка НЕ меняет: верх ступеней 50% по-прежнему вдвое выше самой глубокой
# скидки, названной владельцем (25% на скутеры), и ступени по-прежнему пришли из среза CRM.
...
Доставка по районам (฿): Патонг/Банг Тао/Сурин/Камала 290; Карон 390;
Ката/Кату/Паклок 490; Раваи/Найхарн/Чалонг 590; Аэропорт 590-690. Забор бесплатно.
...
ПРЕДОПЛАТА (решение владельца 20.08.2026): сто процентов предоплаты берётся ТОЛЬКО когда бронь оформляется за месяц и раньше до дат, попадающих в декабрь, январь, февраль. Во ВСЕХ остальных случаях процент предоплаты клиенту НЕ называй вовсе — скажи, что уточним, и передай вопрос менеджеру.
СКИДКА (живая опора — узел business_rules, блок «ГИБКОСТЬ ЦЕНЫ: КОГДА УСТУПАЕМ», со слов владельца 17.08.2026): в пик скидки не предоставляются вовсе, а вне пика уступка возможна лишь при условиях, которых ты не видишь — ни сезона, ни занятости парка тебе не дано. Поэтому уступка остаётся решением человека: НИ ОДНОГО процента скидки и НИ ОДНОЙ ступени скидки за срок клиенту НЕ называй — скидку и итог даёт ТОЛЬКО блок ЦЕНА по конкретным датам.
ЧАСЫ ДОСТАВКИ (решение владельца 20.08.2026): доставки и заборы — до 17:30. Позже 17:30 ты сам НЕ решаешь: скажи, что время нужно согласовать, и передай вопрос менеджеру. Ни отказа, ни обещания от себя.
СОСТАВ КОМПЛЕКТА (решение владельца 20.08.2026): всегда входят держатель для телефона и шлемы; число шлемов НЕ утверждай, а спрашивай. ЗАМОК в комплект НЕ входит. Ничего сверх названного в комплект не добавляй."""
...
STYLE_GUIDE = (
    ...
    # ДО 12.09 здесь стояло безусловное «на эти даты бронь закрепляется по 100% предоплате» —
    # памятка учила называть сто процентов ВСЕГДА. Правило владельца 20.08 (узел business_rules,
    # блок ПЯТЬ ПРАВИЛ КОМПЛЕКТА И ПРЕДОПЛАТЫ, правило 2) прямо отменяет ту жёсткую строку:
    # сто процентов — только раннее оформление на декабрь-январь-февраль, иначе процент НЕ
    # называется вовсе. Дословная формулировка правила — в критичных фактах (реестр
    # `rules_decided.RULES`, ключ prepay_100); здесь стиль, а не источник правила.
    "• Про бронь: процент предоплаты САМ не называй — уточни модель и точные даты, скажи, что уточнишь условия брони по ним, и передай вопрос менеджеру.\n"
...
STYLE_FEWSHOT_PAIRS = [
    ...
    # ПРИМЕР ПЕРЕПИСАН 12.09 под правило владельца 20.08 (правило 2 блока ПЯТЬ ПРАВИЛ КОМПЛЕКТА
    # И ПРЕДОПЛАТЫ). Прежний ответ называл сто процентов безусловно и был вторым — после памятки
    # выше — местом, откуда голова брала отменённую формулировку: на «забронировать на февраль»
    # мы не знаем, за месяц ли до дат оформляется бронь, а от этого правило и зависит. Пример
    # больше не называет процента ни в каком случае; правило целиком — в критичных фактах.
    ("Хочу забронировать на февраль",
     "Продублируйте, пожалуйста, модель и точные даты — уточню условия брони по ним и вернусь. "
     "Оплата каким способом удобнее будет?"),
    ...
    # ВЕЛИЧИНА СТАВКИ УБРАНА 15.09 (задание 62-i): строение строки, порядок полей, тон и подводка
    # целы посимвольно — сменилось РОВНО одно: «7%» → место «N%». Пример учит ФОРМЕ (модель | дней
    # | стоимость | (скидка за срок, цена в день) | депозит), а величину скидки по конкретным
    # датам даёт ТОЛЬКО блок ЦЕНА. Слово «N%» взято не с потолка: им зовёт это же место шапка
    # блока ЦЕН ПО МОДЕЛЯМ («НЕ опускай часть про скидку за срок «(Скидка за срок N%, … в день)»»),
    # так что образец и указание теперь совпадают дословно.
    ("Добрый день, что есть с 08.10. На 8 дней из скутеров",
     "Здравствуйте, пару минут\n"
     "Вот что можем предложить:\n"
     "NMAX 155CC | дней: 8 стоимость: 2 507 (Скидка за срок N%, 313 в день), депозит: 3 000 бат\n"
     "FORZA 300CC | дней: 8 стоимость: 3 854 (Скидка за срок N%, 482 в день), депозит: 5 000 бат\n"
     "XMAX 300CC НОВОЕ ПОКОЛЕНИЕ | дней: 8 стоимость: 5 238 (Скидка за срок N%, 655 в день), депозит: 5 000 бат\n"
     "XADV 750CC | дней: 8 стоимость: 15 557 (Скидка за срок N%, 1 945 в день), депозит: 20 000 бат"),
    ("Можно на 2 дня?",
     "Скутеры у нас от 5 дней, мотоциклы от 3. На какие даты рассматриваете?"),
    # ПРИМЕР ПЕРЕПИСАН 12.09 под правило владельца 15.08 (узел business_rules, блок ЗАПРЕТ СКИДКИ
    # В ПИК). Прежний ответ называл клиенту ступени «неделя ~6–15%, две недели ~15–25%, месяц
    # ~35–50%» — и называл их БЕЗУСЛОВНО, тогда как в пиковый период скидки нет вовсе. Пример
    # учил голову обещать процент до всякого расчёта; проценты убраны, обещание заменено на
    # честный порядок «назовите даты — посчитаю». Сам запрет целиком — в критичных фактах.
    ("А если брать на подольше, скидка будет?",
     "За срок цена считается по датам — назовите период, посчитаю точно и скажу итог."),
    ...
    ("What do you have from Oct 8th, for 8 days? Scooters",
     ...
     # та же правка 62-i, что и у русского близнеца: величина ставки → место «N%»
     "NMAX 155CC | days: 8 total: 2 507 (Term discount N%, 313 per day), deposit: 3 000 baht\n"
     "FORZA 300CC | days: 8 total: 3 854 (Term discount N%, 482 per day), deposit: 5 000 baht\n"
     "XMAX 300CC NEW GEN | days: 8 total: 5 238 (Term discount N%, 655 per day), deposit: 5 000 baht"),
]
...
# ПАСПОРТ ПОРОГА: терпит=ВСЕ выученные правила делят одну книгу в 100 мест
#   | замер=НЕТ, число круглое. Ни длина промпта при полной книге, ни потеря качества от
#   вытеснения не мерены | снят=— | делится=выученные правила
#   | предел-единицы=НЕ НУЖЕН: единица — одно правило, места она занимает ровно одно.
#   Но у ВЫТЕСНЕНИЯ следа нет: FIFO выбрасывает самые ранние правила молча (это НЕ временной
#   порог, счётный, и след ему заводится не этим заданием — названо, чтобы не молчало)
...
# ПАСПОРТ ПОРОГА: терпит=НИЧЕГО не делит — это минимальный срок аренды скутера, правило цен
#   | замер=слово владельца, «правила цен v2, п.2» | снят=2026-07-15 | делится=НЕ ДЕЛИТСЯ
#   | предел-единицы=НЕ НУЖЕН | род=решение | артефакт=—
# (найден ОТКРЫТИЕМ этого правила 06.09, а не рукой: в списке премисы его не было. Ровно ради
#  таких находок открытие ищет ФОРМУ, а не имена из списка.)
SCOOTER_MIN_DAYS = 5
# ПАСПОРТ ПОРОГА: терпит=НИЧЕГО не делит — минимальный срок аренды мотоцикла, правило цен
#   | замер=слово владельца, «правила цен v2, п.2» | снят=2026-07-15 | делится=НЕ ДЕЛИТСЯ
#   | предел-единицы=НЕ НУЖЕН | род=решение | артефакт=—
MOTO_MIN_DAYS = 3
...
def _safe_quote_for_model(model, ds, de, getter=None, name_filter=None):
    ...
    `quote_for_model` и правкой не затрагиваются.

    ЧИСЛО КЛИЕНТУ — ТОЖЕ СЛОВО ДВЕРИ, С 12.09.2026 (решение владельца, узел `business_rules`,
    блок ДВЕРЬ-ИСТОЧНИК-1209: «можно ориентироваться на этот лист»). Счёт по файлу
    `price_source.reprice` ЗДЕСЬ БОЛЬШЕ НЕ ЗОВЁТСЯ: он подменял верное число двери своим и
    занижал его — замер 12.09 на XMAX 300 нового поколения дал 704 ฿/сут у двери (сошлось с
    листом владельца до бата) против 682 у файла, а на карточке из трёх моделей недобор 765 ฿
    (`docs/artifacts/2026-09-12-cena-tri-chisla-sezon.md`). Файл остался для внутренних
    прикидок и для ПРИБОРА РАСХОЖДЕНИЯ — он пишет разницу в журнал, но ответа не трогает.

    СТОРОЖ СВЕЖЕСТИ ФАЙЛА (`price_gate`) КЛИЕНТСКОЕ ЧИСЛО БОЛЬШЕ НЕ ГАСИТ, и это названо
    вслух, потому что до 12.09 гасил. Его предмет — старение СНИМКА: пока число клиенту давал
    файл, несвежесть обязана была означать молчание. Число даёт дверь, а она и есть живое
    положение ручек — гасить её верный ответ из-за старости снимка значило бы оставить файлу
    власть над клиентским ответом ровно там, где решение владельца её сняло. Вердикт сторожа
    остаётся у внутренних путей; здесь он не спрашивается ни одной веткой.

    ТРЕТИЙ ИСХОД ВМЕСТО МОЛЧАНИЯ: дверь не ответила или ответила БЕЗ ЦЕНЫ — `door_price`
    называет это состояние (`noquote`), и путь ответа обязан произнести слова про человека,
    не назвав ни одного числа. Прежние читатели видят прежние `status='error'`/`quote=None`."""
    try:
        res = pricing.quote_for_model(model, ds, de, _get=getter, name_filter=name_filter)
    except Exception:
        res = None
    try:
        return door_price.client_quote(res, model, ds, de, _bike_key)
    except Exception as e:
        # Источник цены НЕ смеет ронять путь ответа. Сбой ветки = третий исход (зовём человека),
        # а не старое число и не выдумка: числа, кроме двериного, у пути нет по построению.
        log.info(f"door_price: ветка упала ({type(e).__name__}) — цену считает человек")
        return {"status": "error", "quote": None,
                "noquote": door_price.KIND_DOOR_NO_PRICE}
...
def _resolve_model_price(model, ds, de, hint_days, monthly, getter=None, name_filter=None):
    """Цена для ОДНОЙ модели по датам ds..de. Возвращает (kind, phrase, quote):
      ...
      noprice_no_row / noprice_not_judged — ТОТ ЖЕ фолбэк и ТА ЖЕ фраза, но с ИМЕНЕМ причины:
            числа нет у САМОЙ МОДЕЛИ (записанное правило прочитано, живая дверь модель
            прокотировала, а базы для неё в правиле нет). На этих двух именах стои́т третий
            исход «модель без цены» — см. `noprice_gate` и его границы класса. С 12.09.2026 на
            КЛИЕНТСКОМ пути этот класс не наступает: число клиенту даёт дверь, и «в файле такой
            строки нет» перестало быть причиной молчать. Имя оставлено живым — его читают
            внутренние пути и его же тесты;
      door_no_price — ДВЕРЬ не ответила или ответила БЕЗ ЦЕНЫ (решение владельца 12.09.2026).
            Тот же фолбэк, и в нём ОБЯЗАТЕЛЬНЫЕ слова, что эту цену считает человек, — ни
            одного числа цены при этом не звучит. Молчание запрещено наравне с выдумкой.
    ...
    # ТРЕТИЙ ИСХОД «МОДЕЛЬ БЕЗ ЦЕНЫ» (правило владельца 19.08.2026, подключено 07.09.2026).
    # Здесь исход только ИМЕНУЕТСЯ, отвечает им `noprice_gate.note_for` — и ровно в одном месте
    # (единственная модель в `_price_note`). ФРАЗА ОСТАЁТСЯ ТА ЖЕ, БУКВА В БУКВУ: пока имя не
    # прочитано, поведение всех прочих веток (несколько моделей, подбор по классу, процент от
    # суммы, маркерный режим) побайтно прежнее — вклад ровно нулевой.
    #
    # СЛОВА ПРО ЧЕЛОВЕКА ДОПИСАНЫ 12.09.2026 (решение владельца, блок ДВЕРЬ-ИСТОЧНИК-1209:
    # «Дверь молчит — бот НЕ считает сам»). Стоя́т РОВНО ЗДЕСЬ, а не отдельной врезкой у
    # единственной модели, и это единственное место по счёту: фолбэк один на ВСЕ ветки без
    # числа (одна модель, несколько моделей, подбор по классу, срок без котировки), и записка,
    # положенная выше, до многомодельной ветки не доехала бы вовсе. Прежний текст СОХРАНЁН
    # ДОСЛОВНО и лишь продолжен: на его подстроках стоя́т чужие замки — разрез `moderation_card`
    # (`GAP_NO_QUOTE`) и голдены резака обещаний, — и переписать их этой правкой нечем.
    # Цифр в дописанном нет ни одной: белый список пост-чека строится из ЛЮБЫХ чисел записки,
    # и цифра здесь стала бы РАЗРЕШЁННОЙ к произнесению ценой.
    fallback = ("точная цена из Календаря сейчас недоступна — НЕ называй никакого числа "
                "(в т.ч. из FAQ); ответь, что уточнишь цену и вернёшься. "
                + door_price.HUMAN_LINE)
    return (noprice_gate.kind_of(res) or door_price.kind_of(res) or "error", fallback, None)
...
# ПАСПОРТ ПОРОГА: терпит=ОДИН кэш сетки на ВСЕХ клиентов окна 180с
#   | замер=НЕТ, число круглое. Ни частота изменения листа, ни цена промаха кэша не мерены
#   | снят=— | делится=все построения сетки в окне
#   | предел-единицы=НЕ НУЖЕН: окно не расходуется построениями, оно сравнивается с часами
#   | род=латентность | артефакт=docs/artifacts/2026-09-06-правило-порога-запись-замера-06.09.md
...
# ПАСПОРТ ПОРОГА: терпит=~111 живых quote сборки делят ВОСЕМЬ рабочих мест пула
#   | замер=НЕТ, число круглое. Мерена была ПОСЛЕДОВАТЕЛЬНАЯ стена (6м16с, ~3.4с на quote) и
#   названа непереносимой — но что даёт именно 8, а не 4 или 16, не мерил никто
#   | снят=— | делится=все quote сборки
#   | предел-единицы=НЕТ. Чем грозит: место занято НА ВСЁ ВРЕМЯ висящего quote (своего потолка
#   у quote нет, см. паспорт pricing.HTTP_TIMEOUT), поэтому 8 залипших дверей замораживают
#   сборку целиком — тот же класс голодания, что закрыт у сторожа 06.09 вторым этажом. Без
#   замера НЕ ЗАВОДИМ: правка без замера воспроизводит ровно тот дефект, который правило и
#   запрещает | род=латентность
...
#   | замер=12 повторов БОЕВОЙ price_sheet_status 06.09 при СНЯТОМ дедлайне (мерим, сколько
#   сборке НУЖНО, а не работу замка), на боевой кардинальности — 111 quote, 13 продуктов, пул 8.
#   ПЛОХОЕ окно двери (дверь воспроизводит 54 живых замера quote_price того же дня, 7 из них
#   выше собственного потолка двери): 99.4 · 101.3 · 102.7 · 103.1 · 103.1 · 104.1 · 104.7 ·
#   108.4 · 112.7 · 113.4 · 116.9 · 118.4с, медиана 104.7, МАКС 118.4 → двукратный запас 236.8.
#   Стенд сверен с живым: на ОБЫЧНОМ окне те же 12 повторов дают медиану 72.9с против живой
#   медианы 64.9с (56 сборок журнала продукта) — расхождение 12 %, стенд не льстит и не пугает.
#   Прежние 120 стояли ВЫШЕ медианы (тем и пережили 16 суток), но НИЖЕ 10 живых сборок из 56 и
#   ниже ВСЕХ ДВЕНАДЦАТИ повторов плохого окна: в плохом окне порог срабатывал ВСЕГДА, и цена
#   срабатывания замерена — 44 и 41 котировка из 111, выброшенные молча
#   | снят=2026-09-06 | делится=все quote сборки (~111)
#   | предел-единицы=НЕТ СВОЕГО. Частичный потолок у единицы есть, и он ЧУЖОЙ —
#   pricing.HTTP_TIMEOUT=20с на один GET. Он же даёт АНАЛИТИЧЕСКИЙ потолок сборки
#   ⌈111/8⌉ × 20 = 280с: выше этого числа законной сборки не бывает, и дедлайн выше 280 был бы
#   мёртвым кодом — потому 236.8 выбрано НИЖЕ него сознательно. Но сокет-таймаут НЕ равен
#   стенному времени вызова (он на recv, а не на вызов), поэтому дедлайн остаётся единственной
#   абсолютной стеной, и свой предел единицы всё ещё нельзя заводить без своего замера
...
_SHEET_DEADLINE = float(os.getenv("PRICE_SHEET_DEADLINE_SEC", "236.8") or "236.8")
                                # замер 06.09 (12 повторов боевой сборки при снятом дедлайне,
                                # плохое окно двери): худшая 118.4с, двукратный запас → 236.8.
                                # Прежние 120 были круглым числом без единой строки обоснования.
...
# ЗНАК РУЧКИ ПЕРЕВЁРНУТ 12.09.2026 ВМЕСТЕ С ИСТОЧНИКОМ (решение владельца, узел `business_rules`,
# блок ДВЕРЬ-ИСТОЧНИК-1209). ПО УМОЛЧАНИЮ СЧЁТ ПО ФАЙЛУ В СЕТКУ НЕ ИДЁТ: требование решения —
# «в клиентский ответ число файла не попадает НИ ОДНОЙ ВЕТКОЙ», а прайс-сетка парка есть ответ
# клиенту ровно так же, как точечная цена. Обе дороги снова берут число в одном месте — у живой
# двери, — и два разных прайса в одном сообщении по-прежнему невозможны.
# ЧТО ЭТА ПРАВКА СОЗНАТЕЛЬНО ВОЗВРАЩАЕТ, названо вслух: сезонную слепоту листа (замер 17.08,
# `69a6c9f` — занижение пика на 27.7-31.7 %). Она не опровергнута; владелец 12.09 велел
# ориентироваться на лист, зная про неё, и Штаб своей властью прежнее правило не возвращает.
# Ручка `PRICE_SHEET_RULE_ON` возвращает счёт по файлу одним словом, без коммита.
_SHEET_RULE_OFF = (os.getenv("PRICE_SHEET_RULE_ON", "").strip().lower()
...
def price_sheet_status(ds, getter=None, _now=None, _fleet=None, _gate=None):
    ...
    results = {}
    cut_by_deadline = []            # СЛЕД СРАБАТЫВАНИЯ порога: что именно он выбросил
    try:
    ...
    # ── СЛЕД СРАБАТЫВАНИЯ ПОРОГА (правило порога 06.09.2026, п.5 задания Штаба) ──────────────
    # Порог, который сработал и что-то выбросил, обязан оставить запись С ЧИСЛОМ выброшенного.
    # МОЛЧА ТЕРЯТЬ ЗАПРЕЩЕНО, и вот чем это стоило: до сегодня дедлайн писал только построчный
    # `log.info` на каждый юнит, а сводка складывала его потери со сбоями отдельных quote в одно
    # слово «пропущено». Отличить «дверь ответила отказом» от «мы перестали ждать» было нельзя
    # НИ ОДНОЙ строкой, и потому размер потери 06.09 (44 и 41 котировка из 111 в двух сборках из
    # десяти) пришлось добывать раскопками журнала, а не читать готовым числом.
    # Уровень WARNING, а не INFO, сознательно: срабатывание общего дедлайна — это НОВОСТЬ о том,
    # что сетка вышла неполной, а не рядовой шаг построения.
    #
    # ЧТО ДОБАВЛЕНО 06.09 ВТОРЫМ ЗАХОДОМ (задание 00-m-sheet-deadline.0906). Числа котировок
    # мало: клиент теряет не «котировки», а СТРОКИ ПРАЙСА. Модель, у которой дедлайн срезал ВСЕ
    # котировки, приходит в `_sheet_availability` с пустым `got` и получает вердикт `unchecked` —
    # и до сих пор уходила в журнал словами «занятость НЕ ПРОВЕРЕНА (признака нет в ответе)».
    # Это НЕ ошибка счёта, это ЛОЖНАЯ ПРИЧИНА: признака нет не потому, что дверь его не дала, а
    # потому, что её перестали ждать. Разбирающий эпизод читал «дверь чего-то не сказала» там,
    # где верно «мы не дождались», — то есть чинил бы дверь вместо порога.
    cut_set = set(cut_by_deadline)
    _own_pairs = lambda per_term: [bd for pairs in per_term.values() for bd in pairs]  # noqa: E731
    models_cut_whole = [lab for lab, _vs, pt in plan
                        if _own_pairs(pt) and all(bd in cut_set for bd in _own_pairs(pt))]
    models_cut_part = [lab for lab, _vs, pt in plan
                       if lab not in models_cut_whole and any(bd in cut_set for bd in _own_pairs(pt))]
    if cut_by_deadline:
        log.warning("SHEET: ПОРОГ СРАБОТАЛ — дедлайн %sс выбросил %d quote из %d (%.0f%%); "
                    "сетка строится на неполных данных. Моделей срезано ЦЕЛИКОМ %d из %d "
                    "(клиент их не увидит вовсе): %s; задето частично %d (вердикт занятости мог "
                    "измениться). Первые выброшенные котировки: %s",
                    _SHEET_DEADLINE, len(cut_by_deadline), len(futs),
                    100.0 * len(cut_by_deadline) / max(1, len(futs)),
                    len(models_cut_whole), len(plan),
                    ("; ".join(models_cut_whole[:6]) or "нет"),
                    len(models_cut_part),
                    "; ".join("%s→%s" % bd for bd in cut_by_deadline[:5]))
    ...
    for label, variants, per_term in plan:
        ...
        if _SHEET_AVAIL_OFF:                   # ручка отката: ветка не исполняется, путь прежний
            ...
            if verdict != _SHEET_FREE:
                ...
                # ПРИЧИНУ НЕ ПОДМЕНЯЕМ (06.09): у модели, срезанной дедлайном целиком, вердикт
                # остаётся `unchecked` (ветку исхода мы не трогаем — она про то, что делать
                # клиенту), но СЛОВАМИ говорится правда и с числом. Иначе потеря порога прячется
                # под чужой причиной, а именно это задание и запрещает.
                if label in models_cut_whole:
                    log.warning("SHEET: модель %s в сетку НЕ вошла — ДЕДЛАЙН %sс срезал ВСЕ %d её "
                                "котировки. Это НЕ «занятость не проверена»: признака нет потому, "
                                "что дверь перестали ждать, а не потому, что она смолчала",
                                label, _SHEET_DEADLINE, len(_own_pairs(per_term)))
                else:
    ...
    log.info(f"SHEET: сетка построена за {time.time() - t0:.1f}с — {len(rows)} моделей, "
             f"{len(futs)} quote ({len(futs) - n_ok} пропущено, из них дедлайном "
             f"{len(cut_by_deadline)}), пул {_SHEET_WORKERS}; "
             ...
             f"(из них дедлайном срезаны целиком {len(models_cut_whole)}), "
...
# ====== ПОДБОР ПО КЛАССУ (правила владельца 20.08.2026 + слово 12.09.2026, узел business_rules) ======
# Метка правила в мозге: «подбор по классу 20.08» и «ПОДБОР — СЛОВО ВЛАДЕЛЬЦА 12.09.2026». Взято ПО
# ИМЕНИ из живого реестра моста, не из пересказа. Лестница классов названа владельцем и задаёт слово
# «выше»; разнос парка — снимок 20.08 (37 сдаваемых единиц из 38; CLICK 125 в аренду не сдаётся).
#
# ЧТО ОТМЕНЕНО 12.09.2026 И ПОЧЕМУ. Владелец дословно: «только про спрошенную модель и одну-две
# альтернативы»; про прежний вывод — «причём тут, когда макси-скутер спрашивают, столько мотоциклов
# выходит». Отменены ДВЕ части правила 20.08: «ВСЕ свободные» (предела по числу нет) и «ИЛИ ВЫШЕ»
# (подъём по лестнице сам собой). Живой замер, на котором это видно: снимок кейса 1 от 11.09 —
# спрошен XMAX 300 (макси), в блоке 10 моделей, из них 7 мотоциклов. Остаются в силе оба запрета
# 20.08: классом НИЖЕ бот сам не предлагает, тяжёлый класс — только по слову клиента про мощность
# или опыт. Механика отмены: потолок класса в `class_offer_scope`, предел числа в
# `class_alternatives` (_CLASS_ALT_MAX), хвост «свободно ещё N» снят в `_class_offer_block`.
#
...
# ПАСПОРТ ПОРОГА: терпит=все модели класса делят ВОСЕМЬ рабочих мест пула
#   | замер=НЕТ, число круглое; взято зеркалом с _SHEET_WORKERS, а не измерено здесь
#   | снят=— | делится=quote всех моделей класса
#   | предел-единицы=НЕТ. Чем грозит — то же, что у _SHEET_WORKERS: место держит висящий
#   quote целиком. Без замера не заводим
#   | род=латентность | артефакт=docs/artifacts/2026-09-06-правило-порога-запись-замера-06.09.md
_CLASS_OFFER_WORKERS = int(os.getenv("PRICE_CLASS_OFFER_WORKERS", "8") or "8")
# ПАСПОРТ ПОРОГА: терпит=quote ВСЕХ моделей класса делят один дедлайн на всех
#   | замер=НЕТ, число круглое. И судить его НЕЧЕМ: строк `CLASS-OFFER:` в живом хвосте
#   журнала продукта на 06.09 было РОВНО НОЛЬ — ветка не оставляла следа вовсе, поэтому даже
#   сказать, срабатывал ли дедлайн хоть раз, было нельзя. След заведён этим заходом 06.09
#   (сводная строка со счётом выброшенного), но КОРПУСА у него ещё нет: первые числа появятся
#   не раньше, чем ветка отработает в бою | снят=— | делится=quote всех моделей класса
#   | предел-единицы=НЕТ. Чем грозит: то же, что у _SHEET_DEADLINE — дедлайн абсолютный, и в
#   момент его наступления все неуспевшие модели выпадают из подбора разом. Без замера не
#   заводим | род=латентность
#   | артефакт=docs/artifacts/2026-09-06-правило-порога-запись-замера-06.09.md
_CLASS_OFFER_DEADLINE = int(os.getenv("PRICE_CLASS_OFFER_DEADLINE_SEC", "60") or "60")
# Потолок сообщения Telegram. ЭТО ТЕХНИЧЕСКОЕ ОГРАНИЧЕНИЕ, А НЕ ПРАВИЛО ВЛАДЕЛЬЦА (правило теперь
# режет раньше — см. _CLASS_ALT_MAX), и после 12.09.2026 оно недостижимо: три строки против 4096.
_CLASS_OFFER_MSG_MAX = 4096
# ПРЕДЕЛ ЧИСЛА АЛЬТЕРНАТИВ — СЛОВО ВЛАДЕЛЬЦА 12.09.2026, дословно «одну-две альтернативы».
# Это НЕ порог-догадка и замера не ждёт: число названо человеком, а не измерено, и трогать его
# вправе только он. Прежнее правило 20.08 («все свободные того же класса или выше, предела по
# числу нет») отменено ровно этой строкой и строкой `class_offer_scope` (потолок = свой класс).
_CLASS_ALT_MAX = 2
...
def class_offer_scope(asked, heavy_ok=False, cheaper_ok=False):
    ...
    СЛОВО ВЛАДЕЛЬЦА 12.09.2026 — «ТОЛЬКО ТОТ ЖЕ КЛАСС». Дословно: «только про спрошенную модель и
    одну-две альтернативы»; и про прежнее поведение — «причём тут, когда макси-скутер спрашивают,
    столько мотоциклов выходит». До этого дня здесь стояло «того же класса ИЛИ ВЫШЕ», и на живом
    парке спрошенный макси-скутер вытаскивал семь мотоциклов (замер: снимок кейса 1 от 11.09 —
    10 моделей в блоке, из них 7 мотоциклов). Верх лестницы САМ БОТ БОЛЬШЕ НЕ ПРЕДЛАГАЕТ.

    Три правила владельца сидят ровно здесь и больше нигде:
      • ТОЛЬКО ТОТ ЖЕ КЛАСС (12.09.2026): пол И ПОТОЛОК списка равны рангу спрошенного;
      • НИЖЕ КЛАССОМ БОТ САМ НЕ ПРЕДЛАГАЕТ НИКОГДА (20.08, остаётся) — пол опускает только явная
        просьба клиента «дешевле/проще» (cheaper_ok);
      • ТЯЖЁЛЫЙ КЛАСС — только если клиент САМ сказал про мощность, размер или опыт (20.08,
        остаётся): heavy_ok — единственный ключ, которым потолок поднимается выше своего класса."""
    r = _class_rank(asked)
    if r is None:
        return None
    out = {_CLASS_LADDER[r]}
    if cheaper_ok:
        out |= {c for i, c in enumerate(_CLASS_LADDER) if i < r}
    if heavy_ok:
        out.add(_CLASS_HEAVY)
    return out
    ...
    try:
        ...
        for i, fut in futs.items():
            try:
                ...
                res[i] = None
                cut_by_deadline.append(products[i][0])
                log.info(f"CLASS-OFFER: {products[i][0]} не уложилась в дедлайн "
                         f"{_CLASS_OFFER_DEADLINE}с или упала — в подбор не идёт")
    finally:
        ex.shutdown(wait=False, cancel_futures=True)
    # ── СЛЕД СРАБАТЫВАНИЯ ПОРОГА (правило порога 06.09.2026, п.5 задания Штаба) ──────────────
    # Здесь молчание было ХУЖЕ, чем у сетки: строк `CLASS-OFFER:` в живом хвосте журнала продукта
    # на 06.09 было РОВНО НОЛЬ — то есть сказать, срабатывал ли этот дедлайн хоть однажды, было
    # нельзя вовсе, и потому у него нет и не может быть замера. Со следом корпус начнёт копиться:
    # первое же срабатывание оставит ЧИСЛО выброшенных моделей, а не догадку.
    if cut_by_deadline:
        log.warning("CLASS-OFFER: ПОРОГ СРАБОТАЛ — дедлайн %sс выбросил %d моделей из %d; "
                    "подбор клиенту уходит неполным. Выброшены: %s",
                    _CLASS_OFFER_DEADLINE, len(cut_by_deadline), len(products),
                    ", ".join(cut_by_deadline[:5]))
    free, skipped = [], []
...
def _alt_price(q):
    """Число, по которому альтернативы сравниваются с ЦЕНОЙ СПРОШЕННОЙ. → float | None.

    Берём сумму за период (`total`) — она уже учла срок и скидку, то есть ровно то, что клиент
    сравнивает глазами. Нет суммы — дневная ставка (на общем для всех строк периоде порядок тот же).
    Нет ни одной — None, и такая строка в арифметику «дешевле/дороже» не идёт: выдумывать цену,
    чтобы отсортировать, нельзя."""
    if not isinstance(q, dict):
        return None
    for key in ("total", "day_price"):
        v = q.get(key)
...
def class_alternatives(free, primary_q=None, limit=None):
    """НЕ БОЛЬШЕ ДВУХ альтернатив из свободных, ближайшие по цене к спрошенной. → list (подмножество
    `free`, порядок исходный).

    СЛОВО ВЛАДЕЛЬЦА 12.09.2026, арифметика дословно: «одну-две альтернативы» — одна ДЕШЕВЛЕ
    спрошенной и одна ДОРОЖЕ, обе ближайшие к ней по цене; с одной стороны пусто — берём обе с
    другой. Прежнее правило 20.08 «все свободные, предела по числу нет» отменено этой строкой.

    ОПОРЫ МОЖЕТ НЕ БЫТЬ: спрошенная ЗАНЯТА → её quote пуст (`_resolve_model_price` отдаёт
    `("none", …, None)`), и «дешевле/дороже» отсчитывать не от чего. Тогда берём ДВЕ САМЫЕ
    ДЕШЁВЫЕ — это названный выбор, а не умолчание: цену спрошенной мы не знаем и придумывать её
    ради красивой симметрии не станем."""
    limit = _CLASS_ALT_MAX if limit is None else int(limit)
    rows = [(_alt_price(item[2]), i, item) for i, item in enumerate(free or [])]
    priced = [r for r in rows if r[0] is not None]
    base = _alt_price(primary_q)
    if base is None:
        pick = sorted(priced, key=lambda r: (r[0], r[1]))[:limit]
    else:
        cheaper = sorted([r for r in priced if r[0] <= base], key=lambda r: (base - r[0], r[1]))
        dearer = sorted([r for r in priced if r[0] > base], key=lambda r: (r[0] - base, r[1]))
        pick = [side[0] for side in (cheaper, dearer) if side][:limit]
        rest = sorted(cheaper[1:] + dearer[1:], key=lambda r: (abs(r[0] - base), r[1]))
        pick += rest[:max(0, limit - len(pick))]        # с одной стороны пусто → обе с другой
    if len(pick) < limit:                               # без числа сортировать нечем — идут в хвост
        pick += [r for r in rows if r[0] is None][:limit - len(pick)]
    return [item for _p, _i, item in sorted(pick, key=lambda r: r[1])]
...
def _class_offer_block(primary, alt_lines, lang="ru", msg_max=None):
    ...
    остался бы без ответа на свой же вопрос).

    ХВОСТ «СВОБОДНО ЕЩЁ N МОДЕЛЕЙ» СНЯТ СЛОВОМ ВЛАДЕЛЬЦА 12.09.2026 (он же снял и то, что хвост
    обслуживал: «все свободные, предела нет»). Хвост был страховкой от МОЛЧАЛИВОЙ обрезки длинного
    списка — а списка больше нет: в блок приходит основная модель и НЕ БОЛЬШЕ ДВУХ альтернатив
    (`class_alternatives`), три строки против потолка в 4096 символов. Число не влезших по-прежнему
    возвращается вызывающему и уходит в журнал — молчания в ЛОГЕ не наступает, снят только хвост
    в тексте клиенту."""
    cap = int(msg_max or _CLASS_OFFER_MSG_MAX)
    head = [primary] if primary else []
    used, kept = sum(len(x) + 1 for x in head), []
    for line in alt_lines:
        if used + len(line) + 1 > cap:
            break
        kept.append(line)
        used += len(line) + 1
    return "\n".join(head + kept), len(alt_lines) - len(kept)


def _class_offer_note(busy_primary: bool, lang: str = "ru", no_alt: bool = False) -> str:
    ...
    ЗАЧЕМ ЗДЕСЬ ПОВТОРЯТЬ АРИФМЕТИКУ БЛОКА (слово владельца 12.09.2026, требование задания).
    Инструкция и блок — два голоса об одном и том же ответе, и разойтись им нельзя: скажи записка
    «модели того же класса ИЛИ ВЫШЕ», пока код кладёт в блок только свой класс, — голова добавит
    мотоциклов словами, без единой цифры, и пост-чек цен этого не поймает (он сторожит ЧИСЛА).
    Поэтому текст ниже называет и предел («не больше двух»), и класс («тот же»)."""
    common = ("Каноничные строки цен вставит КОД ДОСЛОВНО на место метки [QUOTE]: выведи метку "
              ...
              "предлагай и цен по ним не называй — НИ ОДНОЙ, даже без цены и даже «для примера». "
              "Классом НИЖЕ спрошенного сам ничего не предлагай — только если клиент сам попросит "
              "дешевле или проще.")
    if no_alt:
        return ("ЦЕНА из Календаря бронирования ПОСЧИТАНА по спрошенной модели — она в блоке одна, "
                "и это не обрезка: свободных ТОГО ЖЕ класса на эти даты больше нет. Перечня "
                "моделей другого класса НЕ давай и цен по ним НЕ называй; вместо перечня — ОДНА "
                "короткая строка-предложение посмотреть класс выше, если клиент готов. " + common +
    ...
    if busy_primary:
        return ("ЦЕНА из Календаря бронирования: ЗАПРОШЕННАЯ МОДЕЛЬ ЗАНЯТА на эти даты (Календарь "
                ...
                "и прямо, что именно её на эти даты предложить не можем, и СРАЗУ дай замену — в "
                "блоке НЕ БОЛЬШЕ ДВУХ моделей ТОГО ЖЕ класса, свободных на эти даты, они уже "
                "посчитаны. " + common +
    ...
    return ("ЦЕНА из Календаря бронирования ПОСЧИТАНА по спрошенной модели и НЕ БОЛЕЕ ЧЕМ ДВУМ "
            "альтернативам: первой в блоке идёт та, что спросил клиент — она ОСНОВНОЙ вариант; "
            "следом идут одна-две модели ТОГО ЖЕ класса, ближайшие к ней по цене и свободные на "
            "эти даты. Больше вариантов не предлагай и другого класса сам не поднимай. Перед "
            "меткой дай короткую подводку в духе «Вот что можем предложить на эти даты:». "
            + common + " «Уточню цену и вернусь» НЕ пиши: расчёт уже есть.")
...
def _class_offer_full_note(label, kind, phrase, q, ds, de, hints, lang="ru", getter=None,
    ...
    None возвращается по пяти названным причинам, и каждая — сознательная рамка правки:
      ...
      • свободных ТОГО ЖЕ КЛАССА нет, А СПРОШЕННАЯ ЗАНЯТА. Тогда бот НЕ скатывается классом ниже
        сам: занятая модель честно уходит в прежний отказ. Если же спрошенная СВОБОДНА, ветка
        возвращает записку с одной посчитанной строкой и предложением посмотреть класс выше
        (слово владельца 12.09.2026) — None здесь больше не возвращается."""
    ...
    # СЛОВО ВЛАДЕЛЬЦА 12.09.2026: из свободных в блок идут НЕ БОЛЬШЕ ДВУХ, ближайшие по цене.
    alts = class_alternatives(free, q)
    if not alts:
        if primary is None:
            # Спрошенная ЗАНЯТА и заменить нечем: прежний честный отказ («уточню наличие и
            # вернусь») остаётся БАЙТ-В-БАЙТ. Строку «посмотрите класс выше» сюда не вешаем
            # СОЗНАТЕЛЬНО — прицепить её не к чему (посчитанного блока в этой ветке нет вовсе,
            # маркер [QUOTE] без блока остался бы в тексте клиента), а отказ уже обещает человека.
            log.info(f"CLASS-OFFER: «{label}» занята, свободных ТОГО ЖЕ класса нет "
                     f"(отсеяно {len(skipped)}); классом НИЖЕ сами не предлагаем — путь прежний")
            return None
        # Спрошенная свободна, а в её классе больше никого: блок из одной посчитанной строки плюс
        # ОДНА строка-предложение посмотреть класс выше — без перечня моделей и без цен.
        log.info(f"CLASS-OFFER: «{label}» — свободных ТОГО ЖЕ класса нет (отсеяно {len(skipped)}); "
                 f"вместо перечня — одна строка про класс выше")
        block, _more = _class_offer_block(primary, [], lang)
        return (_class_offer_note(False, lang, no_alt=True)
                + "\n" + _QUOTE_OPEN + "\n" + block + "\n" + _QUOTE_CLOSE)
    alt_lines = []
    for alt_label, alt_phrase, alt_q in alts:
        if passport_dep:
    ...
    log.info(f"CLASS-OFFER: «{label}» — свободных того же класса {len(free)}, в блок идут "
             f"{len(alt_lines)} (предел владельца {_CLASS_ALT_MAX}, ближайшие по цене)")
    block, more = _class_offer_block(primary, alt_lines, lang)
    if more:
        log.warning("CLASS-OFFER: в сообщение не влезло %d из %d альтернатив — клиенту про это НЕ "
                    "сказано (хвост с числом снят словом владельца 12.09), новость живёт здесь",
                    more, len(alt_lines))
...
def _build_pricing_note_core(hints: dict, lang: str = "ru", getter=None, today=None) -> str:
    ...
    # ГРАНИЦА СЕЗОНОВ — ТРЕТИЙ ИСХОД «не считаю, зову человека» (правило владельца 19.08.2026,
    # подключено 06.09.2026). Стоит РОВНО ЗДЕСЬ и ровно в одном месте:
    #   · ПОСЛЕ резолва модели — карточке владельцу нужна модель парка, а не серийный корень, и
    #     неоднозначная серия обязана выиграть у границы (её переспрос дешевле и тише карточки);
    #   · ДО обращения к Календарю — считать срок, который правило посчитает по дате НАЧАЛА,
    #     значит родить число вне данных, а потом гасить его пост-чеками;
    #   · НЕ в прайс-сетке парка и НЕ на месячном тарифе: класс закрывается доказательством на
    #     ОДНОМ месте, а три подключения разом дают владельцу поток карточек вместо сигнала.
    # Возврат ДО котировки снимает маркер [QUOTE], поэтому КОД не вставит строку J, а белый список
    # пост-чека пуст (цифр в записке нет ни одной) — любое число ответа будет заклеймлено.
    # Молчания при этом не наступает: записка ТРЕБУЕТ ответить и позвать человека, среднее по
    # периоду запрещено ею отдельной строкой.
    _season_note = season_gate.note_for(hints, lang=lang)
    if _season_note is not None:
        return _season_note + dep
    ...
    if len(products) == 1:
        ...
        kind, phrase, q = _resolve_model_price(m, ds, de, hint_days, monthly, getter=getter,
                                               name_filter=nf)
        # МОДЕЛЬ БЕЗ ЦЕНЫ — ТРЕТИЙ ИСХОД «не считаю, зову человека» (правило владельца
        # 19.08.2026, второй из трёх классов; первый — граница сезонов, 06.09). Стои́т РОВНО
        # ЗДЕСЬ и ровно в одном месте:
        #   · ПОСЛЕ котировки — раньше вердикта нет: «есть ли число у этой модели» знает только
        #     ответ файла на ИМЯ ПРОКОТИРОВАННОГО ЮНИТА (обрывок речи клиента «NMAX» в файле не
        #     стои́т буквально, и судить по нему значило бы звать человека на живой цене);
        #   · ДО подбора по классу и до сборки записки — иначе клиенту уехали бы цены СОСЕДНИХ
        #     моделей вместо той, о которой он спросил;
        #   · НЕ в ветке «срок короче минимального», НЕ в многомодельной ветке и НЕ в прайс-
        #     сетке парка: класс закрывается доказательством на ОДНОМ месте, а три подключения
        #     разом дают владельцу поток карточек вместо одного честного сигнала.
        # Возврат ДО сборки снимает маркер [QUOTE] — КОД не вставит строку J, а белый список
        # пост-чека пуст (цифр в записке нет ни одной): любое число ответа будет заклеймлено.
        # Молчания при этом не наступает: записка ТРЕБУЕТ ответить и позвать человека.
        _noprice_note = noprice_gate.note_for(kind, hints, model=m, lang=lang)
        if _noprice_note is not None:
            return _noprice_note + dep
        if passport_dep:
        ...
        # ПОДБОР ПО КЛАССУ (правило 20.08 + слово владельца 12.09): спрошенная модель идёт ОСНОВНЫМ
        # вариантом, следом — НЕ БОЛЬШЕ ДВУХ свободных на эти даты моделей ТОГО ЖЕ класса,
        # ближайших к ней по цене. Занята спрошенная — это больше не отказ, а замена тем же классом.
        # Отдельной карты замен НЕТ и не заводится: её роль исполняет класс. Ветка возвращает
        # готовую note ЦЕЛИКОМ либо None (прежний путь).
...
# --- ВЫРЕЗКА ЛЕСТНИЦЫ ПРОЦЕНТОВ СКИДКИ ЗА СРОК НА ПУТИ ПОДАЧИ FAQ (задание 62-e, 15.09.2026) ---
#
# ПРАВИЛО ОДНОЙ ФРАЗОЙ: лестница — это фрагмент подаваемого FAQ (предложение либо заголовок
# вместе со своим списком), где рядом со словом о скидке названы ДВА И БОЛЕЕ процента и ДВА И
# БОЛЕЕ РАЗНЫХ срока аренды; такой фрагмент целиком заменяется отсылкой к блоку ЦЕНА.
#
# ЗАЧЕМ: критичные факты ЭТОГО ЖЕ промпта (CRITICAL_FACTS) запрещают называть клиенту хоть один
# процент скидки и хоть одну ступень скидки за срок, а блок FAQ ниже подавал готовую лестницу до
# 50% — спор стоял внутри одного промпта. Док мозга `faq` и локальный фолбэк
# `manager-bot/docs/turbobaby_faq_v1.md` НЕ правятся (их редакция — слово владельца): лестница
# снимается ровно на подаче, то есть здесь.
#
# ПОЧЕМУ ПО СМЫСЛУ, А НЕ ПО СТРОКЕ: вырезка по точной строке «неделя ~6–15%, 2 недели ~15–25%,
# месяц ~35–50%» переживёт ровно до первой редакции дока — переписанная другими числами, другим
# дефисом, по-английски или пунктами списка лестница обошла бы её МОЛЧА. Правило выше не смотрит
# ни на числа, ни на разметку.
_RE_LADDER_DISCOUNT = re.compile("скидк|уступк|discount", re.I)
_RE_LADDER_PCT = re.compile(r"\d+(?:[.,]\d+)?\s*%")
# СРОК АРЕНДЫ — единица времени, при желании с числом или числительным перед ней. Число берётся
# ВНУТРЬ совпадения сознательно: «7 дней» и «30 дней» — РАЗНЫЕ сроки, хотя единица у них одна, и
# лестница, записанная сутками («7 дней 6%, 30 дней 35%»), обязана ловиться наравне со словесной.
_RE_LADDER_TERM = re.compile(
    r"(?:\b\d+\s*|\b(?:одн|дв[ауе]|тр[ёеи]|четыр|пят|шест|сем|восем|девят|десят|пол)\w*\s+"
    r"|\b(?:one|two|three|four|five|six|seven|ten|half)\s+)?"
    r"\b(?:недел\w*|week\w*|месяц\w*|мес\.|month\w*|дн[еиияйёо]\w*|день|сут\w*|days?\b"
    r"|год\w*|лет\b|years?\b)", re.I)
_RE_LADDER_ITEM = re.compile(r"^\s*(?:[-*•‣–—]|\d+[.)])\s+")
_RE_SENTENCE_SPLIT = re.compile(r"(?<=[.!?])(?=\s)")

# Текст, который встаёт НА МЕСТО вырезанного фрагмента. Сознательно НЕ содержит ни слова
# «ступень» (иначе счётчик запрета в критфактах перестанет отличаться от лестницы), ни единого
# числа с процентом (иначе вырезка сама стала бы тем, что вырезает).
LADDER_CUT_MARK = ("(лестница процентов скидки за срок убрана на подаче — проценты клиенту не "
                   "называем; скидку и итог даёт ТОЛЬКО блок ЦЕНА по конкретным датам)")


def _ladder_terms(fragment: str):
    """Множество РАЗНЫХ сроков аренды, названных во фрагменте (нормализованных по регистру и
    пробелам). «неделя» и «2 недели» — разные; «7 дней» и «30 дней» — тоже разные."""
    return set(re.sub(r"\s+", " ", m.group(0)).strip().lower()
               for m in _RE_LADDER_TERM.finditer(fragment))


def _is_ladder_fragment(fragment: str) -> bool:
    """Тот самый критерий одной фразой: слово о скидке И два-плюс процента И два-плюс РАЗНЫХ
    срока аренды в одном фрагменте.

    Привязку «какой процент к какому сроку» мы СОЗНАТЕЛЬНО не вычисляем: она решается расстоянием
    и на живых переписях ошибается (в «За месяц уступка 40%, за неделю 8%» ближайшим к 40% стоит
    «неделю», а не «месяц»), из-за чего лестница проходила бы мимо реза. Перечисление двух-плюс
    процентов при двух-плюс разных сроках и слове о скидке И ЕСТЬ лестница — как бы её ни
    записали.

    Один процент («скидка за срок 6%» в примере расчёта, «скидка категории 25%») лестницей НЕ
    является и остаётся на месте — это отдельный предмет, не этой вырезки."""
    if not _RE_LADDER_DISCOUNT.search(fragment):
        return False
    if len(_RE_LADDER_PCT.findall(fragment)) < 2:
        return False
    return len(_ladder_terms(fragment)) >= 2


def _ladder_fragments(text: str):
    """Текст FAQ → список фрагментов (кусков исходного текста в порядке следования). Фрагмент —
    предложение; но строка, кончающаяся двоеточием и говорящая о скидке, склеивается со своим
    списком в ОДИН фрагмент: лестницу можно записать и пунктами, и тогда в каждом пункте будет
    ровно один процент."""
    out = []
    lines = text.split("\n")
    i = 0
    while i < len(lines):
        line = lines[i]
        if line.rstrip().endswith(":") and _RE_LADDER_DISCOUNT.search(line):
            j = i + 1
            while j < len(lines) and lines[j].strip() and _RE_LADDER_ITEM.match(lines[j]):
                j += 1
            if j > i + 1:
                out.append("\n".join(lines[i:j]))
                i = j
                continue
        for piece in _RE_SENTENCE_SPLIT.split(line):
            if piece:
                out.append(piece)
        if i < len(lines) - 1:
            out.append("\n")
...
def strip_term_discount_ladder(faq: str):
    """Убрать лестницу процентов скидки за срок из текста FAQ ПЕРЕД подачей в промпт.
    → (текст без лестницы, сколько фрагментов убрано). Боевых файлов и дока мозга не касается."""
    if not faq:
        return faq, 0
    cut = 0
    out = []
    for frag in _ladder_fragments(faq):
        if frag != "\n" and _is_ladder_fragment(frag):
            lead = frag[:len(frag) - len(frag.lstrip())]
            out.append(lead + LADDER_CUT_MARK)
            cut += 1
        else:
            out.append(frag)
    return "".join(out), cut
...
# --- ВЕЛИЧИНА ПРОЦЕНТА СКИДКИ ЗА СРОК В ПРИМЕРАХ (задание 62-i, 15.09.2026) ------------------
#
# РАСШИРЕНИЕ ПРАВИЛА ВЫШЕ, НАЗВАННОЕ ЯВНО. Вырезка 62-e снимала ЛЕСТНИЦУ — перечисление
# «неделя столько, месяц столько». Одиночный процент она пропускала сознательно, и после неё в
# собранном промпте осталось ДЕВЯТЬ величин (замер 15.09: 7 в примерах формата, 2 в подаваемом
# FAQ). Все девять стоя́т внутри ПРИМЕРОВ: семь — в парах «клиент → наш ответ», два — в строках
# FAQ под словами «Формат ответа, который ты используешь». Голова читает их наравне с правилами.
#
# ПОЧЕМУ ДВА РАЗНЫХ ДЕЙСТВИЯ, А НЕ ОДНО. Лестница — это ОБЕЩАНИЕ клиенту, и она вырезается
# фрагментом целиком: формы, которой стоило бы учить, в ней нет. Одиночный процент в примере
# расчёта — наоборот, ФОРМА строки цены, которую критфакты требуют держать дословно («НЕ опускай
# часть про скидку за срок»). Вырезать пример целиком значило бы отнять у головы образец формата;
# поэтому у него снимается ВЕЛИЧИНА, а место остаётся: «(Скидка за срок N%, 313 в день)». Пример
# учит форме, а не величине — ровно тем же словом `N%`, которым зовёт это место шапка блока ЦЕН
# ПО МОДЕЛЯМ («(Скидка за срок N%, … в день)»), так что пример и указание сошлись дословно.
#
# ЧЕГО МАСКА НЕ КАСАЕТСЯ: блока ЦЕНА/ПРАЙС. Там процент приходит строкой J Календаря по
# КОНКРЕТНЫМ датам — это единственный живой источник скидки, он едет клиенту посимвольно и заперт
# отдельным тестом. Маска стои́т только на подаче FAQ и на подаче примеров, а ценовую записку
# вызывающий кладёт мимо неё.
TERM_DISCOUNT_PCT_SLOT = "N%"
# ОКНО И СЛОВАРЬ — ДОСЛОВНО ТЕ ЖЕ, ЧТО У СЧЁТЧИКА ЗАМЕРА 13.09 (±60 симв. вокруг «N%», слово о
# скидке И слово о сроке). Маска НЕ опирается на богатый словарь сроков вырезки лестницы выше, и
# это решение замером, а не вкусом: `_RE_LADDER_TERM` знает слово «день», и на нём маска съедала
# «**Скутеры (скидка категории 25%):**» — строку ПРАЙСА, где скидка КАТЕГОРИИ, а не за срок
# (замер: на живом доке «25%» и «15%» пропадали оба). Прайса эта полоса не правит вовсе.
#
# Разница словарей не случайна и держится СМЫСЛОМ: «дней/дня/дню» — срок аренды («7 дней»), а
# «฿/день» — ставка за сутки, то есть цена, а не срок. Счётчик ловит первое (`\bдн\w*` на «день»
# не совпадает: букв «дн» в нём подряд нет) и пропускает второе.
#
# ПОЧЕМУ ОДНО ОПРЕДЕЛЕНИЕ НА ПРАВИЛО И НА ЗАМЕР. Величину, которую надо снять, задание называет
# счётчиком («тем же счётчиком, каким мерили 13.09 и 15.09»); правило, говорящее другим словарём,
# либо не добрало бы, либо срезало лишнее — оба случая мы и увидели на замере. Возражение «ноль
# мерится собственным ситом» снимается не расхождением словарей, а ОТРИЦАТЕЛЬНЫМ ТЕСТОМ: счётчик
# заведён 13.09, до всякой правки, и подложенный процент он видит в каждом из трёх мест.
# ЧЕГО ПРАВИЛО НЕ ЗНАЕТ (названо честно): процент, у которого слово о сроке дальше 60 симв.,
# не увидят ни счётчик, ни маска — общая слепота, а не расхождение.
_TERM_PCT_WINDOW = 60
_RE_TERM_PCT_TERM = re.compile("срок|term|недел|week|месяц|month|\\bдн\\w*|\\bday", re.I)


def mask_term_discount_percent(text: str):
    """Снять ВЕЛИЧИНУ у каждого процента скидки за срок, оставив место `N%`.
    → (текст, сколько величин снято). Строение и тон строки не трогаются ничем, кроме числа."""
    if not text:
        return text, 0
    spans = []
    for m in _RE_LADDER_PCT.finditer(text):
        lo = max(0, m.start() - _TERM_PCT_WINDOW)
        hi = min(len(text), m.end() + _TERM_PCT_WINDOW)
        win = text[lo:hi]
        if _RE_LADDER_DISCOUNT.search(win) and _RE_TERM_PCT_TERM.search(win):
    ...
    for a, b in spans:
        ...
        out.append(TERM_DISCOUNT_PCT_SLOT)
        prev = b
    out.append(text[prev:])
    return "".join(out), len(spans)


def _faq_for_prompt(faq: str) -> str:
    """Подача FAQ в промпт. ВЫРЕЗКА ОБЪЯВЛЯЕТ СЕБЯ: молчаливое вырезание запрещено — каждый
    сработавший рез оставляет в журнале строку с ЧИСЛОМ убранных фрагментов и своей причиной."""
    text, cut = strip_term_discount_ladder(faq)
    if cut:
        log.info(
            "SUGGEST: FAQ на подаче в промпт — вырезано %d фрагмент(ов) лестницы процентов скидки "
            "за срок; причина: критичные факты ЭТОГО ЖЕ промпта запрещают называть клиенту хоть "
            "один процент и хоть одну ступень скидки за срок, а док FAQ правке с полосы ПК не "
            "подлежит (задание 62-e, 15.09.2026)", cut)
    text, masked = mask_term_discount_percent(text)
    if masked:
        log.info(
            "SUGGEST: FAQ на подаче в промпт — снято %d величин(ы) процента скидки за срок "
            "(остались места «%s»); причина: проценты стоя́т в примерах ФОРМАТА ответа, а формату "
            "величина не нужна — скидку по конкретным датам даёт ТОЛЬКО блок ЦЕНА "
            "(задание 62-i, 15.09.2026)", masked, TERM_DISCOUNT_PCT_SLOT)
    return text


def _examples_for_prompt(block: str) -> str:
    """Подача блока ПРИМЕРОВ в промпт — и ручных пар, и подобранных под вопрос живых.

    Ручные пары ниже уже переписаны местом `N%` в самом коде, поэтому на них маска обычно даёт
    ноль. Стои́т она ради ДРУГОГО пути: подобранные живые примеры берутся из обезличенной базы
    переписок, править которую полоса ПК не смеет, а проценты скидки за срок в живой переписке
    есть (замер 15.09 видел до семи в одной сборке)."""
    text, masked = mask_term_discount_percent(block)
    if masked:
        log.info(
            "SUGGEST: примеры ответов на подаче в промпт — снято %d величин(ы) процента скидки за "
            "срок (остались места «%s»); причина: пример учит ФОРМЕ, а не величине "
            "(задание 62-i, 15.09.2026)", masked, TERM_DISCOUNT_PCT_SLOT)
    return text
...
def make_system_prompt(faq: str, lang: str, is_first_contact: bool = False, pricing_note: str = "",
    ...
    return (
        ...
        + playbook_block
        + "\n\nFAQ и эталонные формулировки:\n" + (_faq_for_prompt(faq) or "(FAQ недоступен — опирайся на критичные факты выше)")
        + _examples_for_prompt(_style_block(client_question))
    )
...
# ОБЕЩАНИЕ «УТОЧНЮ И ВЕРНУСЬ» — ОДНА ПАРА КЛИЕНТСКИХ СТРОК НА ВСЮ СБОРКУ (названа 12.09.2026).
# Пара НЕ НОВАЯ и НЕ ПРАВЛЕНАЯ: она жила инлайном внутри `postcheck_draft` (строка `ask`) и в этом
# виде уходила клиенту. Здесь она только НАЗВАНА ПО ИМЕНИ, буква в букву — клиентские формулировки
# заморожены. Имя понадобилось потому, что говорить это обещание обязаны ДВА места сборки, и
# говорить РОВНО ОДНО И ТО ЖЕ: пост-чек (переписывает сегмент-нарушитель, когда голова назвала
# число вне белого списка) и гарантия `ensure_price_promise` (дописывает обещание, когда его нет
# вовсе). Второй похожий текст здесь был бы новой клиентской формулировкой, а её заводить нельзя.
PRICE_PROMISE_LINES = {
    "ru": "Уточню у команды и вернусь.",
    "en": "Let me check with the team and get back to you.",
}


def price_promise_line(lang: str = "ru") -> str:
    """ДОСЛОВНАЯ клиентская строка обещания уточнить цену и вернуться. ЕДИНСТВЕННЫЙ источник для
    обоих мест сборки. Не 'en' → RU: это прежнее поведение пост-чека буква в букву (там язык
    проверялся равенством `lang == "en"`), то есть fail-safe по языку, а не догадка."""
    return PRICE_PROMISE_LINES["en" if lang == "en" else "ru"]
...
def postcheck_draft(draft: str, lang: str = "ru", pricing_note: str = "",
    ...
    en = (lang == "en")
    ask = price_promise_line("en" if en else "ru")   # та же пара строк, что у гарантии обещания
...
# ────── ОБЕЩАНИЕ «УТОЧНЮ И ВЕРНУСЬ» — ГАРАНТИЯ КОДА, А НЕ УСЕРДИЕ ГОЛОВЫ (12.09.2026) ─────────
# ЖИВОЙ СЛУЧАЙ, которым правка рождена (`docs/artifacts/2026-09-11-ЗОНД-pyat-sborok-1109.md`, §6
# «цена ошибки наружу»): в тёмном круге кейса 8 11.09 записка ушла в ветку «точная цена из
# Календаря сейчас недоступна», и наружу поехал ответ БЕЗ цены. Обещание вернуться в том круге
# прозвучало — но прозвучало СЛОВАМИ ГОЛОВЫ: записка её ПРОСИТ («ответь, что уточнишь цену и
# вернёшься»), а ни одна строка конвейера этого не дописывала. `ensure_price_figure` выше на такой
# записке возвращает черновик байт-в-байт (посчитанных цифр нет — вставлять нечего), а `ensure_
# closing_question` дописывает вопрос воронки, а не обещание. Цена просьбы к голове мерена на
# соседнем классе — ≈14 % красных кругов на побайтно одном промпте (кейс 10, замер 06.09): то есть
# примерно каждый седьмой клиент услышал бы вместо обещания тишину о цене и ничего взамен.
#
# АСИММЕТРИЯ, КОТОРУЮ ПРАВКА СНИМАЕТ, названа функциями самого файла: резак обещания есть
# (`drop_price_deflection` выше — вырезает «уточню и вернусь», когда цифра ПОСЧИТАНА и доехала до
# клиента), дописывателя для обратного случая не было ни одного. Теперь их ровно два, и спорить им
# НЕЧЕМ: у обоих ОДИН вентиль — `computed_price_figures(pricing_note)`, — только читают они его в
# противоположных знаках (резак работает при НЕПУСТОМ множестве цифр, дописыватель при ПУСТОМ).
# Ни на одной записке не срабатывают оба, поэтому ПОРЯДОК их вызова исхода не меняет; это не
# рассуждение, а прогон обоих порядков на всех фикстурах (`test_rezak_i_dopisyvatel_ne_sporjat`).
#
# РЕШЕНИЕ ПРИНИМАЕТ ЗАПИСКА, А НЕ ТЕКСТ, КОТОРЫЙ СИСТЕМА НАПЕЧАТАЛА САМА. «Нужно ли обещание» —
# вопрос к ценовой записке (что случилось с ценой), и все условия читаются из неё. Текст черновика
# читается РОВНО ДЛЯ ОДНОГО — «обещание уже есть, второго не дописывать». Признака вида «в ответе
# написано слово, значит событие произошло» здесь нет ни одного: написанное слово может закрыть
# дописку, но не может объявить цену посчитанной.
#
# Записка САМА просит обещания: тёмные ценовые ветки печатают просьбу «ответь, что уточнишь цену …
# и вернёшься» (`_resolve_model_price`: min/sanity/none/noprice/error, плюс сторожа сезона и
# модели-без-цены). Два носителя ищем ПОРОЗНЬ, а не одной фразой: формы веток разные («уточню и
# вернусь», «уточнишь цену и вернёшься», «уточню наличие и цену на эти даты и вернусь») и
# расстояние между словами в них не постоянно.
_PROMISE_NOTE_ASK_RE = re.compile(r"уточн\w+", re.I)
_PROMISE_NOTE_BACK_RE = re.compile(r"верн[уёе]\w*", re.I)
# ОБЕЩАНИЕ УЖЕ ЗВУЧИТ — детектор ШИРЕ резакова `_DEFLECT_RE`, и это не небрежность, а РАЗНАЯ ЦЕНА
# ОШИБКИ у двух ролей одного слова. Резак РЕЖЕТ чужой текст: его промах уносит наружу смысл,
# поэтому он узкий. Дописыватель на промахе лишь МОЛЧИТ (возврат байт-в-байт = прежнее поведение),
# а вот пропущенное обещание дало бы ДУБЛЬ в живом ответе. Порог мерен ЖИВОЙ фразой кейса 8 — «let
# me check the exact price for those dates and get right back to you»: узкий `_DEFLECT_RE` её НЕ
# ловит (между «get» и «back» стои́т «right»), и на ней дописыватель поставил бы второе обещание.
# Язык детектор НЕ различает СОЗНАТЕЛЬНО: голова на RU-треде отвечает англоязычному клиенту
# по-английски, и детектор, зашторенный языком карточки, увидел бы пустое тело и дал дубль.
_PROMISE_SAID_RE = re.compile(
    r"верн[уё]сь|верн[её]мся|отпишус[ья]|сообщу\s+позже|напишу\s+(?:вам\s+)?позже"
    r"|уточн[юяё]\w*\s+(?:у\s+\w+|цен|стоимост|наличи|тариф)"
    r"|get\s+(?:\w+\s+){0,2}back\s+to\s+you|come\s+(?:\w+\s+){0,2}back\s+to\s+you"
    r"|let\s+you\s+know(?:\s+\w+){0,2}\s+later"
    r"|check\s+(?:\w+\s+){0,3}(?:with\s+the\s+team|price|availability|rate)", re.I)


def price_promise_due(pricing_note: str = "") -> bool:
    """Нужно ли ЭТОМУ ответу обещание «уточню и вернусь» → судим по ЦЕНОВОЙ ЗАПИСКЕ, и только по
    ней. ТРИ условия, все из записки:
      1) записка непуста — без ценового круга обещать нечего (fail-safe);
      2) цифр, ПОСЧИТАННЫХ источником, в ней НЕТ (`computed_price_figures` пуст) и метки «цена
         посчитана» (`_PRICE_OK_MARKS`) в ней нет — ровно ОБРАТНОЕ условие резака;
      3) записка САМА просит обещания (тёмная ветка печатает «уточнишь … и вернёшься»).
    Третье условие и есть замок от лишнего обещания: на записке «дат аренды в диалоге НЕТ» ответ
    просит даты У КЛИЕНТА, на записке «клиент назвал серию» — модель; обещать там «вернусь» не к
    месту, просьбы обещать в этих ветках нет, и гарантия молчит.
    ЧЕГО ЭТО УСЛОВИЕ НЕ ЛОВИТ, названо честно: посчитанная ДОСТАВКА даёт непустое множество цифр
    при тёмной АРЕНДЕ — тогда обещание не дописывается. Тот же шов у резака (он на таком наборе
    обещание вырежет), класс унаследован, а не заведён здесь."""
    note = pricing_note or ""
    if not note.strip():
        return False
    if computed_price_figures(note) or any(mark in note for mark in _PRICE_OK_MARKS):
        return False
    return bool(_PROMISE_NOTE_ASK_RE.search(note) and _PROMISE_NOTE_BACK_RE.search(note))


def price_promise_said(body: str) -> bool:
    """Обещание ЗВУЧИТ клиенту → True. Чьими словами — головы или кода — здесь не различается: это
    ПОСТУСЛОВИЕ гарантии, и мерить им надо готовый текст, а не авторство (то же правило, что у
    `min_term_said`)."""
    return bool(_PROMISE_SAID_RE.search(body or ""))


def ensure_price_promise(draft: str, pricing_note: str = "", lang: str = "ru") -> str:
    """ОБЕЩАНИЕ ОБЯЗАНО ЗВУЧАТЬ КЛИЕНТУ, КОГДА ЦЕНЫ НЕТ: записка ушла в тёмную ветку и сама просит
    «уточнишь цену и вернёшься», а в клиентском теле обещания нет — дописываем ДОСЛОВНУЮ строку
    страховки (`price_promise_line`) хвостом, КОДОМ. Обещание не требуется / уже звучит / пустой
    вход → БАЙТ-В-БАЙТ (fail-safe). Форма взята у `ensure_min_term` дословно: чужой текст не
    правим, служебная пометка модератору остаётся хвостом.

    ЗОВЁТСЯ ДВАЖДЫ ЗА ПРОХОД, как и `ensure_min_term`, и по той же причине. Функция ИДЕМПОТЕНТНА
    (вставленную строку она видит как «обещание в теле» — в RU-строке стои́т «вернусь», в
    EN-строке «get back to you»), поэтому:
      * РАННИЙ вызов ставит обещание в тело ДО закрывающего вопроса, и оно читается частью ответа,
        а не припиской после «Бронируем?»;
      * ПОЗДНИЙ — ПОЯСОМ, последним из трогающих клиентский текст, и он и есть гарантия: между
        ранним вызовом и концом сборки стоят `drop_answered_questions` и `ensure_closing_question`,
        а живой круг 06.09 показал на минимальном сроке, как они вдвоём съедают правильный абзац
        головы и подставляют вместо него «Бронируем?».

    ЧЕСТНАЯ ЦЕНА ПОРЯДКА (та же, что у пояса минимального срока): гард наличия стои́т ПОСЛЕ и в
    своём безопасном фолбэке заменяет черновик целиком — тогда строка уйдёт вместе с ним.
    Последнее слово намеренно оставлено правилу безопасности.

    ОТКАТ одной ручкой: `PRICE_PROMISE_GUARANTEE_OFF=1` в окружении — ветка мертва целиком, и
    система снова становится той, где обещание держится усердием головы."""
    text = draft or ""
    if not text.strip() or os.getenv("PRICE_PROMISE_GUARANTEE_OFF", "") == "1":
        return draft
    if not price_promise_due(pricing_note):
        return draft
    m = _SERVICE_TAIL_RE.search(text)
    body, tail = (text[:m.start()], text[m.start():]) if m else (text, "")
    if price_promise_said(client_facing_text(body)):
        return draft
    line = price_promise_line(lang)
    log.warning("ensure_price_promise: цена не посчитана, а обещания вернуться в теле не было — "
                "строка «%s» добавлена КОДОМ (гарантия продукта, не усердие головы)", line)
    return body.rstrip() + "\n\n" + line + (("\n" + tail.strip()) if tail.strip() else "")


# ─────────────── МИНИМАЛЬНЫЙ СРОК — ГАРАНТИЯ КОДА, А НЕ НАСТРОЕНИЕ ГОЛОВЫ (06.09.2026) ────────
# ПИСАНОЕ ПРАВИЛО, дословно (живое чтение узла `KB_knowledge_base`, раздел «Правила цен v2»,
# п.5, sha256 21a9570a…2637c2, 06.09.2026):
...
# Цифры 5/3 живут в SCOOTER_MIN_DAYS/MOTO_MIN_DAYS и здесь НЕ дублируются: строку собираем из
# ЗАПИСКИ, которую напечатал `_resolve_model_price` веткой `min` — тот же источник, что видит LLM.
#
# ЗАЧЕМ КОД, А НЕ ПРОМПТ. Правило в записку попадает всегда, но до клиента его доносила ТОЛЬКО
# голова, и делала это НЕ ВСЕГДА: замер 06.09 (10 повторов кейса 10 на побайтно одном промпте,
# docs/artifacts/2026-09-06-кейс-десять-расхождение-кругов-06.09.md) — 9 зелёных и 1 красный,
# ≈14% на круг. Красный ответ звучал «Учли — NMAX 155 на 6–7 октября. Бронируем?»: он СОГЛАСИЛСЯ
# сдать байк на срок, которого контора не сдаёт. Это жёсткий предел бизнеса, а не стилистика,
# поэтому он держится кодом. Форма взята у `ensure_price_figure` дословно: fail-safe байт-в-байт,
# дописка хвостом перед служебной пометкой, никакой правки чужого текста.
_MIN_TERM_NOTE_RE = re.compile(r"([^\s:;–—-][^:;\n]{0,24}?)\s+сдаём от (\d+) дней "
                               r"\(короче срок не оформляем\)")
# Голова УЖЕ сказала правило сама → не дописываем ничего (иначе дубль в каждом здоровом ответе).
# Признак согласия/отказа нужен ВТОРЫМ условием только для формы «N дней» без «от»: сама «от N дней»
# и есть заявленный минимум, доказывать её отказом нечем.
_MIN_TERM_REFUSAL_RE = re.compile(r"не\s+оформля|не\s+сда[её]м|не\s+сдаем|минимальн|минимум|"
                                  r"к\s+сожалению|короче", re.I)
_MIN_TERM_REFUSAL_EN_RE = re.compile(r"minimum|min\.|do(?:es)?\s+not\s+rent|don't\s+rent|"
                                     r"unfortunately|shorter", re.I)


def min_term_pairs_from_note(pricing_note: str):
    """Пары (как назвать тип, минимум в днях) из ветки `min` записки цены → список без дублей.
    Ветка не срабатывала (срок клиента не короче минимума) → пустой список."""
    seen, out = set(), []
    for label, days in _MIN_TERM_NOTE_RE.findall(pricing_note or ""):
        try:
            n = int(days)
        except ValueError:                     # pragma: no cover — \d+ числом быть обязан
            continue
        key = (label.strip().lower(), n)
        if key in seen:
            continue
        seen.add(key)
        out.append((label.strip(), n))
    return out


def min_term_line_from_note(pricing_note: str, lang: str = "ru"):
    """ДОСЛОВНАЯ клиентская строка минимального срока, которую вставляет КОД, | None (ветка `min`
    не срабатывала). ТРЕТИЙ ИСХОД в одной строке: срок короче минимума НЕ сдаётся (прямой отказ)
    и минимум НАЗВАН числом. Молчания и выдуманного согласия эта строка не допускает одинаково."""
    pairs = min_term_pairs_from_note(pricing_note)
    if not pairs:
        return None
    if lang == "en":
        parts = ", ".join(f"{n} days" for _, n in pairs)
        return (f"We do not rent for a shorter term — minimum rental: {parts}.")
    parts = ", ".join(f"{label} сдаём от {n} дней" for label, n in pairs)
    return f"На более короткий срок не оформляем: {parts}."


def min_term_said(body: str, pairs, lang: str = "ru") -> bool:
    """Правило ЗВУЧИТ клиенту → True. Чьими словами — головы или кода — здесь не различается:
    это ПОСТУСЛОВИЕ гарантии, и мерить им надо готовый текст, а не авторство.
    Засчитываем только ПОЛНЫЙ третий исход: минимум назван числом И (для формы без «от»)
    сказано, что короче не сдаём. Одна из пар не прозвучала → False: неполный ответ = молчание."""
    low = (body or "").lower()
    if lang == "en":
        refused = bool(_MIN_TERM_REFUSAL_EN_RE.search(low))
        for _, n in pairs:
            if re.search(r"from\s+%d(?!\d)\s*(?:day|d\b)" % n, low):
                continue
            if refused and re.search(r"(?<!\d)%d(?!\d)\s*(?:day|d\b)" % n, low):
                continue
            return False
        return True
    refused = bool(_MIN_TERM_REFUSAL_RE.search(low))
    for _, n in pairs:
        if re.search(r"от\s+%d(?!\d)\s*(?:дн|сут)" % n, low):
            continue
        if refused and re.search(r"(?<!\d)%d(?!\d)\s*(?:дн|сут)" % n, low):
            continue
        return False
    return True


def ensure_min_term(draft: str, pricing_note: str = "", lang: str = "ru") -> str:
    """МИНИМАЛЬНЫЙ СРОК ОБЯЗАН ЗВУЧАТЬ КЛИЕНТУ: расчёт ушёл в ветку «короче минимума», а в
    клиентском теле правила нет — дописываем строку отказа с числом ХВОСТОМ, КОДОМ.
    Ветка не срабатывала / правило уже в теле / пустой вход → БАЙТ-В-БАЙТ (fail-safe).

    ЗОВЁТСЯ ДВАЖДЫ ЗА ПРОХОД, и это не описка. Функция ИДЕМПОТЕНТНА (уже вставленную строку она
    видит как «правило в теле» и возвращает вход байт-в-байт), поэтому:
      * РАННИЙ вызов — рядом с `ensure_price_figure`: строка встаёт в тело ДО того, как ответ
        оформят закрывающим вопросом, и читается частью ответа, а не припиской после «Бронируем?»;
      * ПОЗДНИЙ вызов — ПОЯСОМ, последним из тех, кто трогает клиентский текст. Он и есть
        гарантия. Живой замер 06.09 (круг 2 кейса 10): голова СКАЗАЛА правило верно —
        «Скутеры мы сдаём от 5 дней, на сутки, к сожалению, не оформим — рассмотрите вариант от
        5 дней?», — а `drop_answered_questions` вырезал этот абзац целиком как переспрос о уже
        собранном сроке, и `ensure_closing_question` заменил его на «Бронируем?». Клиенту уехало
        «Учли — NMAX 155 на 6–7 октября. Бронируем?» — согласие на срок, которого мы не сдаём,
        собранное КОДОМ из правильного ответа головы. Ровно этот текст артефакт 06.09 считал
        молчанием головы; молчания там не было.

    ЧЕСТНАЯ ЦЕНА ПОРЯДКА (та же, что у пояса обязательного упоминания 23.08): гард наличия стои́т
    ПОСЛЕ и в своём безопасном фолбэке заменяет черновик целиком — тогда строка уйдёт вместе с
    ним. Последнее слово намеренно оставлено правилу безопасности.

    ОТКАТ одной ручкой: `MIN_TERM_GUARANTEE_OFF=1` в окружении — ветка мертва целиком, поведение
    возвращается к прежнему (нужен КОНТРФАКТУ: снятая гарантия обязана вернуть прежний ответ)."""
    text = draft or ""
    if not text.strip() or os.getenv("MIN_TERM_GUARANTEE_OFF", "") == "1":
        return draft
    pairs = min_term_pairs_from_note(pricing_note)
    if not pairs:
        return draft
    m = _SERVICE_TAIL_RE.search(text)
    body, tail = (text[:m.start()], text[m.start():]) if m else (text, "")
    if min_term_said(client_facing_text(body), pairs, lang):
        return draft
    line = min_term_line_from_note(pricing_note, lang)
    if not (line or "").strip():             # pragma: no cover — pairs непусты ⇒ строка есть
        return draft
    log.warning("ensure_min_term: срок короче минимального, а правила в теле не было — строка "
                "«%s» добавлена КОДОМ (гарантия продукта, не настроение головы)", line)
    return body.rstrip() + "\n\n" + line.strip() + (("\n" + tail.strip()) if tail.strip() else "")
...
def generate_draft(transcript: str, lang: str, faq: str,
    ...
    # МИНИМАЛЬНЫЙ СРОК — тем же классом, что цифра цены: «не сдаём короче N дней» доносит КОД, а
    # не настроение головы. Место выбрано ЖИВЫМ ДЕФЕКТОМ, а не вкусом: РАНЬШЕ `drop_answered_
    # questions` этот вызов бесполезен — резак съедает правило вместе с абзацем головы (замер
    # 06.09, круг 2 кейса 10). Здесь же оно встаёт ДО закрывающего вопроса и читается частью
    # ответа, а не припиской после «Бронируем?». Гарантией служит ПОЯС ниже, а не этот вызов.
    out = ensure_min_term(out, pricing_note, lang)
    # ОБЕЩАНИЕ «УТОЧНЮ И ВЕРНУСЬ» — тем же классом, что минимальный срок: когда цена НЕ посчитана,
    # обещание доносит КОД, а не усердие головы. Раннее место — то же и по той же причине: строка
    # встаёт в тело ДО закрывающего вопроса. Гарантией служит ПОЯС ниже, а не этот вызов.
    out = ensure_price_promise(out, pricing_note, lang)
    ...
    # ПОЯС МИНИМАЛЬНОГО СРОКА (06.09.2026) — ПОСЛЕДНИЙ из тех, кто трогает клиентский текст, и
    # именно поэтому он и есть гарантия. Ранний вызов выше её НЕ даёт: между ним и этим местом
    # стоят `drop_answered_questions` и `ensure_closing_question`, и живой круг 06.09 показал, как
    # они вдвоём съедают правильный отказ головы и подставляют вместо него «Бронируем?».
    # Функция идемпотентна: правило доехало — вход возвращается байт-в-байт.
    out = ensure_min_term(out, pricing_note, lang)
    # ПОЯС ОБЕЩАНИЯ (12.09.2026) — рядом с поясом минимального срока и по тем же двум причинам:
    # позже резака вопросов и позже сборки закрывающего вопроса, идемпотентен. Резаку обещания
    # (`drop_price_deflection`) он не противоречит НИ В КАКОМ ПОРЯДКЕ: у них общий вентиль
    # `computed_price_figures`, читаемый в противоположных знаках — цена посчитана → работает
    # только резак; не посчитана → только пояс. Оба на одной записке не срабатывают никогда.
    out = ensure_price_promise(out, pricing_note, lang)
...
def regenerate_draft(transcript: str, lang: str, faq: str, is_first_contact: bool,
    ...
    out = drop_price_deflection(out, pricing_note, lang)
    out = drop_answered_questions(out, facts, lang)
    # Тот же класс и то же МЕСТО, что в generate_draft: минимальный срок доносит КОД, и делает это
    # ПОСЛЕ резака вопросов — иначе правило уедет вместе с абзацем головы.
    out = ensure_min_term(out, pricing_note, lang)
    # Тот же класс и то же место, что в generate_draft: обещание при непосчитанной цене доносит КОД.
    out = ensure_price_promise(out, pricing_note, lang)
    out = ensure_closing_question(out, facts, lang, sheet_mode=sheet_mode, ready=ready, just=just)
    ...
    out = enforce_partner_delivery_guard(out, is_partner, lang)
    # ПОЯС МИНИМАЛЬНОГО СРОКА — тот же класс и то же место, что в generate_draft: последним из
    # трогающих клиентский текст (гарда наличия на этой дороге нет вовсе, значит пояс здесь и
    # закрывает ответ). Идемпотентен: правило доехало — байт-в-байт.
    out = ensure_min_term(out, pricing_note, lang)
    # ПОЯС ОБЕЩАНИЯ — тот же класс и то же место: гарда наличия на этой дороге нет, значит этот
    # пояс и закрывает ответ. Идемпотентен: обещание доехало — байт-в-байт.
    return ensure_price_promise(out, pricing_note, lang)
...
# ПРИЗНАК «ОДНА БРОНЬ ↔ ПЕРЕЧИСЛЕНИЕ ПАРКА» — СТРОКА-НОСИТЕЛЬ МОДЕЛИ как ПОЗИЦИЯ суммы
# (06.09.2026, ТРЕТИЙ случай класса: 30.07 в тренажёре, 31.07/08.08 у ревизора, 04.09 карточкой).
#
# ЗАЧЕМ. Инвариант чека — про ОДНУ БРОНЬ: у одной брони один байк, значит один депозит, и вторая
# сумма рядом с ним это самопротиворечие. В ПРАЙС-ЛИСТЕ депозит свой У КАЖДОЙ МОДЕЛИ (лестница
# парка 3000/5000/7000/15000/20000/25000 ฿ — CRITICAL_FACTS выше, 20 моделей), и шесть сумм там
# означают шесть РАЗНЫХ объектов, а не шесть мнений об одном. Прежняя ветка сравнивала суммы по
# ВСЕМУ тексту, ни к чему их не привязав, — и красила прайс, который сама же и печатает.
#
# ПРИЗНАК: позиция суммы — СТРОКА, называющая модель (своя, иначе ближайшая ВЫШЕ в пределах
# _SMOKE_DEP_POS_BACK). Суммы сравниваются ТОЛЬКО внутри одной позиции; сумм в разных позициях
# чек не сличает вовсе. Правило поиска «своя строка, иначе ближайшая выше» — то же, что у боевого
# _money_models_at (карточка прайса печатает модель ЗАГОЛОВКОМ блока, а сумму строкой ниже).
#
# ПОЧЕМУ КЛЮЧ — СТРОКА, А НЕ КАНОН-МОДЕЛЬ. Замер 06.09 на ДОСЛОВНОМ тексте карточки (13 блоков):
# ключ по канону разложил суммы в 12 корзин, но «XMAX 300» и «XMAX 300 New Gen» сошлись в ОДНУ
# корзину 'XMAX' (_detect_models канонизирует поколения в один ключ) — и корзина 'XMAX':
# [5000, 7000] осталась ЛОЖНОЙ находкой. Это не мелочь: XMAX двух поколений — две позиции парка с
# РАЗНЫМИ депозитами (CRITICAL_FACTS: 5000 у 2020-2022, 7000 у NEW 2023+), и живой текст id=292
# печатает их двумя строками. Ключ по строке поколения различает, канон — нет. На том же тексте
# ту же ложную пару даёт БОЕВОЙ money_contradictions → [('deposit','THB',[5000,7000])]: у него ключ
# как раз канон-модель (_money_models_at). Это ВТОРОЙ чек с той же слепотой; здесь его НЕ чиним.
#
# ГДЕ ПРИЗНАК ОШИБЁТСЯ (честно): если бот назвал модель ЗАНОВО на строке со второй суммой —
# «…депозит 3000 ฿.⏎ По NMAX депозит 5000 ฿» — строки разные, значит позиции разные, и настоящее
# противоречие уедет молча. Цена принята сознательно: за наблюдение 23.07–09.08 у чека 43 находки
# и НИ ОДНОЙ настоящей там, где существо установлено (docs/artifacts/2026-08-09-class92-43-
# unresolved.md §4). Не ошибётся там, где вторая сумма модель НЕ переназывает («Итого … депозит
# 5000 ฿») и где обе суммы на ОДНОЙ строке, — это и есть живые формы самопротиворечия.
# Ветку «деньги И паспорт» признак НЕ трогает: она судит по КЛАУЗЕ, то есть уже по одной позиции,
# и работает при любом числе моделей — половина чека остаётся в прежней силе дословно.
_SMOKE_DEP_POS_BACK = 300              # как далеко ВВЕРХ ищем строку-заголовок (как _MONEY_MODEL_BACK)


def _smoke_line_spans(s: str):
    """Границы строк текста [(начало, конец), …] — тот же разделитель, что у _money_line_at."""
    spans, left = [], 0
    for m in _MONEY_LINE_SPLIT_RE.finditer(s):
        spans.append((left, m.start()))
        left = m.end()
    spans.append((left, len(s)))
    return spans


def _smoke_dep_position_at(s: str, pos: int):
    """ПОЗИЦИЯ прайса, к которой относится сумма депозита в точке pos → смещение СТРОКИ, называющей
    модель (своей, иначе ближайшей ВЫШЕ в окне _SMOKE_DEP_POS_BACK); модели рядом нет → None.
    Смещение, а не имя модели: два поколения одной модели стоят разными строками и обязаны остаться
    РАЗНЫМИ позициями (канон _detect_models их склеивает — см. разбор признака выше)."""
    spans = _smoke_line_spans(s)
    here = next((k for k, (a, b) in enumerate(spans) if a <= pos <= b), len(spans) - 1)
    for k in range(here, -1, -1):
        a, b = spans[k]
        if k != here and b < pos - _SMOKE_DEP_POS_BACK:
            break                                     # выше окна поиска — заголовок не наш
        if _detect_models(s[a:b]):
            return a
    return None


def _smoke_deposit_conflict(text) -> str:
    """Противоречие депозита → строка-описание, иначе ''. Конфликт: ДВЕ разные суммы депозита
    В ОДНОЙ ПОЗИЦИИ ИЛИ сумма депозита и «паспорт» ТРЕБУЮТСЯ ОБА (KB: деньги ЛИБО паспорт, не оба).

    Суммы сравниваются только внутри ОДНОЙ позиции — строки, называющей модель (см. разбор признака
    над функцией). Перечисление парка сравнением сумм не судится: там своя сумма у каждой модели,
    и это прайс, а не самопротиворечие брони.
...
# ───────────── ОЖИДАНИЕ НЕ СОБРАЛОСЬ = ТРЕТИЙ ИСХОД, А НЕ ПРОВАЛ (11.09.2026) ─────────────
# КЛАСС, а не один чек. Чеки транспорта берут ожидание из ЦЕНОВОЙ ЗАПИСКИ — из той же, что питает
# черновик. Записка собирается не на всех ветках: промах моста, граница сезонов, длинная аренда,
# неразобранная модель — и тогда quote/delivery/sheet-блока в ней НЕТ ВОВСЕ. До 11.09 чек в этом
# случае отдавал ПРОВАЛ: искал строку, которой не существует, и красил RED. Это ложный красный —
# судить было НЕЧЕМ, кода мы не уличили ни в чём.
#
# ПРАВИЛО, ОДНО НА ВСЕ ТАКИЕ ЧЕКИ: ожидание чека не собралось → чек несёт `unknown` с дословной
# причиной. «Неизвестно» СИЛЬНЕЕ зелёного и СЛАБЕЕ провала: кейс не зачтён, ворот не открывает,
# в зачётную серию не идёт и её же не рвёт (`trainer_run.run_case` → уже существующая ветка
# `unknown`). Перевод такого исхода в ЗЕЛЁНОЕ — дефект хуже исходного и запрещён прямо.
#
# Поле `ok` при этом НЕ ТРОГАЕТСЯ: его читает e2e-смоук живого контура (`runLiveSmoke`), где
# несобранный quote-блок означает сломанный контур пробы, а не «нечем судить». Политику исхода
# объявляет ЧИТАТЕЛЬ, признак «basis отсутствует» объявляет ПРОИЗВОДИТЕЛЬ ожидания — здесь.
#
# Живой случай, которым правило заведено: кейс 3 предполёта 11.09, круг 1 — записка quote-блока не
# несла (`docs/artifacts/2026-09-11-ЗАПИСКА-kejs3-sborka-1109.md`), сборка и голова невиновны.
def no_basis(why: str) -> str:
    """Причина третьего исхода «судить нечем» — ЕДИНСТВЕННАЯ формулировка класса на обоих
    производителей ожиданий (`_smoke_checks` здесь и `trainer_run.case_checks`)."""
    return "судить нечем: " + str(why)


def _smoke_checks(draft, exp) -> list:
    """Чеки инвариантов #92 и смежных по ГОТОВОМУ черновику → список dict(name, ok, expected, fact).
    Чек, ожидание которого НЕ СОБРАЛОСЬ, несёт ещё и `unknown` (см. узел «ОЖИДАНИЕ НЕ СОБРАЛОСЬ»
    выше): для прибора набора это третий исход, для живого смоука — прежний провал по `ok`."""
    ...
    if not j_lines:
        chk["unknown"] = no_basis("quote-блока в ценовой записке нет — искать нечего")
    checks.append(chk)
    ...
    if not dl or zprice is None:
        chk["unknown"] = no_basis("блока доставки в ценовой записке нет" if not dl
                                  else "цена зоны не названа ни кейсом, ни строкой доставки")
    checks.append(chk)
...
# ПАСПОРТ ПОРОГА: терпит=НИЧЕГО — это АДРЕС группы, а не лимит; открытие нашло его по форме
#   (числовая ручка окружения), и молча выкидывать находку нельзя: исключение обязано быть
#   ЗАПИСЬЮ, а не дыркой в правиле открытия | замер=НЕТ | снят=— | делится=НЕ ДЕЛИТСЯ
#   | предел-единицы=НЕ НУЖЕН | род=не порог | артефакт=—
