...
* ``paid_mute`` (21.09.2026) — канал ОТРАБОТАЛ заход, списал за него деньги и не
  отдал ответа нашей дверью. Повтора нет НИ ОДНОГО ВИДА: отправка купила бы
  второй платный разбор тех же вопросов (замер 19–21.09: 6 пакетов → 9 задач →
  10 кредитов → 0 ответов), забор вернул бы ту же пустоту (задача терминальна).
  Исход — НЕИЗВЕСТНО, и идентификатор оплаченной задачи записывается в запись,
  чтобы пережить сутки.
...
ORIGINS = ("external", "spent", "ours", "channel_answered", "paid_mute")
...
_EXTERNAL = {
    ...
    # Канал закончил задачу и не сказал ни слова. Здесь это `external` только для
    # каналов, НЕ названных в `_MUTE_CHANNELS`: у manus тот же ярлык уводит в
    # `paid_mute` (см. ниже) — его первая половина обоснования умерла замером.
...
# ЗАХОД ОПЛАЧЕН, А КАНАЛ НЕМ — и потому повтора нет НИ ОДНОГО ВИДА. Отдельное
# происхождение, а не оттенок `external` и не оттенок `spent`, потому что от
# обоих оно отличается ДЕЙСТВИЕМ, и оба действия здесь вредны:
#
# * `external` → ОТПРАВКА. Она покупает ВТОРОЙ платный разбор тех же вопросов и
#   тоже его не забирает. Замер 19–21.09.2026 по девяти живым телам канала
#   (`GET /v1/tasks/{id}`, снимок 21.09 11:05, разбор 69s): 6 пакетов → 9 задач →
#   10 кредитов → 0 собранных ответов; три пакета уехали больше одного раза,
#   дайджест 21.09 — дважды с разницей 19 мин, оба кредита списаны, оба тела пусты.
# * `spent` → ЗАБОР. Он вернул бы ту же пустоту: задача терминальна, и перечитанная
#   через 2 ч 20 мин она по-прежнему несёт НОЛЬ сообщений ассистента.
#
# ПЕРВАЯ ПОЛОВИНА СТАРОГО ОБОСНОВАНИЯ УМЕРЛА ЗАМЕРОМ, И ЭТО ЗДЕСЬ ГЛАВНОЕ.
# Комментарий у `channel_idle` говорил: «канал сам напечатал квитанцию в ноль —
# значит „заход оплачен“ про этот случай ЛОЖНО». Свой счёт 21.09 по тем же девяти
# телам: квитанция ноль ровно у ОДНОЙ задачи, у восьми `credit_usage` = 1 или 2.
# То есть заход оплачен в 8 случаях из 9, и ветка «повторяем отправкой» стояла на
# факте, которого больше нет. Вторая половина («терминальная пустая задача пуста
# навсегда») жива — потому и `refetch` здесь не открывается.
#
# Исход при этом остаётся НЕИЗВЕСТНО и «готово» не пишется ни одной веткой
# (:func:`outcome`, :func:`archive_outcome_of`): ответ на стороне канала
# существует — его видно в его же кабинете, — и утверждать «ответа не будет»
# слой не вправе.
_PAID_MUTE = {
    "channel_idle": "канал отработал задачу, списал кредит и не сказал ни слова",
}
# Каналы, у которых немота ИЗМЕРЕНА. Список, а не «все»: у codex свои исходы и
# своя цена, мерили его не мы и не сегодня. Сегодня ярлык `channel_idle` рождает
# ровно один классификатор — `review_send.classify_manus` (грep 21.09: одно место
# на весь живой код), поэтому список из одного имени покрывает весь живой класс,
# а не его часть.
_MUTE_CHANNELS = ("manus",)
...
def origin(reason, channel=None):
    """Причина отказа (+ КАНАЛ) → происхождение и человеческое «почему». → (origin, why).
    ...
    КАНАЛ — ВТОРОЙ ВХОД, И ОН НЕОБЯЗАТЕЛЬНЫЙ. Одна и та же причина у разных
    каналов стоит разных денег: немота manus оплачена (8 квитанций из 9 не ноль,
    замер 21.09), и повтор отправкой там покупает второй разбор, а про codex это
    не мерил никто. Канал не назван → судим как раньше, по причине: запись без
    канала очередь не собирает вовсе (:func:`key` требует оба поля), так что
    молчаливой дырой этот вход не становится.
    """
    ...
    if reason in _PAID_MUTE and (channel or "").strip().lower() in _MUTE_CHANNELS:
        return "paid_mute", _PAID_MUTE[reason]
...
def retry_kind(org, task_id=None):
    ...
    if org == "paid_mute":
        # Повтора нет НИ ОДНОГО ВИДА, и обе двери закрыты РАЗНЫМИ фактами:
        # отправка — ценой (второй платный разбор), забор — пустотой (задача
        # терминальна). Идентификатор задачи здесь не ключ к повтору, а
        # СОХРАНЁННЫЙ АДРЕС оплаченного: по нему спрашивают руками и у канала.
        if task_id:
            return "none", (
                "канал отработал и промолчал: ОТПРАВКА купила бы второй платный разбор тех же "
                "вопросов, ЗАБОР вернул бы ту же пустоту (задача терминальна). Оплаченная задача "
                "%s записана и не теряется" % task_id
            )
        return "none", (
            "канал отработал и промолчал: ОТПРАВКА купила бы второй платный разбор тех же "
            "вопросов, ЗАБОР вернул бы ту же пустоту (задача терминальна). Идентификатора "
            "оплаченной задачи в шапке захода нет — спрашивать по ней нечем"
        )
    if org == "spent":
        if task_id:
...
def advance(rec, *, reason, at, task_id=None):
    ...
    org, why = origin(reason, rec.get("channel"))
    # Идентификатор задачи НАКАПЛИВАЕТСЯ, а не переписывается пустотой: свежий
    # заход мог не назвать его вовсе (канал не отдал `task_id`), и обнулить им
    # уже записанный значило бы потерять адрес оплаченного разбора — ровно то,
    # ради чего он здесь и лежит.
...
def archive_outcome_of(rec):
    ...
    if rec.get("origin") == "paid_mute":
        # Отказом это звать НЕЛЬЗЯ: канал не отверг работу, он её СДЕЛАЛ и списал
        # за неё деньги — просто не отдал результат нашей дверью. «ОТКАЗ КАНАЛА»
        # здесь был бы ложью о чужой работе и о наших тратах разом.
        return "unknown", "НЕИЗВЕСТНО", (
            "канал отработал заход и оплату списал, а ответа нашей дверью не отдал — он мог "
            "быть написан на его стороне; повтор отправкой купил бы второй платный разбор и "
            "тоже его не забрал%s"
            % (", оплаченная задача %s записана" % rec["task_id"] if rec.get("task_id") else "")
        )
...
# Третья шапка, и она не оттенок первых двух: канал не лежал (работу сделал и
# деньги списал) и нашей вины тут нет (пакет доехал целиком). Назвать это
# «КАНАЛ ЛЕЖАЛ» значило бы позвать читающего повторить отправку рукой — то
# самое действие, которое эта ветка и закрывает.
_HEAD_MUTE = "СОСТОЯНИЕ КАНАЛА (не находка и не задание): КАНАЛ ОТРАБОТАЛ, СПИСАЛ И ПРОМОЛЧАЛ"
...
def audit_text(rec, *, queued_left=None):
    ...
    if org == "paid_mute":
        # Команды повторной ОТПРАВКИ здесь нет и быть не должно: напечатать её
        # значит предложить владельцу купить второй платный разбор тех же
        # вопросов. Печатаем то, что ДЕЙСТВИТЕЛЬНО можно сделать с оплаченным —
        # спросить задачу по её идентификатору, ценой ноль новых заходов.
        lines += [
            "",
            "повторной отправки НЕТ и рукой она не предлагается: заход уже оплачен, "
            "а второй такой же купил бы второй разбор тех же вопросов",
        ]
        if rec.get("task_id"):
            lines += [
                "спросить оплаченную задачу (новых заходов ноль):",
                "  venv/Scripts/python.exe review_send_run.py --manus-task %s" % rec["task_id"],
            ]
        else:
            lines.append("идентификатора оплаченной задачи в шапке захода не было — спрашивать нечем")
        lines += ["", _TAIL]
        return "\n".join(lines)
    lines += [
        "",
