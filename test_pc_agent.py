...
class TestUnknownCommandReply(unittest.TestCase):
    ...
    def test_long_command_truncated(self):
        """ЭХО ОБРЕЗАНО — и мерится именно ЭХО, а не вся строка ответа.

        До 09.09.2026 здесь стоял потолок на ВЕСЬ ответ (`< 900` при живых 792), и падал он не
        от раздутого эха, а от роста ПЕРЕЧНЯ команд: четыре новых формы «урок набор …» увели
        ответ на 1118 (запаса оставалось 108 символов на все будущие двери сразу) — то есть тест
        ловил бы каждую новую дверь агента как дефект обрезки,
        которой она не касается. Замер обоих чисел — в артефакте НАБОР-СНЯТИЕ-ДВЕРЬ-…-0909.
        Предмет разведён: здесь — вклад эха, ниже — свой потолок перечня."""
        r = a.unknown_command_reply("ы" * 500)
        self.assertIn("…", r)
        self.assertNotIn("ы" * 200, r, "полкоманды доехало до ответа")
        self.assertLess(len(r) - len(a.HELP), 200, "эхо раздувает ответ, а не обрезается")

    def test_known_commands_nonempty(self):
        self.assertTrue(len(a.KNOWN_COMMANDS) >= 7)

    def test_the_command_list_still_fits_one_telegram_message(self):
        """У ПЕРЕЧНЯ СВОЙ ПОТОЛОК, и он не про обрезку эха.

        Перечень растёт с каждой новой дверью агента (замер: 14 форм → 715 символов до
        09.09.2026, 17 форм → 956 после), и это законный рост. Незаконным он станет там, где ответ
        перестаёт
        быть ОДНИМ сообщением Telegram (4096 символов): дальше его порежет транспорт, и владелец
        увидит обрубок перечня без слова о том, что он обрублен. Потолок взят с запасом вдвое."""
        self.assertLess(len(a.HELP), 2000, "перечень команд пора резать или уводить в файл")
...
class TestAccountsWord(unittest.TestCase):
    """«учётки» / «учётка N» (25.09.2026, задание «учётки одним словом»).

    ОТРИЦАТЕЛЬНЫЕ ТЕСТЫ ЗАДАНИЯ: не владелец — отказ словами, дверь не поднимается; «учётка 9» —
    доходит до двери ТОЛЬКО числом, отказ произносит дверь (реестр 1…3). Дверь `_accounts_cli`
    подменена шпионом, отправка — сборщиком: наружу не уходит ничего, subprocess не зовётся."""
    ...
    def setUp(self):
        ...
        def _spy(action, number=None):
            self.door.append((action, number))
            return "ДВЕРЬ: %s %s" % (action, number)
        for name, val in (("_send", _collect), ("_accounts_cli", _spy)):
            self.addCleanup(setattr, a, name, getattr(a, name))
            setattr(a, name, val)

    def say(self, text, uid=None):
        msg = types.SimpleNamespace(text=text, message_thread_id=a.HQ_THREAD_ID)
        update = types.SimpleNamespace(
    ...
    def test_owner_nine_reaches_door_only_as_number(self):
        self.say("учётка 9")
        self.assertEqual(self.door, [("switch", "9")])
        self.assertIn("№9", self.sent[0])

    def test_garbage_number_refused_in_words(self):
        for word in ("учётка", "учётка три", "учётка 100", "учётка 3; rm"):
            with self.subTest(word):
                self.sent.clear()
                self.say(word)
    ...
    def test_cli_argv_carries_only_digits(self):
        ...
        with mock.patch.object(a.subprocess, "run", side_effect=fake_run), \
                mock.patch.object(a, "VENV_PY", Path(sys.executable)):
            door = a._accounts_cli                  # шпион setUp снимается на время прямого вызова
            a._accounts_cli = type(self)._real_cli
            try:
                self.assertEqual(a._accounts_cli("switch", "03"), "ok")
                self.assertEqual(seen["argv"][-2:], ["--switch", "3"])
                self.assertEqual(a._accounts_cli("list"), "ok")
                self.assertEqual(seen["argv"][-1], "--report")
