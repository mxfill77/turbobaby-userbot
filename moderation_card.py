...
import moderation_core   # ЕДИНЫЙ текст «уже закрыто, кем» — его же видит живой бот (06.09.2026)
import price_source
...
# Словарь статусов и сам текст «уже закрыто» переехали в `moderation_core` (06.09.2026) — и не
# ради красоты: ТОТ ЖЕ текст обязан показывать опоздавшему ЖИВОЙ бот, а он импортирует
# moderation_core и не импортирует эту карточку. Две копии одного текста разъехались бы и
# соврали по-разному в двух местах одного контура. Здесь оставлено ИМЯ (ссылка на живой словарь),
# чтобы читавший карточку не потерял след.
_STATUS_RU = moderation_core.STATUS_RU
...
def render_closed(row):
    ...
    Текст один на весь контур и живёт в `moderation_core.render_closed` — карточка и ЖИВОЙ бот
    показывают опоздавшему ДОСЛОВНО одно и то же."""
    return moderation_core.render_closed(row)
