...
ГРАНИЦА ПРОЦЕССА НА ОТДЕЛЬНОМ РЕБРЕ (`PROCESS_BOUNDARY_EDGES`, 07.09.2026). Срез умеет выкинуть
ТОЛЬКО файл целиком, а бывает файл, который живой бот импортирует по-настоящему (и его синтаксис
боту важен), но ОДИН его импорт исполняется в ОТСОЕДИНЁННОМ процессе. Живой случай ровно один:
`lesson_regress.py` — бот зовёт из него `spawn()` (Popen `lesson_regress.py --after-lesson`,
DETACHED_PROCESS), а `import trainer_run` стои́т ВНУТРИ `measure()`, то есть исполняется только в
ребёнке. Срезом такое не выражается: срез снял бы сам `lesson_regress.py`, а он клиентский честно.
Поэтому граница — на ПАРЕ (файл-импортёр, имя модуля), и её три замка:
  • ребро НАЗВАНО поимённо и живёт одним кортежем, который читается глазами (не эвристика);
  • граница действует только на это ребро: тот же модуль, импортированный ЛЮБЫМ другим файлом
    замыкания, въезжает как раньше (проверяется отрицательным тестом на синтетическом дереве);
  • право стои́т на ЛЕНИВОСТИ импорта: поднимут `import trainer_run` на верхний уровень — бот
    начнёт грузить прогонщик В СВОЮ ПАМЯТЬ, и граница станет ложью. Это стережёт тест
    `test_trainer_run` (импорт обязан лежать внутри функции, импортёр обязан быть ровно один).
Направление ошибки здесь НЕ безопасное (граница сужает охрану), поэтому кортеж короткий и каждое
новое ребро обосновывается замером «меняет ли правка ответ клиента хоть одной дорогой».
...
# ГРАНИЦА ПРОЦЕССА НА ОДНОМ НАЗВАННОМ РЕБРЕ (07.09.2026). См. «ГРАНИЦА ПРОЦЕССА» в шапке.
# Пара — (ФАЙЛ-ИМПОРТЁР, ИМЯ МОДУЛЯ): обход не идёт ЭТИМ ребром, но сам файл-импортёр остаётся в
# замыкании целиком, и все ОСТАЛЬНЫЕ его импорты считаются как раньше. Этим граница отличается от
# среза: срез выкидывает файл, граница — одно ребро.
PROCESS_BOUNDARY_EDGES = (
    # `lesson_regress.spawn()` поднимает ОТСОЕДИНЁННЫЙ процесс `lesson_regress.py --after-lesson`
    # (DETACHED_PROCESS), и `import trainer_run` живёт ВНУТРИ `measure()` — то есть исполняется
    # только в ТОМ процессе. В памяти живых ботов прогонщика нет ни одной дорогой: `trainer.py`
    # зовёт из бота ровно `spawn()`. Ребро межпроцессное, и держать по нему прогонщик клиентским
    # значит требовать слова владельца на каждую правку экзамена.
    ("lesson_regress.py", "trainer_run"),
)
_BOUNDARY = frozenset(PROCESS_BOUNDARY_EDGES)
...
def _build(repo, entries, cut=FOREIGN_ENTRIES):
    ...
    while queue:
        ...
        for name in _imports_of(tree):
            if (here, name) in _BOUNDARY:
                continue                   # граница процесса: ребро исполняется в ЧУЖОМ процессе
...
# ------------- ЗАМОРОЗКА КОНТУРА: под заморозкой владельца НЕ СПРАШИВАЮТ ВОВСЕ -----
...
# задержанный коммит становился ВИДИМЫМ немедленно. Уводя отказ в ленту, мы теряем именно
...
#
# ПОЧЕМУ ПРАВИЛО «ГЛУШИМ ПОВТОР ФОРМЫ» НЕ СРАБОТАЛО (замер 05.09.2026, тот же лог). С 21.08 в ленту
# уходил ПОВТОР отказа той же формы, а форма = «кого держим | какие клиентские файлы». Коммит из
# подписи убрали — и правильно, но список файлов в ней остался, а он растёт с КАЖДОЙ задачей ночи:
# метка `_last_child_commit` при отказе не двигается, поэтому дифф считается от одной и той же
# старой точки и хвост клиентских файлов накапливается. За сутки 05.09: 25 отказов ворот, 21
# подавлен, и ЧЕТЫРЕ формы всё-таки родились заново (7 файлов → 8 → 10 → 3) — четыре карточки на
# один и тот же ответ «нет, контур заморожен». Дефект не в пороге и не в дедупе: сама МЕРА «новая
# форма — новость» под заморозкой ложна, потому что новость меряется составом файлов, а решение
# владельца от состава файлов не зависит вовсе.
#
# ПОСТОЯННОЕ РЕШЕНИЕ ВЛАДЕЛЬЦА 05.09.2026, дословно: «пока контур заморожен — про выкатку детей не
# спрашивать вовсе». Поэтому ВОПРОС ИСЧЕЗАЕТ КАК КЛАСС: пока лежит флаг, адрес отказа один — лента,
# независимо от формы, состава файлов, коммита и от того, записался ли счётчик. Первый отказ формы
# больше НЕ громкий: это и была та единственная ветка, которая рождала все четыре карточки.
#
# ЧЕГО ЭТО НЕ ТРОГАЕТ (границы решения, каждая — замок с тестом):
#   • FAIL-CLOSED. Ворота держат ровно как держали: обе ветки возвращают АДРЕС сообщения, а не
#     право на выкатку. Молчание ≠ разрешение — под заморозкой выкатка не происходит по-прежнему;
#   • ЖУРНАЛ. Отказ пишется в ленту ЦЕЛИКОМ и как прежде — коммит, кого держим, поимённый состав
#     клиентских файлов, причина (`_client_block` → `_cowork`). Невидимо владельцу ≠ невидимо вообще;
#   • РАЗМОРОЗКА. Нет флага → прежнее поведение байт-в-байт, спрашиваем на КАЖДОМ отказе. Это
#     первая ветка функции, и она не изменилась ни строкой;
#   • ЧУЖИЕ КЛАССЫ. Состоявшаяся выкатка, провал самообновления по гейту, голос выкатки
#     (`deploy_voice`) и находки ревизора (`_revizor_undecidable`) сюда не заходят и не глушатся.
#
# ЗАВЕДЕНИЕ И ОТМЕНА — ОДНО ДВИЖЕНИЕ И ОБА СЛОВОМ ВЛАДЕЛЬЦА: правило живёт ровно столько, сколько
# лежит файл `pc_orchestrator.contour_frozen`. Создать файл — тишина включена, удалить — карточки
# возвращаются тем же тиком (демон читает признак каждый раз, перезапуск не нужен). Другой ручки
# нет НИ ОДНОЙ: ни env, ни конфига, ни порога — снимать нечего, кроме файла.
#
# ПОЧЕМУ РЕЕСТР ФОРМ ОСТАЁТСЯ, ХОТЯ АДРЕСА БОЛЬШЕ НЕ РЕШАЕТ: он считает, СКОЛЬКО раз одна форма
# была отвергнута в эту заморозку, и это число уезжает в ленту — разбору оно нужно ровно так же,
# как раньше. Но теперь счёт — БУХГАЛТЕРИЯ, а не решение: сорвалась запись → в ленту уедет отказ
# без номера, и НИ ОДНА ветка не превратит это в карточку. Прежний fail-loud («не смогли запомнить
# → снова громко») здесь снят СОЗНАТЕЛЬНО: он охранял правило, которого больше нет. Fail-loud
# остался там, где он про знание, а не про память: `frozen()` при любом сбое отвечает False, то
# есть «не знаю, заморожены ли» → СПРАШИВАЕМ.
#
# ПОЧЕМУ РЕЕСТР НА ДИСКЕ, А НЕ В ПАМЯТИ: демон перезапускается self-update'ом по нескольку раз в
# сутки, и память процесса начинала бы счёт заново. ПОЧЕМУ У ЗАПИСИ ЕСТЬ ЭПИЗОД (mtime флага):
# заморозка — состояние временное, и счёт «мы про эту форму уже говорили» верен ВНУТРИ одного
# эпизода; следующая заморозка начинает счёт заново.
...
def _seen_bump(shape, episode, ts, path=None):
    """Счётчик отказов ОДНОЙ формы внутри ОДНОГО эпизода заморозки → (номер | None, причина сбоя).

    Ведёт ту же запись, что вёл прежний дедуп, но АДРЕСА не решает ни одной веткой: под заморозкой
    адрес один, и сбой записи его не меняет (см. «ПОЧЕМУ РЕЕСТР ОСТАЁТСЯ» в шапке раздела).
    `None` — «номер назвать не смог», а НЕ «первый»: в ленту уедет отказ без номера и с дословной
    причиной, а не отказ, притворившийся первым."""
    shapes = _seen_load(path)
    rec = shapes.get(shape)
    prev_ep = rec.get("ep") if isinstance(rec, dict) else None
    if not isinstance(prev_ep, int) or prev_ep != int(episode):   # форма новая ИЛИ заморозка другая
        rec = {"ep": int(episode), "first": ts, "last": ts, "n": 1}
    else:
        n = rec.get("n")
    ...
    return (rec["n"] if saved else None), err
...
def gate_route(kinds, held, flag=None, path=None, now=None, episode=None, commit=None,
    ...
    не влияет ничем: ворота fail-closed и держат в обоих исходах.

    ПРАВИЛО ОДНО И ЧИТАЕТСЯ ЦЕЛИКОМ: заморозка лежит → ВСЕГДА лента, вопроса владельцу нет вовсе
    (постоянное решение владельца 05.09.2026). Заморозки нет → ВСЕГДА карточка, прежнее поведение
    байт-в-байт. Ни состав файлов, ни коммит, ни номер повтора, ни успех записи реестра адреса
    больше не меняют — ровно они и рождали карточки, которые владелец запретил.
    ...
    if d:
        return d
    # ЕДИНСТВЕННАЯ ветка, ведущая к вопросу владельцу. Стои́т ДО всякой работы с формами: пока она
    # цела, снятая заморозка возвращает карточки тем же тиком и на КАЖДОМ отказе.
    if not frozen(flag):
    ...
    ts = time.time() if now is None else now
    n, err = _seen_bump(shape, ep, ts, path)
    why = ("контур заморожен — про выкатку детей владельца не спрашиваю ВОВСЕ (постоянное решение "
           "владельца 05.09.2026); отказ %s формы «%s» — в ленту"
           % (("№%d" % n) if n else "(номер назвать не смог)", shape))
    if n is None:
        why += (" (счётчик форм НЕ записан [%s]; на молчание это не влияет — решение владельца "
                "стои́т на флаге заморозки, а не на реестре)" % err)
    return ROUTE_FEED, why
...
# ── АДРЕС ОТВЕТА (05.09.2026, жалоба владельца «я в этой группе вообще ничего нажать не могу») ──
# Карточка ворот уезжает в ТЕМУ-ИНБОКС 1160 (dispatch_notify.deliver → send_critical: замер
# _scratch_card_where_0905/probe_where.py). Слово, написанное ТАМ ЖЕ, рычага не поднимает ни одной
# веткой — и это не догадка: devbot темы-инбокса отвечает «это тема-инбокс подтверждений: „да N“ /
# „нет N“ или кнопки под карточкой», а самого слова «выкати» не знает вовсе (0 совпадений по
# devbot.py клона VPS). Разбор рычага живёт на ПОЛОСЕ ПК (pc_orchestrator._match_command) и видит
# только ТЕКСТ ЗАДАЧИ ОЧЕРЕДИ. Значит у карточки два честных ответа, и оба обязаны быть В НЕЙ:
#   • ОСНОВНОЙ — КНОПКА под самим сообщением (dispatch_notify --gate-card → pc_agent ловит
#     callback тем же токеном). Кнопка отвечается ровно там, где карточка показана;
#   • ЗАПАСНОЙ — СЛОВО, но названным местом: тема PC-дев, сообщением «задача: <слово>». Только
#     оттуда слово становится задачей очереди, которую разберёт тот самый _match_command.
# Ни одна из дорог новых слов не принимает и ворот не открывает мимо прежних оснований: кнопка
# ПОДСТАВЛЯЕТ то же самое слово в тот же самый разбор (pc_orchestrator --gate-word).
ANSWER_AT = (
    "жми кнопку под этим сообщением — [✅ Выкатить] или [⛔ Не выкатывай].\n"
...
# Та же мысль без обещания кнопки — для доставки, где кнопок не будет (личка-фолбэк).
ANSWER_AT_WORDS = (
    "кнопок у этого сообщения нет — напиши в тему «PC-дев» одно сообщение:\n"
    "   «задача: выкати» либо «задача: не выкатывай». В теме-инбоксе голое слово НЕ\n"
    "   сработает: она принимает только «да N» / «нет N» и кнопки.")


def card_text(kinds, commit, client_files, subject="", where="", trainer_available=False,
              trainer_note="", answer_at=None, buttons=True):
    ...
    АДРЕС ОТВЕТА ОБЯЗАТЕЛЕН (05.09.2026, вечер). Карточка называла СЛОВА ответа и молчала о том,
    КУДА их писать, — а писать их там, где карточка показана, бесполезно (см. ANSWER_AT). Пустой
    `answer_at` — не «карточка без адреса», а ValueError: карточка, просящая невозможного, хуже
    отсутствующей, поэтому собраться без адреса она не может ВОВСЕ. `answer_at=None` — не пустой,
    а «возьми дефолт полосы» (ANSWER_AT / ANSWER_AT_WORDS); замолчать адрес можно только явной
    пустой строкой, и она запрещена. `buttons=False` — честная ветка доставки, где кнопок нет
    (личка-фолбэк): тогда единственным путём называется слово, и карточка не обещает кнопку,
    которой владелец не увидит."""
    at = ((ANSWER_AT if buttons else ANSWER_AT_WORDS) if answer_at is None
          else str(answer_at or "")).strip()
    if not at:
        raise ValueError("card_text: адрес ответа не назван — карточка без него не собирается "
                         "(владелец не должен угадывать, куда писать «выкати»)")
