...
БЮДЖЕТ ВРЕМЕНИ — ВТОРОЙ ЗАМОК. Одолженный у демона клиент моста несёт сокет-таймаут 90с на
ОДНУ пробу; девять зависших подряд повесили бы путь ответа на четверть часа. Пробы обёрнуты
дедлайном `PRICE_GATE_PROBE_SEC` (дефолт 90с на ВСЕ девять): исчерпан — следующая проба
...
ОТКУДА 90, А НЕ ПРЕЖНИЕ 20 (замер 21.08, `docs/artifacts/2026-08-20-freshness-budget-fix.md`).
Число 20 не было измерено ничем: оно опиралось на «пробы по 306с и 599с», а те 306/599 секунд
в `2026-08-20-silent-timeout.md` — цена ПОЛНОГО unittest-гейта, а не похода к мосту. Живая цена
прохода: шесть раз подряд боевой дверью — лучший 40.1с · медиана 41.1с · ХУДШИЙ 43.0с, одиночная
проба 3.4–6.3с. Прежний бюджет был меньше нужды ВДВОЕ, поэтому последняя категория (J3) голодала
детерминированно — на КАЖДОМ ответе, — и сторож гасил цену при вердикте, который на полных
данных равен `СВЕЖЕЕ`: в проде бот не назвал бы цену ни разу. 90 = двукратный запас от худшего
прохода (43.0с) и ровно потолок одной двери (`bc.timeout=90`): бюджет МЕНЬШЕ потолка одной
пробы отдаёт весь проход одной медленной двери, и сторож слепнет от неё же.

ЧЕМ ЗА ЭТО ПЛАТИТ КЛИЕНТ, названо числом. Дедлайн проверяется ПЕРЕД пробой, поэтому потолок
ожидания — 90 (бюджет) + 90 (последняя проба до сокет-таймаута) = 180с, а обычная цена — те
самые 40–43с. Платится она РАЗ В ОКНО `PRICE_GATE_TTL_MIN`, а не на каждое сообщение, и внешнего
предела на путь ответа нет (`userbot_listen.py` зовёт `build_pricing_note` через `to_thread`
без `wait_for`) — то есть увеличенный бюджет никем снаружи не обрезается. Осторожность не
тронута: «не успел прочитать» по-прежнему означает МОЛЧАНИЕ цены, меняется только то, что
«не успел» перестаёт случаться на здоровом мосте.
...
PROBE_DEFAULT = 90.0            # замер 21.08: худший проход 43.0с (лучший 40.1) → двукратный запас
                                # и ровно потолок одной двери (bc.timeout=90). Прежние 20с гасили
                                # цену на КАЖДОМ ответе: J3 не успевала спроситься ни разу.
