...
PARTS = (
    # ЗДОРОВЬЕ СТОИ́Т ПЕРВЫМ, И ЭТО НЕ ВКУС. Витрина режется с ХВОСТА при переполнении
    # (:func:`render`, потолок :data:`TEXT_MAX`), а прямой заказ владельца 04.09 — «где я могу
    # посмотреть, что работает, а что не работает». Список приговоров, который вправе оказаться
    # обрезанным, хуже отсутствующего: он молча покажет часть узлов как весь контур.
...
def health_row(node):
    ...
    Три вещи обязательны у КАЖДОЙ строки (пункт 4 задания), и порядок их — порядок чтения с
    телефона: сперва слово, потом чем оно снято, и только потом слепота прибора.
    """
    ...
    blind = one_line(got.get("blind"), 120)
    if blind:
        out += " · не покрывает: %s" % blind
    return out


def part_health(nodes):
    ...
    return [health_row(n) for n in nodes]
...
def body(facts):
    ...
    made = {
        "health": part_health(got.get("health")),
