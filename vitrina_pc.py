...
def part_nums(counted, taken, day, external, awaiting, box_stop=""):
    """ЧИСЛА: серия с ОПОРОЙ · ящик Штаба · внешний контур · карточки гарда И ЗАЯВКИ.
    ...
    out.append("карточки гарда в ожидании: %s"
               % number(None if awaiting is None else got.get("cards"), why))
    out.append("заявки внешних каналов ждут решения: %s (ответ кнопкой; задачами они не станут)"
               % number(None if awaiting is None else got.get("zayavki"), why))
    return out
...
def part_owner(waiting):
    ...
    for w in waiting:
        if w.get("zayavka"):
            rows.append("#%s ЗАЯВКА внешнего канала (ключ %s) — суть и кнопки в инбоксе, "
                        "задачей не станет" % (w.get("id"), w.get("key") or "?"))
        else:
            rows.append("#%s %s" % (w.get("id"), one_line(w.get("goal"), GOAL_MAX)))
    return listed(rows)
