...
# ═══ СЛЕД «ИСХОД ПОБЫВАЛ НЕСВЕРЕННЫМ» — ЧАСТОТА КЛАССА (19.09.2026) ══════════════════════════
# ПОЧЕМУ ФЛАГ НА РЯДУ, А НЕ СЧЁТЧИК. Состояние «не сверен» ЖИВЁТ МИНУТАМИ: сверка упавших зовётся
# на ПЕРВОМ же обороте после ухода строки, и к ближайшему снятию слепка исход уже стои́т словом.
# Поэтому счётчик `исход не сверен N` в теле слепка показывает состояние НА МОМЕНТ СНЯТИЯ и в
# обычные сутки честно печатает ноль — класс оставался невидимым в суточных числах ЦЕЛИКОМ
# (замер 19.09: окно ложного нуля 554 с не оставило в суточной статистике ни одного следа,
# `docs/artifacts/2026-09-19-SERIYA-67m-106-1909.md` §6).
#
# СОБЫТИЕ ЗДЕСЬ ОДНО НА РЯД И НЕПОВТОРИМО: запись в реестр закрытых делается РОВНО ОДИН РАЗ
# (`merge_closed` пропускает ключ, который в реестре уже есть), а сам реестр подрезан окном
# суток. Значит «рядов с флагом» = «случаев за сутки», без отдельного счётчика, без второго
# файла и без единой новой записи на диск.
#
# ФЛАГ НЕ СТИРАЕТСЯ ПРИ СВЕРКЕ НАМЕРЕННО: ряд, чей исход уже прочитан, остаётся свидетелем того,
# что окно у него БЫЛО. Стереть — значит вернуть ровно ту невидимость, ради которой флаг и заведён.
F_WAS_UNSURE = "was_unsure"
...
def render_body(rows, closed, failed_at, now):
    ...
    # ДВА РАЗНЫХ ЧИСЛА ПРО ОДИН КЛАСС, И СЛИВАТЬ ИХ НЕЛЬЗЯ (19.09.2026, п. 3 решения Штаба):
    # «исход не сверен N» — СОСТОЯНИЕ НА МОМЕНТ СНЯТИЯ, а «побывал несверенным M» — ЧАСТОТА
    # КЛАССА ЗА СУТКИ. Первое почти всегда ноль (состояние живёт минутами), и читать его как
    # «такого не бывает» — ровно та ошибка, из-за которой окно ложного нуля 19.09 не оставило
    # в суточных числах ни следа. Второе печатается ВСЕГДА, в том числе нулём.
    parts.append("ЗАКРЫТО ЗА СУТКИ — на момент снятия "
                 "(%d: сдано %d, упало %d, отклонено владельцем %d, НЕИЗВЕСТНО %d, "
                 "исход не сверен %d; за сутки исход побывал несверенным у %d рядов):"
                 % (len(closed), len(done_ids), len(failed_rows), len(rejected_rows),
                    len(unknown_rows), len(unsure_ids), was_unsure))
    if done_ids:
...
def merge_closed(prev_open, now_open, prev_closed, now, window):
    ...
    for tid, item in prev_open.items():
        ...
        if key in closed:
            continue
        # ФЛАГ СТАВИТСЯ ЗДЕСЬ И ТОЛЬКО ЗДЕСЬ: это единственная минута полосы, когда исход
        # ряда становится несверенным, и единственное место, которое её знает (см. шапку
        # `F_WAS_UNSURE`). Ставится ВМЕСТЕ с самим `outcome: None`, а не отдельной веткой —
        # разойтись им тогда нечем.
        closed[key] = {"id": tid, "at": now, "goal": item.get("goal"),
                       "outcome": None, "why": "", F_WAS_UNSURE: True}
    return closed
...
def apply_failed(closed, failed_items, read_at, window):
    ...
    for it in failed_items:
        ...
        # СЛЕД «ПОБЫВАЛ НЕСВЕРЕННЫМ» ПЕРЕЖИВАЕТ СВЕРКУ. Запись здесь собирается ЗАНОВО и
        # кладётся поверх прежней целиком — без этой строки флаг молча терялся бы ровно у
        # тех рядов, у которых он и ставился, и суточное число случаев считало бы только
        # ещё не сверенные, то есть снова состояние момента, а не частоту класса.
        if isinstance(out.get(key), dict) and out[key].get(F_WAS_UNSURE):
            row[F_WAS_UNSURE] = True
        out[key] = row
