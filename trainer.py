...
# ── ДВА ПОЛЯ ОДНОЙ ПОДСКАЗКИ: наблюдение и правило (12.09.2026, экзамен) ────────────────────
# Критик ОДИН на обе двери — кнопку «🎓 Обучить» модербота и карточку экзамена (`exam_show`).
# Второго критика не заводим: он разъехался бы с первым молча, и владелец получал бы на одну и ту
# же ошибку бота два разных суждения в зависимости от того, из какого окна смотрит.
#
# Экзамену нужно на ОДНО поле больше, чем кнопке. Кнопка показывает гипотезу формой правила («не
# обещай сроки выдачи») — этого хватает, потому что рядом висит сам диалог и владелец видит, к чему
# правило. Карточка экзамена показывает ЗАМОРОЖЕННЫЙ ответ, снятый когда-то раньше, и правило без
# наблюдения заставляет владельца самому искать, ЧТО ИМЕННО в этом ответе не так. Поэтому у
# подсказки два поля: НАБЛЮДЕНИЕ (что не так в этом ответе) и ПРАВИЛО (как надо).
#
# СДЕЛАНО ФЛАГОМ, А НЕ ВТОРОЙ ФУНКЦИЕЙ, и умолчание `paired=False` выбрано ради боевого пути: у
# модербота вызов позиционный (`moderation_bot.py:409`), и его текст запроса этой правкой не
# меняется НИ НА СИМВОЛ — ни системная часть, ни пользовательская. Меняется только ветка
# `paired=True`, которой до сегодня не звал никто.
HYPS_PAIR_SEP = "||"


def split_pair(line):
    """Строка подсказки → dict(observation, rule). FAIL-SAFE и в этом весь смысл формы.

    Разделителя нет (старый плоский ответ, модель забыла формат, ответ обрезан) → наблюдение
    ПУСТО, а вся строка идёт правилом. Так подсказка остаётся показуемой и годной к уроку даже
    когда формат не соблюдён: правило — то, что ложится в базу уроков, и терять его из-за
    отсутствия второго поля нельзя. Обратный выбор (вся строка в наблюдение) дал бы урок с пустым
    текстом правила, то есть запись, по которой бот не ответит ничего."""
    s = " ".join(str(line or "").split()).strip()
    if HYPS_PAIR_SEP not in s:
        return {"observation": "", "rule": s}
    left, _, right = s.partition(HYPS_PAIR_SEP)
    left, right = left.strip(" -—:"), right.strip(" -—:")
    if not right:                      # «наблюдение ||» — правила нет, левая часть и есть правило
        return {"observation": "", "rule": left}
    return {"observation": left, "rule": right}


def hypotheses_prompt(incoming, answer, canon=None, rules=None, transcript=None, paired=False):
    """(system, user) для LLM-тренера: 2–4 короткие гипотезы «что не так/что улучшить» в ОТВЕТЕ
    бота, каждая формой правила.

    `paired=True` — та же сверка и те же материалы, но каждая строка просится ДВУМЯ полями
    («наблюдение || правило», :func:`split_pair`). Боевой путь модербота зовёт без этого довода и
    получает прежний запрос дословно.
    ...
    if paired:
        task = ("Назови от 2 до 4 КОРОТКИХ замечаний к ПОСЛЕДНЕМУ ОТВЕТУ бота. По одному на "
                "строку, без нумерации и без вводных слов, по-русски. КАЖДАЯ СТРОКА — ДВЕ ЧАСТИ "
                "через «" + HYPS_PAIR_SEP + "»:\n"
                "слева НАБЛЮДЕНИЕ — что именно не так В ЭТОМ ответе (с опорой на его текст, а не "
                "вообще);\n"
                "справа ПРАВИЛО — как надо на будущее («делай так» / «не делай так»).\n"
                "Образец строки: назвал срок выдачи, которого не знает " + HYPS_PAIR_SEP
                + " не называй сроков подготовки и выдачи — их согласует менеджер.")
    if canon:
    ...
    guard = (
        ...
        + HYPS_MARK_KEEP + "\n<годные гипотезы, по одной на строку%s>\n" % (
            ", каждая двумя частями через «" + HYPS_PAIR_SEP + "»" if paired else "")
...
def parse_hypotheses(text, limit=4, rules=None):
    ...
    for ln in keep_lines:
        ...
        if key in seen:
            continue
        seen.add(key)
        # Сверяем с книгой ПРАВИЛО, а не всю строку: у парной формы («наблюдение || правило»)
        # левая часть — описание ЭТОГО ответа, и с правилом книги она не совпадёт никогда, то есть
        # замок повтора при `paired=True` молча перестал бы работать. У плоской строки
        # `split_pair` отдаёт её же — поведение прежнего пути не меняется ни на символ.
        probe = split_pair(s)["rule"] or s
        hit = _repeat_of(probe, known, mod) if known else None
        if hit:
