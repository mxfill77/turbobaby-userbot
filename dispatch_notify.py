    ...
    try:
        ...
        if args and args[0] == "--hook":
            ...
            if kind == "stop":
                ...
                # «сессия кончилась». Тот же факт через СЕКУНДУ уходит в инбокс 1160 хуком
                # session_end, и там он с настоящей сводкой. Замер за 7 суток (29.07–05.08,
                # dispatch_notify.log): 392 сообщения в личку, из них 178 — эта строка, то
                # есть 45 % пульта тратилось на дубль без содержания.
                # Событие не теряется: строка лога ниже остаётся, карточка session_end уходит
                # в 1160, а `--hook stop` по-прежнему возвращает 0 (хук не считает это сбоем).
                _log.info("итог(stop): личка пропущена — итог сессии уходит в инбокс "
                          f"{INBOX_THREAD_ID} хуком session_end | {text[:90]}")
                sys.exit(0)
            if kind == "notification":
