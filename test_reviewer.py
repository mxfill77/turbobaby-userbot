...
class TestDepcheck(unittest.TestCase):
    ...
    # ── ЖИВЫЕ тексты прода (moderation_ipc.db, выписаны в артефакт 31.07): ВЫБОР, не находка ──
    # Правило-класс CLAUDE.md: голдены детекта — дословные фразы, а не идеальные образцы.

    def test_live_213_passportom_mozhno(self):
        """Окно 529849022, drafts.id=213, final_text — ОТПРАВЛЕНО клиенту 08.07.
        Союз «и» стоит в ПОСТОРОННЕЙ фразе («…ездили раньше и как долго»), выбор назван словом
        «паспортом можно». До клауз-скоупа чек красил этот текст."""
        self.assertIsNone(R._depcheck(
            "Спасибо за подробности! 🤝 На ваши даты, 15–20 июля, YAMAHA NMAX 155 на 5 дней — "
    ...
    def test_live_214_vmesto_deneg(self):
        """Окно 529849022, drafts.id=214: «как вы И хотели» — тоже посторонняя фраза."""
        self.assertIsNone(R._depcheck(
            "депозит 3000 бат — можно оставить паспорт вместо денег, как вы и хотели"))

    def test_live_sheet_slash_is_choice(self):
        """Строку печатает КОД из прайса: разделитель «/» здесь и есть «ИЛИ»."""
        self.assertIsNone(R._depcheck("• Депозит: 3000 ฿ / паспорт"))

    def test_price_slash_does_not_mask_real_finding(self):
        """«337 ฿/день» рядом — НЕ выбор: слэш засчитываем только вплотную к «паспорт»."""
        self.assertIsNotNone(R._depcheck("Аренда 337 ฿/день, депозит 3000 ฿ и паспорт."))

    def test_and_in_other_clause_not_flagged(self):
        """Союз «и» в соседнем предложении находкой быть не может."""
        self.assertIsNone(R._depcheck("Депозит 3000 ฿ — паспорт. Привезём и заберём бесплатно."))
