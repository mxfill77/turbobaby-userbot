...
import lesson_word_forms  # ФОРМА слова «урок включи N: причина» — разбор и образец в одном месте
...
#
# ФОРМА СЛОВА ЖИВЁТ НЕ ЗДЕСЬ (22.09.2026, задание 70i). Её знали четыре места, и три из них
# печатали ОБРАЗЕЦ формы руками — поправь глагол в этой регулярке, и образцы продолжили бы
# молча диктовать владельцу строку, которую разбор уже не принимает. Теперь определение одно
# (`lesson_word_forms.LESSON_ON_RE`), и образец собирает тот же модуль, сверяя собранное ЭТИМ
# ЖЕ разбором. Имя здесь остаётся прежним: разбор ниже и голдены `test_lesson_promote` зовут
# его по этому имени, и переименование было бы правкой ради переименования.
LESSON_ON_RE = lesson_word_forms.LESSON_ON_RE
...
# АДРЕС ОТВЕТА — ИЗ ТОГО ЖЕ МЕСТА, ЧТО И ФОРМА СЛОВА (22.09.2026, 70i): квитанция экзамена
# зовёт владельца сказать слово, и звать его она обязана ТУДА ЖЕ, где слово слушают. Строка
# ниже собирается из `PROMOTE_WHERE` и остаётся байт в байт прежней.
LESSON_ANSWER_AT = (lesson_word_forms.PROMOTE_WHERE + " / консоль ПК, командой "
