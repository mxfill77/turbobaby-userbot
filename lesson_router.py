...
# --- КЛАСС-ФИКС промаха осей (разбор #164): УСЛОВИЕ на состоянии данных/трекера → факт/логика (code) --
# Урок 292 «не пиши "данные получил", если клиент прислал только локацию — данных ещё нет» — это ФАКТ
# (истинность утверждения о состоянии диалога), а keyword-оси ловили behavior/unclear: речь-акт
# «не пиши/пиши» ПЕРЕВЕШИВАЛ условность на СОСТОЯНИИ ДАННЫХ. Корень: правило вида «(не) делай X,
# ЕСЛИ/КОГДА данные собраны/не собраны/присланы/получены/пусто» — это ГАРД/ИНВАРИАНТ машины на
# состоянии трекера окна, а НЕ манера речи → code НЕЗАВИСИМО от глагола. Разводит ОБЕ оси одинаково
# (расхождение осей больше не молчит — ЯВНОЕ ПРАВИЛО ПРИОРИТЕТА): classify_lesson_remark → ФАКТ,
# classify_lesson_type → CODE. Детерминированно, keyword — юнит без claude/сети.
_COND_MARKER_KW = (
    "если", "когда", "пока не", "как только", "в случае", "при условии", "стоит только",
    "unless", "only if", "only when", " if ", " when ",
)
# СОСТОЯНИЕ собранных/полученных данных или трекера (получен/собран/прислан/пришёл/заполнен/пусто/
# не хватает). НЕ включаем «назвал/выбрал/захотел» — это ПРЕДПОЧТЕНИЕ клиента (дефолт-предложение =
# поведение), а не состояние данных; и НЕ «молчит» в общем виде («клиент молчит» — про каденс, тон).
_DATA_STATE_KW = (
    "получ", "собра", "не собра", "прислал", "прислан", "присыл", "пришл", "пришёл", "пришел",
    "не пришл", "заполн", "не заполн", "хватает", "не хватает", "нехватает", "данных нет",
    "нет данных", "данных ещё нет", "данные есть", "есть данные", "пуст", "трекер",
    "received", "collected", "filled", "missing", "empty", "tracker",
)
# Дефолт-ПРЕДЛОЖЕНИЕ (что предлагать по умолчанию) — ПОВЕДЕНИЕ даже с условием-фолбэком: условие лишь
# описывает КОГДА действует дефолт (суть — что ПРЕДЛАГАТЬ). Такое из code-ветки условия исключаем.
_DEFAULT_OFFER_KW = ("по умолчанию", "умолчанию", "дефолт", "default")


def _data_state_condition(low):
    """True, если замечание — ПРАВИЛО С УСЛОВИЕМ на состоянии данных/трекера («…если/когда данные
    собраны/присланы/получены/пусто…»): это гард/инвариант МАШИНЫ, не манера речи → code НЕЗАВИСИМО
    от речь-акта «не пиши/пиши» (корень промаха 2-й оси #164, урок 292). Исключение — дефолт-
    предложение («…по умолчанию…»): условие там описывает КОГДА действует дефолт (что ПРЕДЛАГАТЬ =
    поведение), а не состояние данных. low — уже нормализована (lower, схлопнуты пробелы)."""
    if any(d in low for d in _DEFAULT_OFFER_KW):
        return False
    if not any(c in low for c in _COND_MARKER_KW):
        return False
    return any(s in low for s in _DATA_STATE_KW)
...
def classify_lesson_remark(remark):
    ...
    # Правило с УСЛОВИЕМ на состоянии данных/трекера — фактическая логика (истинность утверждения),
    # а не тон/манера: → ФАКТ независимо от речь-акта «не пиши/пиши» (корень #164, урок 292). Бьёт
    # попутный стилевой сигнал так же, как факт-слово (условие на данных ≠ формулировка).
    if _data_state_condition(low):
        return FACT, "условие на состоянии данных/трекера → факт/логика"
    if fact:
...
def classify_lesson_type(remark):
    ...
    # ПРИОРИТЕТ (класс-фикс #164): правило с УСЛОВИЕМ на состоянии данных/трекера («…если/когда данные
    # собраны/присланы/получены/пусто…») — про МАШИНУ (гард/инвариант на состоянии окна), а НЕ манера
    # речи: речь-акт «не пиши/пиши» тут НЕ перевешивает. Урок 292 «не пиши "данные получил", если клиент
    # прислал только локацию» — ФАКТ→code (ось-1 LLM права), keyword ловил behavior на «не пиши». Условие
    # на данных → code независимо от глагола, ДО keyword-разбора behavior/code.
    if _data_state_condition(low):
        return _type_result(CODE, "high",
                            f"условие на состоянии данных/трекера → code (речь-акт не перевешивает; beh={beh})",
                            beh, cod)
    if beh == 0 and cod == 0:
...
# ===== ГАРД PLAYBOOK: behavior-правило НЕ смеет ОСЛАБЛЯТЬ код-инвариант (родитель 112, шаг 4/8) =====
# playbook подмешивается в промпт СТРОГО НИЖЕ кап-цены и CRITICAL_FACTS (см. suggest.make_system_prompt:
# «playbook соблюдать, но НЕ отменяет ценовую политику и критичные факты»). Значит поведенческое
# «правило» вида «можно занижать цену», «игнорируй кап», «отдавай без Click», «не бери депозит»,
# «паркуй где угодно» — это НЕ манера речи, а попытка ТЕКСТОМ отменить кодовый гард (цена/кап/парковка/
# Click/депозит/наличие). Такое в playbook НЕ пишем: ответ «это код-инвариант» + роут в CODE-путь
# (дев-очередь) — инвариант меняется ТОЛЬКО в коде с тестом, а не строкой в книге правил. Иначе playbook
# стал бы дырой в гардах: строка «можно ниже прайса» переопределила бы то, что код держит железно.
# Детектор keyword-детерминированный (юнит без claude/сети), КОНЪЮНКЦИЯ двух осей — иначе ложные срабы:
#   ТЕМА-инвариант (цена/кап/парк/Click/депозит/наличие) И маркер-ОСЛАБЛЕНИЕ (заниж/игнор/отмени/без/
#   не бери/ниже прайса…). Одна тема без маркера — легальная манера («называй цену дружелюбнее» → ок).
_INVARIANT_TOPIC_KW = (
    # цена / тариф / прайс
    "цена", "цену", "цены", "цене", "ценой", "тариф", "прайс", "стоимост", "прайслист",
    # кап / потолок цены
    "кап", "капу", "капа", "потолок цен", "потолком цен", "максимальн", "предел цен", "лимит цен",
    # парковка
    "парков", "паркуй", "паркова", "parking",
    # Click / выдача-гейт
    "click", "клик", "клика", "клике",
    # депозит / залог / предоплата
    "депозит", "залог", "предоплат", "аванс",
    # наличие / склад
    "наличи", "склад",
    # EN
    "price", "tariff", "cap", "deposit", "parking",
)
_WEAKEN_KW = (
    # понизить/поднять цену мимо прайса
    "заниж", "завыш", "сниз", "снизь", "дешевл", "ниже прайс", "ниже цен", "ниже тариф",
    "выше кап", "больше кап", "сверх кап", "поверх кап", "ниже кап",
    # обойти/отменить/игнорировать гард
    "игнор", "отмен", "обход", "обойд", "пропусти", "пропуск", "наруш", "ослаб", "не соблюда",
    "не применя", "не проверя", "можно не", "поверх инвариант", "поверх гард",
    # выдать/оформить без обязательного шага
    "не бери", "не требуй", "не спрашивай депозит", "без депозит", "без залог", "без предоплат",
    "без клик", "без click", "без проверк",
    # парковка где попало
    "где угодно", "куда угодно", "везде можно", "в любом месте", "где попало",
    # EN
    "override", "bypass", "ignore", "skip", "waive", "lower", "cheaper", "without deposit",
    "without click", "any spot", "anywhere",
)


def weakens_code_invariant(rule):
    """True (+тема ослабления), если behavior-«правило» пытается ОСЛАБИТЬ код-инвариант (цена/кап/
    парковка/Click/депозит/наличие): тема-инвариант И маркер-ослабление ОДНОВРЕМЕННО (конъюнкция —
    одна тема без маркера легальна). Такое в playbook НЕ пишется — инвариант правится ТОЛЬКО кодом с
    тестом. → (bool, reason). Пусто / чистая манера речи → (False, ''). Чистая функция (юнит без I/O)."""
    low = " ".join(str(rule or "").split()).lower()
    if not low:
        return False, ""
    topic = next((k for k in _INVARIANT_TOPIC_KW if k in low), "")
    weaken = next((w for w in _WEAKEN_KW if w in low), "")
    if topic and weaken:
        return True, f"инвариант «{topic.strip()}» + ослабление «{weaken.strip()}»"
    return False, ""


def build_invariant_reject_ack(rule):
    """Ack учителю, когда его «правило» ослабляет код-инвариант: это НЕ манера речи, а кодовый гард
    (цена/кап/парковка/Click/депозит) — в книгу правил не пишем, отправили в дев-очередь ПК (меняется
    только кодом с тестом). Пустое правило → нейтральная заглушка. Чистая функция."""
    r = _one_line(rule) or "(см. окно диалога)"
    return (f"🛡 Это код-инвариант, не правило поведения: «{r}» затрагивает кодовый гард "
            "(цена/кап/парковка/Click/депозит). В книгу правил не пишу — отправил в дев-очередь ПК "
            "(инвариант меняется только кодом с тестом).")
...
def _apply_behavior_type(text, parsed, remark, card, a_style, replace, conflict, reply_mod, notify, reason):
    ...
    ГАРД (шаг 4/8): ПЕРЕД любой записью — правило не смеет ОСЛАБИТЬ код-инвариант (цена/кап/парковка/
    Click/депозит). Это единый чокпоинт записи behavior-правила (route_lesson_by_type + resume_type_
    unsure «правило»), поэтому даже ЯВНЫЙ выбор учителя «это правило» не протащит ослабление гарда в
    playbook: такое уходит в CODE-путь (дев-очередь), инвариант меняется только кодом с тестом."""
    # (0) ГАРД PLAYBOOK: правило ослабляет код-инвариант? → в playbook НЕ пишем, ответ «это код-инвариант»
    # и роут в CODE-путь (дев-очередь). Проверка ПЕРВОЙ — до конфликт-пробы и до записи (playbook чист).
    weak, why = weakens_code_invariant(remark)
    if weak:
        ack = build_invariant_reject_ack(remark)
        try:
            reply_mod(card, ack)
        except Exception as e:                       # noqa: BLE001 — ack не должен ронять маршрут
            reason = f"{reason}; ack-reply: {e}"
        dec = _apply_code_type(text, parsed, remark, card,
                               f"{reason}; behavior-правило ослабляет код-инвариант ({why}) → в код, не в playbook")
        dec["invariant_guard"] = why
        dec["ack_understanding"] = ack
        dec["result"] = (f"🛡 Правило-инвариант: «{_one_line(remark)}» ослабляет код-гард ({why}) — "
                         "в playbook НЕ записал, ушло в дев-очередь ПК (меняется кодом с тестом)")
        return dec
