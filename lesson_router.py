...
# --- КЛАСС-ФИКС промаха 2-й оси (живой кейс 22.07 16:49), ОБРАТНЫЙ к #164: УПОМИНАНИЕ ДЕНЕГ ≠ ПРАВКА
# ЦЕНОВОЙ ЛОГИКИ. Урок «не жди с ответом уточню-и-вернусь — сразу указывай стоимость, если тариф
# известен» ушёл в CODE (карточка владельцу «нужен код-фикс») из-за РОВНО ОДНОГО слова «тариф» в
# _CODE_KW: beh=0, cod=1 → ветка «чистый code-сигнал». А урок чистый BEHAVIOR — правило о ФОРМУЛИРОВКЕ
# УЖЕ ВЫЧИСЛЕННОГО результата: не обещать «уточню и вернусь» то, что уже посчитано. Кода он не требует —
# ровно это правило уже живёт ТЕКСТОМ в стилевом промпте (suggest.make_system_prompt: «если расчёт/цена
# уже готовы — не отписывайся "уточню и вернусь с расчётом", а сразу дай цифры»), т.е. слой правки —
# playbook, а не резолвер.
# КРИТЕРИЙ (симметричен #164, но в ДРУГУЮ сторону):
#   • ФОРМУЛИРОВКА уже вычисленного («не жди / не обещай / не дублируй / не переспрашивай / сразу
#     называй») → BEHAVIOR, ДАЖЕ ЕСЛИ в тексте есть цена/тариф/стоимость/доставка;
#   • ИСТОЧНИК или УСЛОВИЕ расчёта («цену бери из столбца J», «не называй цену, если Bridge молчит»)
#     → CODE (это машина: откуда берём и когда вообще считаем).
# Реализация — КОНЪЮНКЦИЯ трёх осей (иначе съели бы code-ветку): речь-акт ПОДАЧИ ответа И нет маркера
# ИСТОЧНИКА расчёта И нет маркера НЕДОСТУПНОСТИ данных. Проверяется СТРОГО ПОСЛЕ _data_state_condition —
# так регресс #164 («не пиши "данные получил", если клиент прислал только локацию» → code) остаётся цел:
# условие на СОСТОЯНИИ ДАННЫХ бьёт речь-акт как и прежде, мы разбираем лишь то, что до него не дошло.
_WORDING_ACT_KW = (
    # не обещать / не тянуть с уже готовым ответом
    "не жди", "не тяни", "не откладыв", "не отписывай", "не обещай", "не обещать",
    "уточню и вернусь", "уточню-и-вернусь", "уточню и верн", "вернусь с расч", "вернусь с цен",
    # не дублировать / не переспрашивать уже названное
    "не дублируй", "не дублир", "не повторяй", "не переспрашивай", "не переспраш",
    "не спрашивай повторно", "не уточняй повторно", "уже назван", "уже сказал", "уже написал",
    # называть сразу
    "сразу указывай", "сразу называй", "сразу говори", "сразу пиши", "сразу дай", "сразу давай",
    "сразу отвечай", "сразу озвуч", "указывай сразу", "называй сразу", "говори сразу",
    # EN
    "don't promise", "do not promise", "don't repeat", "do not repeat", "don't re-ask",
    "right away", "straight away", "immediately state", "state it right away",
)
# ИСТОЧНИК/способ расчёта — откуда берётся число (это МАШИНА → code, речь-акт не перевешивает).
_CALC_SOURCE_KW = (
    "столбец", "столбц", "колонк", "ячейк", "строк листа", "лист1", "таблиц", "поле ",
    "прайслист", "прайс-лист", "из прайса", "по прайсу", "формул", "коэффициент", "округл",
    "резолвер", "resolver", "парс", "quote", "bridge", "бридж", "api", "endpoint", "sheet",
    "json", "column", "вебхук", "webhook",
)
# НЕДОСТУПНОСТЬ/сбой источника — «если Bridge молчит / цена не пришла / неизвестна»: это УСЛОВИЕ
# расчёта (когда вообще можно называть число), а НЕ манера речи → code.
_UNAVAILABLE_KW = (
    "молчит", "молчал", "молчание", "недоступ", "не отвеча", "не ответил", "не приход", "не пришл",
    "не отдал", "не вернул", "нет ответа", "ошибк", "упал", "таймаут", "timeout", "неизвест",
    "не посчитан", "не известен", "не известна", "unavailable", "no answer", "silent",
)


def _wording_of_known_result(low):
    """True, если урок — про ФОРМУЛИРОВКУ уже вычисленного результата («не жди/не обещай/не дублируй/
    не переспрашивай/сразу называй»), а НЕ про источник или условие расчёта → behavior ДАЖЕ ПРИ словах
    цена/тариф/стоимость (корень мисклассификации живого кейса 22.07 16:49 — одно слово «тариф» в
    _CODE_KW отправило чистое поведение карточкой владельцу). Конъюнкция: речь-акт подачи ответа И нет
    маркера ИСТОЧНИКА расчёта (столбец/формула/Bridge/quote) И нет маркера НЕДОСТУПНОСТИ данных
    (молчит/не пришла/неизвестна) — оба последних означают правку МАШИНЫ, а не манеры.
    low — уже нормализована (lower, схлопнуты пробелы). Зовётся ПОСЛЕ _data_state_condition (#164)."""
    if not any(w in low for w in _WORDING_ACT_KW):
        return False
    if any(s in low for s in _CALC_SOURCE_KW):
        return False
    return not any(u in low for u in _UNAVAILABLE_KW)
...
def classify_lesson_remark(remark):
    ...
    # ЗЕРКАЛО 2-й оси (живой кейс 22.07 16:49): правило о ФОРМУЛИРОВКЕ уже вычисленного («не жди
    # уточню-и-вернусь — сразу указывай стоимость») — манера речи, а не расчёт: → СТИЛЬ (книга правил),
    # хотя факт-слова цена/тариф/стоимость в тексте есть. Оси разводим ОДИНАКОВО, как в #164.
    if _wording_of_known_result(low):
        return STYLE, "формулировка уже вычисленного результата → стиль (упоминание денег ≠ расчёт)"
    if fact:
...
def classify_lesson_type(remark):
    ...
    if _data_state_condition(low):
        return _type_result(CODE, "high",
                            ...
                            beh, cod)
    # ПРИОРИТЕТ (класс-фикс живого кейса 22.07 16:49, ОБРАТНЫЙ #164): урок о ФОРМУЛИРОВКЕ уже вычисленного
    # («не жди уточню-и-вернусь — сразу указывай стоимость, если тариф известен») — ПОВЕДЕНИЕ, хотя в
    # тексте есть цена/тариф/стоимость: упоминание денег ≠ правка ценовой логики. Бьёт code-счётчик,
    # который тут держался на ОДНОМ слове «тариф» (cod=1, beh=0 → «чистый code-сигнал» → карточка
    # владельцу). Источник/условие расчёта (столбец J / Bridge молчит) внутрь детектора не пускаются.
    if _wording_of_known_result(low):
        return _type_result(BEHAVIOR, "high",
                            f"формулировка уже вычисленного результата → behavior "
                            f"(упоминание денег ≠ расчёт; cod={cod})", beh, cod)
    if beh == 0 and cod == 0:
