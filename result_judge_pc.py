...
ЦЕНТРАЛЬНОЕ ПРАВИЛО, из которого выведены ВСЕ ветки чтения:
    ...
    Любой сбой САМОГО ЧТЕНИЯ (git не запустился, базу не открыть, мост не ответил, руки
    чтения сломаны, ключ узла полосе неизвестен, указатель не разобран) — НЕИЗВЕСТНО.

СБОЙ СБОЮ РОЗНЬ, И ОДНИМ СЛОВОМ ОНИ НЕ НАЗЫВАЮТСЯ. Все перечисленные исходы — `НЕИЗВЕСТНО`, но
текст чтения обязан РАЗЛИЧАТЬ их: «поломка рук» не смеет выглядеть как «источник недоступен» —
иначе диагност чинит мост, когда сломан свой модуль. Вина приезжает от рук ОДНИМ СЛОВОМ
(`BLAME_*`), и словарь `BLAME_WORDS` переводит её в текст вердикта.
...
═══ ЧЕМ ЧИТАЕТСЯ КАЖДЫЙ ИЗ ПЯТИ ВИДОВ ═══════════════════════════════════════════════════════
    ...
    brain          узел, ПРОЧИТАННЫЙ ПО НАЗВАННОМУ КЛЮЧУ, СОДЕРЖИТ НАЗВАННУЮ ПОДСТРОКУ;
...
Две границы чтения, обе — решения, а не случайности:
  ...
  • `brain` ходит наружу ЧЕРЕЗ ОДНУ НАЗВАННУЮ ДВЕРЬ — `brain_probe_pc` (`BRAIN_HANDS`), живые
    руки полосы поверх доверенного `brain_writer.read_text`. Прямых сетевых имён у судьи
    по-прежнему нет вовсе (`bridge_http`/`urllib`/`socket`/`requests`/`brain_writer` — инвариант),
    и дверь обязана быть ровно одна и ровно в одном месте: замок не снят, у него появился
    ИМЕНОВАННЫЙ проход. Читателя всё так же можно подставить (`Ctx.read_doc`) — тест ходит своим.

═══ ЧТО СЧИТАЕТСЯ ДОКАЗАТЕЛЬСТВОМ У ВИДА `brain` (решение Штаба 17.08.2026) ═════════════════

ДОКАЗАТЕЛЬСТВО — НАЙДЕННАЯ ОЖИДАЕМАЯ ПОДСТРОКА В ТЕЛЕ УЗЛА, ПРОЧИТАННОМ ПО НАЗВАННОМУ КЛЮЧУ.
Адрес несёт и ключ, и подстроку (`<док>§<искомое>`), и этого довольно. Опорное прежнее
состояние узла обязательным НЕ является.

ЗАБОР ЧЕСТЕРТОНА, СНЯТЫЙ ЗДЕСЬ, НАЗВАН ВСЛУХ. Половина «вырос» (`Ctx.brain_baseline`) защищала
ровно от одного подлога: «названное лежало в узле ЕЩЁ ДО шага», то есть шаг доказывает чужой
текст. ЭТУ ЗАЩИТУ МЫ ТЕРЯЕМ — прямо, а не молча: адрес, чья подстрока была в узле и вчера,
получит ДОКАЗАН. Взамен вид перестаёт быть КОНСТАНТОЙ (замер 17.08: `НЕИЗВЕСТНО` одинаково на
легшем и на непоставленном разделе — вид не нёс НИ ОДНОГО бита), а базовой линии в каноне
адреса нет и не было, поэтому в бою половина «вырос» не закрывалась НИ ОДНОЙ дорогой.

ЛИНИЯ НЕ УДАЛЕНА, А СТАЛА НЕОБЯЗАТЕЛЬНОЙ УСИЛИТЕЛЬНИЦЕЙ: задал вызывающий `brain_baseline` —
она по-прежнему судится и может СНЯТЬ зелёное (нашлось, но узел не вырос → НЕ ДОКАЗАН); не
задал — вердикт стои́т на одной подстроке. Так забор остаётся доступным тому, кто знает размер
«до», и не превращает вид в вечное «неизвестно» для всех остальных.

АДРЕС БЕЗ ПОДСТРОКИ («только имя узла») — НЕИЗВЕСТНО, и отказ выдаётся ДО чтения: доказывать
таким адресом нечего, а живое чтение стои́т секунды. Зелёного эта ветка не даёт ни при каких
условиях.
...
═══ ТЕНЬ ЦЕПОЧКИ: СЧИТАЕМ, НЕ ПРИМЕНЯЕМ (пункт 3 контракта, 17.08.2026) ═════════════════════

`chain_shadow()` считает ТЕНЕВОЙ вердикт НА ВСЮ ЦЕПОЧКУ: адрес берётся из КОРНЕВОЙ записи
(единица полосы — «цепочка = корневая запись очереди со своими шагами», рамка §8г, перепись
467), читается назад, исход один из трёх. Форма СРАЗУ ЦЕПОЧЕЧНАЯ: пошаговую не повторяем —
на полосе VPS она уже показала ноль (заявленное, отсюда не проверяемое).

Одно правило здесь важнее прочих, потому что без него прибор вырождается за один шаг:
ШАГ БЕЗ СВОЕГО АДРЕСА ЦЕПОЧКУ НЕ ОБРЫВАЕТ — и это `continue` ДО суда, а не оговорка. Судись
такой шаг, он дал бы НЕИЗВЕСТНО; НЕИЗВЕСТНО сильнее ДОКАЗАНного — и любая цепочка, где хоть
один шаг промолчал, села бы в НЕИЗВЕСТНО навсегда, то есть «нет адреса — нет зелёного» вошло
бы боковой дверью (тот же класс, что и в правиле про сбой чтения). Молчание при этом НЕ
ПРЯЧЕТСЯ: безадресные шаги считаются числом (`MUTED`).

ТЕНЬ НА ХОД НЕ ВЛИЯЕТ — устройством, а не обещанием: модуль не зовёт никто (инвариант
`RESULT_JUDGE_UNWIRED`), очередь он не трогает ни одним именем, входные ряды не мутирует, и
писателя у него нет вовсе (имени `open` в судье быть не может — инвариант `TestJudgeOnlyReads`).
Место тени названо одно — `SHADOW_LOG`; готовую запись отдаёт `shadow_record()`, а сам акт
записи делает вызывающий. Настоящий исход цепочки судья НЕ ВЫВОДИТ САМ: `shadow_diff()` берёт
его ТРИСТЕЙТОМ снаружи (True/False/None) и статусов очереди не знает ни одним литералом.
...
# ЕДИНСТВЕННАЯ ДВЕРЬ СУДЬИ НАРУЖУ — имя модуля живых рук. Записано КОНСТАНТОЙ, потому что по
# ней же инвариант проверяет, что дверь одна и в одном месте (`test_result_judge_pc`).
BRAIN_HANDS = "brain_probe_pc"

# ВИНА ЗА НЕСОСТОЯВШЕЕСЯ ЧТЕНИЕ УЗЛА — одно слово от рук. Литералы ДОСЛОВНО повторяют
# `brain_probe_pc.BLAME_*`; совпадение сторожит тест (идиома полосы: маркер сверяется с чужим
# литералом, а не подразумевается). Импортировать их сюда нельзя — дверь одна, и она в
# `_live_read_doc`; словарь обязан работать и тогда, когда рук нет вовсе.
BLAME = "blame"
BLAME_HANDS = "руки"
...
BLAME_WORDS = {
    BLAME_HANDS: "РУКИ ЧТЕНИЯ СЛОМАНЫ (это НЕ «источник недоступен»)",
    BLAME_SOURCE: "ИСТОЧНИК НЕДОСТУПЕН (мост не ответил)",
    BLAME_NODE: "КЛЮЧА ТАКОГО УЗЛА ПОЛОСА НЕ ЗНАЕТ",
    BLAME_TESTRUN: "ЖИВОЕ ЧТЕНИЕ ЗАПРЕЩЕНО ПРИЗНАКОМ ТЕСТ-ПРОГОНА",
}
BLAME_UNNAMED = "ЧТЕНИЕ СОРВАЛОСЬ, ВИНА НЕ НАЗВАНА"
...
def _live_read_doc(doc):
    """ЖИВЫЕ РУКИ ПО УМОЛЧАНИЮ: ключ узла → текст, тем же путём, каким узлы читает вся полоса.

    ЕДИНСТВЕННОЕ МЕСТО, ГДЕ СУДЬЯ ВЫХОДИТ НАРУЖУ. Импорт ЛЕНИВЫЙ и не обёрнут `try` намеренно:
    сорвавшийся импорт рук — это отказ С ВИНОЙ, и разбирает его `probe_brain` тем же словарём,
    что и все прочие отказы чтения. Свои руки — `brain_probe_pc`; они же держат замок «под
    тест-прогоном наружу не ходим», поэтому гейт этой дверью в сеть не уезжает."""
    import brain_probe_pc                                              # noqa: PLC0415
    return brain_probe_pc.read_doc(doc)


class Ctx(object):
    """Чем судье читать. Всё внешнее — ПОДСТАВЛЯЕМОЕ: тест не ходит ни в git, ни на диск, ни в
    мост, а живой вызов получает те же ветки, что и тест.

    ЧИТАТЕЛЬ УЗЛА ЕСТЬ ПО УМОЛЧАНИЮ (правка 17.08.2026): до неё вид `brain` обрывал разбор ДО
    чтения веткой «читателя не дали», поставщиков в бою было ноль, и вид не нёс ни одного бита.
    Теперь дефолт — живые руки (`_live_read_doc`), а `read_doc=` осталось точкой подстановки."""
    ...
    def __init__(self, repo=None, window=None, read_doc=None, brain_baseline=None,
        ...
        self.window = window
        self.read_doc = read_doc if read_doc is not None else _live_read_doc
...
def _read_failed(doc, exc):
    """Сорвавшееся чтение узла → СЛОВА С НАЗВАННОЙ ВИНОЙ. Вина приезжает от рук одним словом
    (`blame`), и «поломка рук» тут обязана звучать иначе, чем «источник недоступен»."""
    blame = getattr(exc, BLAME, "")
    head = BLAME_WORDS.get(blame, BLAME_UNNAMED)
    return "%s — узел «%s»: %s: %s" % (head, doc, type(exc).__name__, exc)


def probe_brain(pointer, ctx=None):
    """brain: УЗЕЛ, ПРОЧИТАННЫЙ ПО НАЗВАННОМУ КЛЮЧУ, СОДЕРЖИТ НАЗВАННУЮ ПОДСТРОКУ.

    Доказательство — НАЙДЕННАЯ ПОДСТРОКА (решение Штаба 17.08.2026, разбор в шапке вместе с
    названным вслух забором, который при этом снят). Опорное прежнее состояние узла
    (`Ctx.brain_baseline`) НЕОБЯЗАТЕЛЬНО: задали — судится второй половиной и может снять
    зелёное; не задали — вердикт стои́т на подстроке, а не садится в НЕИЗВЕСТНО, как раньше.

    Порядок веток: подстрока проверяется на существование ДО чтения (адрес без неё доказывать
    нечем, а живое чтение стои́т секунды), всё остальное — ПОСЛЕ попытки прочитать узел."""
    box = _ctx(ctx)
    doc, needle = _brain_parts(pointer)
    if doc is None:
        return _reading(None, "указатель мозга пуст")
    if len(needle) == 0:
        return _reading(None, "адрес назвал узел «%s», но НЕ НАЗВАЛ подстроку — доказывать "
                              "нечем (доказательство вида `brain` — найденная подстрока)" % doc)
    if box.read_doc is None:
        return _reading(None, "читателя мозга не дали: руки сняты подстановкой")
    try:
        text = box.read_doc(doc)
    except Exception as exc:                                          # noqa: BLE001
        return _reading(None, _read_failed(doc, exc))
    if not isinstance(text, str):
        return _reading(None, "%s — узел «%s»: читатель вернул не текст (%s)"
                        % (BLAME_WORDS[BLAME_HANDS], doc, type(text).__name__))
    has = bool(needle in text)
    base = box.brain_baseline
    grew = None if base is None else bool(len(text) > int(base))
    answer = has if grew is None else _combine_answers([has, grew])
    return _reading(answer, "узел «%s» ПРОЧИТАН (%d симв.): подстрока найдена — %s, вырос — %s "
                            "(линия %s)" % (doc, len(text), _word(has), _word(grew), base))
...
#
# ЕДИНИЦА ВЗЯТА С ПОЛОСЫ, А НЕ ПРИДУМАНА: «цепочка = КОРНЕВАЯ запись очереди со своими шагами»
# (рамка Штаба §8г, перепись 467 — `docs/artifacts/2026-08-11-unexplained-refusals-pc-census.md`
# §1). Отсюда и адрес: он берётся из КОРНЕВОЙ записи, читается назад, и исход — ОДИН НА ВСЮ
# ЦЕПОЧКУ. Пошаговую форму здесь НЕ повторяем: на полосе VPS она уже показала ноль.

# ГДЕ ЛЕЖИТ ТЕНЬ — МЕСТО НАЗВАНО ЯВНО И ОДНО. Файл ОТДЕЛЬНЫЙ: ни настоящего вердикта, ни хода
# цепи, ни счёта серии он не касается, и ни один боевой читатель его не открывает.
SHADOW_LOG = os.path.join(REPO, "result_shadow_chain_pc.jsonl")

# ПИСАТЕЛЯ В МОДУЛЕ НЕТ — И ЭТО УСТРОЙСТВО, А НЕ ЗАБЫВЧИВОСТЬ: инвариант `TestJudgeOnlyReads`
# требует, чтобы имени `open` в судье не было вовсе (виду `file` хватает stat). Поэтому судья
# отдаёт ГОТОВУЮ ЗАПИСЬ (`shadow_record`), а сам акт записи в `SHADOW_LOG` делает вызывающий.
# Так «только чтение» остаётся замком, а место тени всё равно названо здесь и одно.

# Разница тени с настоящим — четыре слова, и каждое отвечает на «в какую сторону».
SAME = "совпало"
STRICTER = "тень строже"          # настоящий ЗЕЛЁНЫЙ, а доказательства нет
SOFTER = "тень мягче"             # настоящий КРАСНЫЙ, а продукт по адресу ЕСТЬ
NOT_CLOSED = "цепочка не закрыта"  # судить нечего: исход ещё не наступил
...
def _item_ref(one):
    """Ряд очереди ЛИБО готовое значение адреса → значение адреса. Ряд читается каноном полосы
    (`result_ref.read_item`: настоящая колонка сильнее маркера), строка берётся как есть."""
    if isinstance(one, dict):
        return rr.read_item(one)
    if isinstance(one, str):
        return one
    return rr.EMPTY


def chain_shadow(root, steps=(), ctx=None):
    """КОРНЕВАЯ ЗАПИСЬ + её шаги → ОДИН теневой вердикт НА ВСЮ ЦЕПОЧКУ.

    ТРИ ПРАВИЛА, и все три — устройством:

    1. АДРЕС ЦЕПОЧКИ — АДРЕС КОРНЯ. Он входит в голосование ВСЕГДА, даже когда не назван:
       не названный адрес корня — это НЕИЗВЕСТНО про всю цепочку, а не «ничего страшного».
    2. ШАГ БЕЗ СВОЕГО АДРЕСА ЦЕПОЧКУ НЕ ОБРЫВАЕТ — и это `continue` ДО суда, а не оговорка в
       комментарии. Если бы такой шаг судился, он давал бы НЕИЗВЕСТНО, а НЕИЗВЕСТНО сильнее
       ДОКАЗАНного — и любая цепочка, где хоть один шаг промолчал, садилась бы в НЕИЗВЕСТНО
       НАВСЕГДА. Правило выродилось бы в «нет адреса — нет зелёного», то самое, что судья шага
       не пускает боковой дверью. Поэтому безадресный шаг в бюллетень НЕ КЛАДЁТСЯ ВОВСЕ, а
       считается отдельным числом (`MUTED`) — молчание обязано быть видно.
    3. ШАГ СО СВОИМ АДРЕСОМ СУДИТСЯ ДОПОЛНИТЕЛЬНО, и его НЕ ДОКАЗАН обрывает цепочку — потому
       что НЕ ДОКАЗАН сильнейший по порядку силы, а не потому, что для него написана ветка.

    Складывает `combine()` — тем же порядком силы, что и всё в этом модуле: НЕ ДОКАЗАН >
    НЕИЗВЕСТНО > ДОКАЗАН. Ничего не пишет, ничего не мутирует: входные ряды остаются байт в
    байт теми же (замок «тень не влияет на ход»)."""
    root_seen = judge(_item_ref(root), ctx)
    ballot = [root_seen["verdict"]]
    judged = []
    muted = []
    # `steps or ()` здесь было бы СЛЕПЫМ ЧИТАТЕЛЕМ по храповику полосы (`nonparse_scan`,
    # подпись `or_empty`): идиома схлопнула бы «шагов не передали» и «шагов ноль» в одно.
    walk = steps
    if steps is None:
        walk = ()
    for one in walk:
        ref = _item_ref(one)
        if rr.parse(ref) is None:
            muted.append(one)          # ПРАВИЛО 2: в бюллетень не кладём — цепочку не рвём
            continue
        seen = judge(ref, ctx)
        ballot.append(seen["verdict"])
        judged.append(seen)
    answer = combine(ballot)
    return {
        "verdict": answer,
        "root": root_seen,
        "steps": judged,
        VOTED: len(ballot),
        MUTED: len(muted),
        "detail": _chain_words(answer, root_seen, judged, len(muted)),
    }


def _chain_words(answer, root_seen, judged, muted):
    """Почему цепочка получила ЭТОТ исход — словами, а не молча. Называем ВИНОВНИКА: первый
    вердикт силы итога, начиная с корня, — именно он цепочку и решил."""
    if root_seen["verdict"] == answer:
        who = "корень: " + root_seen["detail"]
    else:
        who = "шаг: молчит"
        for seen in judged:
            if seen["verdict"] == answer:
                who = "шаг: " + seen["detail"]
                break
    return "%s · %s · шагов судимо %d, без адреса %d" % (answer, who, len(judged), muted)


def shadow_diff(real_green, shadow):
    """(НАСТОЯЩИЙ исход цепочки, ТЕНЕВОЙ вердикт) → в какую сторону они разошлись.

    `real_green` — ТРИСТЕЙТ вызывающего: True «настоящий зелёный», False «настоящий красный»,
    None «цепочка не закрыта». Настоящий исход судья НЕ ВЫВОДИТ САМ и статусов очереди не знает
    ни одним литералом — иначе тень начала бы толковать чужой слой."""
    if real_green is None:
        return NOT_CLOSED
    if real_green:
        return SAME if shadow == PROVEN else STRICTER
    return SOFTER if shadow == PROVEN else SAME


def shadow_record(chain_id, real_green, shadow):
    """ГОТОВАЯ ЗАПИСЬ ТЕНИ: теневой вердикт РЯДОМ с настоящим и разница между ними — одной
    строкой для `SHADOW_LOG`. Пишет её вызывающий (см. врезку про `open` выше)."""
    # Сужаем ЯВНЫМИ ветками, а не `or {}`: слепая идиома стёрла бы разницу между «тени не дали»
    # и «тень посчиталась и пуста» — ровно то различение, ради которого у судьи третий исход.
    seen = {}
    if isinstance(shadow, dict):
        seen = shadow
    root_seen = seen.get("root")
    if not isinstance(root_seen, dict):
        root_seen = {}
    answer = seen.get("verdict", UNKNOWN)
    real_word = REAL_OPEN
    if real_green is True:
        real_word = REAL_GREEN
    elif real_green is False:
        real_word = REAL_RED
    return {
        CHAIN: chain_id,
        SHADOW: answer,
        REAL: real_word,
        DIFF: shadow_diff(real_green, answer),
        "kind": root_seen.get("kind", ""),
        "pointer": root_seen.get("pointer", ""),
        VOTED: seen.get(VOTED, 0),
        MUTED: seen.get(MUTED, 0),
