...
import mark_vs_address_pc     # МАРКЕР ГАРДА ПРОТИВ АДРЕСА: исход захода судит продукт; чистое решение, без ввода-вывода
...
import accounts_registry as accounts  # ВЫБОР УЧЁТКИ СТРОИТЕЛЕЙ (25.09): реестр учёток вне git решает судьбу CLAUDE_CONFIG_DIR у ребёнка; чистый (io/json/os/re + profile_choice)
try:
...
# ВЕХА 2026-09-06 (СНЯТИЕ ЗАЛИПШЕГО ОТКАЗА ГЕЙТА, 1b621b7): механика ЗЕРКАЛЬНАЯ предыдущим — там
# blob-дифф нужен был, чтобы новый код доехал, здесь — чтобы доехал СТАРЫЙ, уже отвергнутый. 06.09
# в 01:49 и 02:17 гейт дважды провалился (27f3b9c→c2193a3 и 27f3b9c→bb28f16) на голдене
# test_vnutrennii_fail_ne_trogaet_vorota: третий исход на границе сезонов законно привёл в
# клиентское замыкание task_metrics.py (цепь suggest → season_gate → dispatch_notify →
# task_metrics, замыкание 37 → 40 файлов). Починен ГОЛДЕН, признак не тронут (разбор —
# docs/artifacts/2026-09-06-гейт-самообновления-зелёный-после-третьего-исхода.md). Но правка легла
# в ТЕСТЫ, а они демону не зависимость: отпечаток замыкания не сдвинулся, а _SU_REJECTED_BLOB
# держит отказ ИМЕННО ПО ЭТОМУ отпечатку — значит зелёный гейт демон бы даже не перегнал и остался
# бы на 27f3b9c до чужой правки любой из 64 зависимостей. Этот комментарий — минимальный blob-дифф,
# чтобы next-poll снял залипший отказ и перегнал гейт по-честному. Логика демона НЕ меняется.
...
LIMIT_MARK = "🚧"        # маркер «НАМ НЕ ДАЛИ РАБОТАТЬ» (класс 21.09.2026): процесс отказался
                         # стартовать работу по ВНЕШНЕМУ ОГРАНИЧЕНИЮ — лимит подписки, перегрузка
                         # поставщика, непродлённый доступ. От 📡 отделён сознательно: связь ЕСТЬ,
                         # и «сеть лежит» увело бы владельца чинить не то. Думатель не чинит
                         # (NO_HEAL_PREFIXES): переформулировка задачи не поднимает лимит
...
#
# ══ ВРЕМЕННЫЙ ОТКАТ 04.09.2026 (умолчание 900 → 0). НЕ РЕШЕНИЕ, А ПАУЗА ═════════════════════
# ПОЧЕМУ. Потолок класс не закрыл, а переложил цену на потребителей, которые ходят в мост
# ОДОЛЖЕННЫМ клиентом (`queue_snapshot_pc._daemon()` → `import pc_orchestrator`). Демон живёт
# как `__main__`, поэтому импорт даёт ВТОРОЙ объект модуля со СВОИМ `_bridge_budget`, а
# `reset()` зовётся только в теле главного цикла `__main__` — у одолженного клиента счётчик
# НЕ ОБНУЛЯЕТСЯ НИКОГДА и растёт до конца жизни процесса. Отпечаток в логе однозначный:
# ящик Штаба срезан 10 раз (13:37→16:06 04.09, «очередь недоступна … потолок (900с)»), а строк
# `ПОТОЛОК МОСТА` от `_bridge_budget_note()` за те же витки — РОВНО НОЛЬ: событие смотрит на
# счётчик `__main__`, а режет счётчик импортированной копии. Полоса не брала задач 12:09→15:13,
# и вернул её не виток, а РЕСТАРТ демона (self-update 16:08), обнуливший процесс.
# ЧТО ВОЗВРАЩАЕМ ЭТИМ ОТКАТОМ ВСЛУХ: один зависший вызов моста снова способен затормозить
# оборот целиком — вчерашний замер даёт до 50 минут на витке, а инъекционный замер лестницы —
# до 4802 с на одном GET. Это известная цена, взятая сознательно и на время.
# ЧТО НЕ ТРОГАЕМ: код потолка цел весь (класс, три исхода, замок пути записи, событие со
# словами) — снято ТОЛЬКО значение. Правило вернёт разведка оборота.
# ВЕРНУТЬ ПОТОЛОК: умолчание "0" → "900" в строке ниже (или `PC_BRIDGE_LOOP_BUDGET=900` в среде
# демона — ручка старше умолчания и перекрывает его без правки кода).
# ПАСПОРТ ПОРОГА: терпит=ВСЕ чтения моста за ОДИН виток демона делят один потолок
#   | замер=ЕСТЬ: 200 строк «залипание вызова моста» живого pc_orchestrator.log (сон измерен и
#   равен 0, связь есть, работа в отрезке не шла) — медиана 349 · p90 504 · p95 605 · p99 779 ·
#   MAX 1437с. Значение 900с = 1.16× p99 и на всём корпусе тронуло бы РОВНО ОДИН виток из 200
#   | снят=2026-09-04 | делится=все чтения моста витка
#   | предел-единицы=НЕ НУЖЕН отдельной ручкой: у каждого вызова есть свой сокет-таймаут
#   (Bridge(timeout=90)). НО он НЕ равен стенному времени — лестница bridge_http (MAX_HOPS × ECHO
#   × READ_TRIES) даёт до 4802с на одном GET по инъекционному замеру, и это ровно та мина, из-за
#   которой общий потолок был нужен | род=латентность
#   | артефакт=docs/artifacts/2026-09-04-мост-потолок-витка.md
# ЖИВОЕ ЗНАЧЕНИЕ 0, А НЕ 900, И ЭТО ОБЪЯВЛЕННЫЙ ВРЕМЕННЫЙ ОТКАТ (04.09, разбор выше): замер не
# опровергнут, снято ТОЛЬКО значение, потому что счётчик у одолженного клиента не обнуляется.
BRIDGE_LOOP_BUDGET = float(os.getenv("PC_BRIDGE_LOOP_BUDGET", "0") or "0")
...
def _route_needs_approval(tid, what, topic, frm=None):
    ...
    try:
        ...
        if not _route_registry_write(row, verdict):
            return topic
        # Лог говорит о СВОЁМ действии, а не о судьбе сообщения (09.09.2026): прежнее
        # «снята с инбокса владельца» утверждало доставку, которой эта ветка не видит, —
        # и живой ряд #232 его опроверг (тема снята в 17:25:16, карточка владельцу пришла).
        log.info("МАРШРУТ ЗАЯВОК: #%s — темы карточки НЕ НАЗЫВАЕМ (дошло ли сообщение до "
                 "владельца, отсюда не видно) — %s | %s",
...
class Bridge:
    ...
    def set_needs_approval(self, tid, what, topic=NEEDS_APPROVAL_TOPIC, frm=None):
        ...
        # горлышко, через которое ряд `needs_approval` СОЗДАЁТСЯ с полосы ПК — сюда стекаются все
        ...
        #
        # ЧЕГО ЭТОТ ЗАМОК НЕ ДЕЛАЕТ, И ЭТО ЗАМЕР, А НЕ ОПАСЕНИЕ (тот же день, ряд #232). Карточку
        # рождает НЕ ТЕМА, а САМ РЯД: её несёт devbot VPS, выбирая ряды по метке `from`
        # (`QUEUE_FROMS_PC`, адрес — инбокс), а поля темы серверный Мост не читает вовсе (см. :1350
        # и :6146 — обе строки написаны здесь до этой правки). Значит снятая тема НЕ гасит карточку
        # ряда, чей `from` девбот знает: 09.09 в 17:25:13 ряд #232 встал `needs_approval`, в
        # 17:25:16 тема была снята — и карточка владельцу всё равно пришла. Ветка остаётся: она
        # честно перестаёт ПРОСИТЬ карточку и записывает решение в реестр. Но звать её «замком
        # доставки» нельзя — доставка живёт на чужой полосе и этой строкой не управляется.
...
def _chatlog_capture(why):
    """Захват рабочей переписки в `chatlog/` — ОТДЕЛЬНЫМ процессом, fire-and-forget (22.09.2026).

    ЗАЧЕМ ВООБЩЕ: у хранилища переписки был склад и не было ЧАСОВ. Модули родились 05.09 одним
    коммитом (f991db9f), захват отработал ОДИН раз руками (05.09 18:42:57) — и с тех пор его не
    звал никто: замер 22.09 — последний байт архива 05.09 18:43:00, прирост за 16 суток 0 байт,
    строк за 16.09–22.09 ноль. Вызывающего не было НИКОГДА, а не «был и пропал»:
    `git log --all -S"chatlog_ingest"` вне артефактов пуст.

    ПОЧЕМУ ОТСЮДА, А НЕ С ПУТИ ПРИЁМА СООБЩЕНИЯ. Захват — ПАКЕТНЫЙ: он перечитывает
    `userbot.log(+ротация)`, `dispatch_notify.log`, `delivery_6m.jsonl` и 30 выгрузок целиком
    (замер: 2.17с, 43 104 сообщения за проход). На входящем сообщении это стоило бы полного
    перечитывания архива НА КАЖДОЕ СООБЩЕНИЕ. К тому же `userbot_listen.py` — клиентское
    замыкание (`client_contour.closure()` 22.09: 50 файлов, он внутри), то есть закрытые ворота.
    Демон не клиентский (тот же замер: `pc_orchestrator.py` вне замыкания), тикает сам и уже
    носит два таких же вызова.

    ПОЧЕМУ ОТДЕЛЬНЫМ ПРОЦЕССОМ: копия `_srv_delivery`/`_queue_snapshot`. Чтение 9 МБ журналов —
    это секунды, и внутри `poll_once` они удлиняли бы оборот, чей возраст меряют О2/О4. ЧАСТОТУ
    решает САМ скрипт по своему штампу (`CHATLOG_TICK_EVERY_MIN`, 360 мин), а не частота вызова
    отсюда. ИМПОРТА НЕТ СОЗНАТЕЛЬНО: спавн не заводит ребра в графе, поэтому клиентское
    замыкание от этой правки не растёт ни на файл (замер до/после: 50 → 50).

    ПОД ЮНИТ-ТЕСТАМИ НЕ СПАВНИТ ВОВСЕ, И ЭТО НЕ ОСТОРОЖНОСТЬ, А ЗАМЕР. Первый же прогон гейта
    (`test_pc_orchestrator`, 1201 тест) после появления этой строки ДОЕХАЛ ДО ЖИВОГО `poll_once`
    и запустил НАСТОЯЩИЙ захват — дважды сразу: в боевой архив легло 1587 законных строк и
    1118 ДУБЛЕЙ в 16 файлах дня, а два файла получили побайтную порчу (3 нечитаемых строки).
    Дедуп от этого не спасает: он читает уже лежащее ДО записи, и два процесса, читающие
    одновременно, видят одно и то же пустое место. Тот же класс, что «гейт написал в БОЕВОЙ спул
...
# «НАМ НЕ ДАЛИ РАБОТАТЬ» — НЕ «МЫ СЛОМАЛИСЬ» (класс 21.09.2026, разбор
# docs/artifacts/2026-09-21-69r-LIMITNEPOLOMKA-2109.md). Перепись 69n измерила: за семь суток
# 14–21.09 шесть рядов закрылись кодом exec_error, и ни у одного причина не была установима по
# записям полосы. Когда текст всё же удалось восстановить из расписок ревью, оказалось, что
# четыре ряда из шести — упор в лимит подписки («You've hit your weekly/session limit · resets
# …»), а два — непродлённый доступ («Failed to authenticate: OAuth session expired»). Все шесть
# владелец прочитал словами «ошибка выполнения», то есть «мы сломались». Следующий шаг у этих
# исходов РАЗНЫЙ: лимит пережидают до названного сброса, доступ чинит человек, а нашу поломку
# чинят кодом. Имя кода названо по тому, что он меряет: процесс ОТКАЗАЛСЯ РАБОТАТЬ по внешнему
# ограничению — работа не начиналась, и от её содержания не зависело ничего.
FAIL_EXTERNAL_LIMIT = "external_limit"         # работать не дали: лимит/перегрузка/доступ (rc≠0 + слова процесса)
...
FAIL_REASONS = {                               # код → (человеческое имя, маркер ПЕРВЫМ символом)
    ...
    # 🚧 намеренно: работать НЕ ДАЛИ. Переформулировка задачи лимит не поднимает и сессию не
    # продлевает, а круг самопочинки сгорит впустую — 21.09 он и сгорал: думатель шёл в тот же
    # отказ и падал exit=1 в 4 случаях из 6.
    FAIL_EXTERNAL_LIMIT: ("внешнее ограничение — работать не дали", LIMIT_MARK),
}
...
def _run_task_impl(tid, text, note="", _mctx=None, approved=(), approved_object=""):
    ...
    # ★ 25.09.2026: решение переехало из файла дерева в РЕЕСТР УЧЁТОК ВНЕ GIT (`accounts_registry.py`,
    ...
    for attempt in (1, 2):
        ...
        if rc != 0:
            ...
            # ═══ СВИДЕТЕЛЬСТВО ВЫХОДА — ПЕРВЫМ ДЕЙСТВИЕМ И В ЛОГ (класс 21.09.2026) ═══════
            # До этой правки у ветки `rc != 0` НЕ БЫЛО СВОЕЙ СТРОКИ ЛОГА ВОВСЕ, а stderr ехал в
            # отчёт только фолбэком при пустом stdout. Итог измерен переписью 69n: шесть рядов
            # за семь суток закрылись «ошибкой выполнения», и причина не была установима НИ У
            # ОДНОГО по записям, которые полоса делает сама. Строка пишется ДО любой развилки —
            # тогда свидетельство остаётся на диске независимо от того, каким именем мы потом
            # назвали исход, и задним числом читается даже у ряда, которого уже нет в очереди.
            # Секреты вырезаны внутри `evidence` (stderr чужого процесса может нести ключи).
            ev = exit_evidence.evidence(rc, out_s, err)
            log.error("id=%s ОТКАЗ ПРОЦЕССА: %s", tid, ev)
            kind, said = exit_evidence.classify(rc, out_s, err)
            ...
            if net:
                ...
                # ОБРЫВ СУДИТСЯ ПЕРВЫМ: у него есть ВТОРОЙ свидетель — измерение полосы
                # (`LinkWatch`), которого у слов процесса нет; спорить с измерением словарём нечем.
                ...
                return "failed", fail_result(FAIL_NETWORK_OUTAGE,
                                             ...
                                             + _clip(exit_evidence.scrub(out_s or err_tail
                                                                        or "нет вывода"), 400),
                                             since=_task_started_get(tid))
            # НАМ НЕ ДАЛИ РАБОТАТЬ. Условий ДВА, и оба обязательны: ненулевой код возврата (его
            # ставит ОС, а текст доклада его не производит) и слово отказа на ПОСЛЕДНЕЙ строке
            # потока. Одних слов НЕ ДОСТАТОЧНО — иначе задача, разбирающая этот самый класс,
            # купила бы себе вердикт «работать не дали» цитатой в собственном докладе.
            if exit_evidence.is_external(kind):
                log.warning("id=%s РАБОТАТЬ НЕ ДАЛИ (%s) — это НЕ поломка исполнителя: %s",
                            tid, kind, exit_evidence.OUT_NAMES.get(kind, kind))
                return "failed", fail_result(FAIL_EXTERNAL_LIMIT,
                                             f"claude exit={rc}: "
                                             + exit_evidence.scrub(out_s or err_tail or "нет вывода")
                                             + " | СВИДЕТЕЛЬСТВО: " + ev,
                                             since=_task_started_get(tid))
            # Отличить нечем → исход остаётся НАШЕЙ ошибкой выполнения, но уже СО СВИДЕТЕЛЬСТВОМ:
            # «не знаю» здесь честное, а не замаскированное под догадку.
            return "failed", fail_result(FAIL_EXEC_ERROR,
                                         f"claude exit={rc}: "
                                         + exit_evidence.scrub(out_s or err_tail or "нет вывода")
                                         + " | СВИДЕТЕЛЬСТВО: " + ev,
...
# ═══ ИСХОД ЗАХОДА СУДИТ АДРЕС РЕЗУЛЬТАТА, А НЕ МАРКЕР ГАРДА (06.09.2026) ═════════════════════
# Доктрина, граница, чем это отличается от дежурного по карточкам и что это значит для высшего
# вида — в докстринге `mark_vs_address_pc`. Здесь только РУКИ: собрать факты, спросить судью
# адреса, отправить ИЗВЕЩЕНИЕ и закрыть ряд. Решение живёт в чистом модуле без импортов, и
# стережёт это инвариант `MARK_VS_ADDR_PURE`, а не докстринг.
#
# ПОЧЕМУ ВЕТКА СТОИ́Т ПЕРВОЙ, ДО ДЕЖУРНОГО. Оба закрывают ряд без кнопки, но разными словами:
# дежурный — терминальным `failed` («за карточкой нет операции»), мы — `done` («работа захода
# прочитана по адресу»). На пересечении верны оба, а правдивее наш: `failed` поверх лежащего и
# прочитанного продукта — то же переписывание доказанного, ради снятия которого заведена ветка.
# Дежурный при этом не задет ни одной строкой: `PARK` возвращает управление ровно туда, где оно
# шло до правки, и его ветка получает тот же вход.
#
# ЧЕГО ЭТА ВЕТКА НЕ ДЕЛАЕТ. Не трогает решение гарда (команда так и не исполнилась), не зовёт
# `maybe_update_bots` (рестарт живых ботов на дороге, где заход упёрся в красное, — не наша
# новость) и не ставит карточку под надзор: карточки у владельца нет, надзирать не за чем.
#
# ОТКАТ: `MARK_VS_ADDR=0` в окружении демона либо стоп-файл рубильника → ветка мертва целиком.
MARK_ADDR_ENV = "MARK_VS_ADDR"
MARK_ADDR_LANE = "ПК"          # чьё имя стоит в извещении владельцу


def _mark_vs_addr_on():
    """Рубильник ветки. ДЕФОЛТ — ВКЛЮЧЕНО, и это осознанное расхождение с дежурным по карточкам.

    У дежурного дефолт выключен потому, что его доказательство ОТРИЦАТЕЛЬНОЕ («за карточкой нет
    операции») и цена ошибки — погашенное владельцу красное. Здесь доказательство
    ПОЛОЖИТЕЛЬНОЕ и снято чужим прибором (V0 прочитал продукт по названному адресу), а цена
    БЕЗДЕЙСТВИЯ измерена: 40 парковок из 43 поверх лежащего продукта, 8 из них — переделкой.
    Выключенная по умолчанию починка не чинит ничего.

    Стоп-файл БЬЁТ значение окружения (тот же `_flag_forced_off`, что у думателя): выключение
    правкой .env имеет тихий отказ по эстафете self-update, а стоп-файл читается каждый вызов."""
    if _flag_forced_off(MARK_ADDR_ENV):
        return False
    return (os.environ.get(MARK_ADDR_ENV) or "1").strip() not in ("0", "no", "false", "off")


def _mark_addr_fields(card):
    """(объект, класс) карточки — С МЕСТА ГАРДА, а не своей регуляркой. Оба читателя канонические
    (`object_from_card` / `kinds_from_card`); гард не импортировался — поля пусты, и извещение
    честно скажет «не назван», а не промолчит."""
    if not pretool_guard:
        return "", ""
    try:
        return (pretool_guard.object_from_card(card or ""),
                ", ".join(sorted(pretool_guard.kinds_from_card(card or ""))))
    except Exception:                     # noqa: BLE001 — чтение полей не смеет уронить ветку
        return "", ""


def _mark_addr_notice(tid, obj, kind, chosen):
    """ИЗВЕЩЕНИЕ ОБ ОТКАЗЕ ГАРДА в ТУ ЖЕ ТЕМУ, куда ушла бы карточка. Владелец видит класс и
    объект и ничего не отвечает — кнопок, номера и слова «да» в извещении нет.

    → 'поставлено' | 'сбой:<причина>'. ИМЕННО «ПОСТАВЛЕНО», А НЕ «ДОСТАВЛЕНО»: `_notify_topic` на
    ПК fire-and-forget, подтверждения доставки у него нет ни у кого (тот же довод, что у заметки
    дежурного). Порядок «извещение ДО закрытия» держит след раньше исхода."""
    try:
        _notify_topic(NEEDS_APPROVAL_TOPIC,
                      mark_vs_address_pc.notice(MARK_ADDR_LANE, tid, obj, kind, chosen))
        return "поставлено"
    except Exception as e:                # noqa: BLE001 — недоставка не отменяет закрытия
        log.warning("маркер-против-адреса: извещение в тему %s не поставлено (%s)",
                    NEEDS_APPROVAL_TOPIC, e)
        return "сбой:%s" % type(e).__name__


def _maybe_mark_vs_address(tid, text, card, base):
    """MARK_VS_ADDR≠0 → вердикт по припаркованной карточке. True = ряд закрыт ПРОДУКТОМ по
    названному адресу и владельцу ушло извещение об отказе; False = прежний путь, байт-в-байт.

    FAIL-SAFE НА КАЖДОМ ШАГЕ: рубильник снят, судья не собрался, вердикт `PARK` → False.
    Закрытие требует положительного доказательства; всё остальное — к владельцу с кнопкой."""
    if not _mark_vs_addr_on():
        return False
    try:
        rec = _task_started_rec(tid)
        run_id = done_judge_pc.run_token(tid, rec.get("draft") if isinstance(rec, dict) else None)
        v = done_judge_pc.judge_parked(tid, text, base, run_id=run_id)
        obj, kind = _mark_addr_fields(card)
        action, rule, proof = mark_vs_address_pc.decide(
            card, _duty_origin(card),
            proven=bool(v) and v.get("verdict") == done_judge_pc.DONE,
            addressed=bool(v) and bool(v.get("address")),
            reason=(v or {}).get("reason", ""), chosen=(v or {}).get("chosen", ""),
            obj=obj, kind=kind)
    except Exception as e:                # noqa: BLE001 — сбор фактов не смеет закрыть ряд молча
        log.warning("маркер-против-адреса: решение не собралось id=%s (%s) → карточка идёт "
                    "владельцу", tid, e)
        return False
    if action != mark_vs_address_pc.CLOSE:
        log.info("маркер-против-адреса: ряд id=%s ПАРКУЕТСЯ (%s: %s)", tid, rule, proof)
        return False
    chosen = (v or {}).get("chosen", "")
    note = _mark_addr_notice(tid, obj, kind, chosen)
    result = _cap_result(mark_vs_address_pc.close_result(
        card, rule, proof, obj=obj, kind=kind, draft=_draft_block(tid)), tid)
    cm = _complete(tid, "done", result)
    log.info("MARK-ADDR-CLOSE id=%s продукт=%s класс=%s объект=%s извещение=%s bridge_ok=%s",
             tid, chosen or "—", kind or "—", obj or "—", note, cm.get("ok"))
    _cowork(f"задача #{tid} → done ПО АДРЕСУ РЕЗУЛЬТАТА (маркер гарда ряд не паркует): "
            f"продукт {chosen or '—'}; отказ гарда класса «{kind or 'не назван'}» над объектом "
            f"«{obj or 'не назван'}» ушёл владельцу ИЗВЕЩЕНИЕМ, операция НЕ выполнена")
    return True
...
def _judge_done(tid, text, status, result, base):
    ...
    2. **Своего СТАТУСА ни у «неизвестно», ни у «не доказано» в очереди нет** — их ровно шесть.
       Недоказанное закрытие ложится `failed` с МАРКЕРОМ; искать такие исходы по статусу
       бесполезно, ровно как отказы владельца (класс `queue_snapshot_pc`, 15.08).

       МАРКЕРОВ С 11.09.2026 ДВА, И ЭТО НЕ ОФОРМЛЕНИЕ. `UNPROVEN_PREFIX` — прибор ПРОЧИТАЛ
       продукт и ответил «нет»; `UNKNOWN_PREFIX` — прочитать не смог. Разбирает оба ОДНА
       функция судьи (`done_judge_pc.outcome_of`), и по ней же слепок очереди даёт неизвестности
       свой исход. Повод — живой: заход 245 (10.09.2026 01:04) сделал работу, положил коммит
       `22f57a2` и артефакт по названному адресу, а лёг в числа провалом, потому что судья
       споткнулся о ФОРМУ значений внутри артефакта (`V0: UNKNOWN / sensitive_content`).
       СТАТУС при этом не сдвинулся ни на шаг: оба исхода по-прежнему `failed`.
    ...
    # КЛЮЧОМ СЛЕДА СЛУЖИТ ИМЯ РЯДА, А НЕ ЕГО НОМЕР (11.09.2026). Номер полоса переиспользует —
    # за 10.09 нумерация начиналась заново дважды, — и вердикт под номером зачитывал бы НОВУЮ
    # строку, которой судья не видел. Имя считает чистый слой; импорт ленивый, потому что у
    # демона `contour_digest` — куст (см. :849), а звать его тут надо раз на закрытие.
    import contour_digest as _cd
    done_judge_pc.note(tid, verdict, name=_cd.row_name(tid, text))
...
def _lanes_owner_priority(order, taken):
    """Сколько рядов из ОТСОРТИРОВАННОЙ очереди виток вправе взять себе, чтобы вторая рука не
    увезла ревизорского родителя мимо приоритета полосы. → int (≥1, ≤ taken). ЧИСТАЯ функция.

    КЛАСС (05.09.2026, красный гейт). Приоритет полосы стои́т на ДВУХ рычагах: клейм (owner впереди
    ревизорского родителя) и уступка МЕЖДУ шагами. Параллель 04.09 сохранила ПОРЯДОК сортировки и
    потеряла ИСКЛЮЧИТЕЛЬНОСТЬ: `min(lanes, len(order))` берёт голову списка ЦЕЛИКОМ, поэтому пара
    [owner, ревизорский родитель] уезжала в работу ОДНИМ витком. Это ровно то, что уступка
    запрещает словами «уступка ДО думателя адаптации — лимиты не тратим»: думатель декомпозиции
    тратился на ревизора, пока owner-работа ещё идёт. Правка касается ТОЛЬКО числа взятых рядов —
    сортировка, claim, исполнение и закрытие не тронуты ни строкой.

    Есть owner-ряды → берём ТОЛЬКО их (одну или две — параллель owner+owner цела). Owner-рядов в
    очереди нет вовсе → прежнее поведение: ревизорские родители клеймятся как раньше."""
    owners = 0
    for it in (order or []):
        if _is_revizor_parent_text(it.get("task_text")):
            break                       # список отсортирован: с первого ревизорского идёт хвост
        owners += 1
    return taken if owners == 0 else min(taken, owners)
...
def process_new():
    ...
    order = sorted(items, key=lambda x: (_is_revizor_parent_text(x.get("task_text")),
                                         int(x.get("id") or 0)))
    # Второй рычаг приоритета живёт ЗДЕСЬ, а не в сортировке: порядок отвечает «кто первый», а
    # число взятых рядов — «сколько их вообще». Параллель обязана уважать оба (класс 05.09.2026).
...
def _process_one(task, _plan_queue):
    ...
    if status == "needs_approval" and _maybe_mark_vs_address(tid, text, result, done_base):
        # ИСХОД СУДИТ АДРЕС РЕЗУЛЬТАТА (MARK_VS_ADDR≠0): продукт захода ПРОЧИТАН по названному
        # адресу тем же прибором, что судит закрытие, — значит маркер гарда, оставшийся от
        # команды, которая НЕ выполнилась, ряд не паркует и поля результата не переписывает.
        # Отказ гарда при этом не гаснет: владельцу ушло ИЗВЕЩЕНИЕ с классом и объектом, а сама
        # операция как была заблокирована, так и осталась. Ветка стоит ПЕРЕД дежурным по
        # карточкам — довод в блоке над `_maybe_mark_vs_address`.
        return
...
# ─── ПОВОД ПЕРЕЖИВАЕТ СВОЙ РЯД (06.09.2026, заказ владельца «а если я сплю?») ──────────────────
DECISION_CLS_APPROVAL = "approval"      # класс свёртки и класс постоянного решения — один и тот же
# Куда владелец отвечает на ПРИПАРКОВАННЫЙ повод. Держим дословно рядом с парковкой: адрес,
# названный в одном месте и забытый в другом, — это карточка, просящая невозможного (класс 05.09,
# `client_contour.ANSWER_AT`). Тема-инбокс принимает «да N»/«нет N» — номер ряда переживает срок.
DECISION_ANSWER_AT = ("тема-инбокс подтверждений — «да <номер>» / «нет <номер>»; на весь класс "
                      "сразу — «да» / «нет» в ответ на сводку открытых решений")


def _park_expired_approval(tid, task, park_fn=None):
    """Протухшая owner-карточка → в парковку открытых решений. → True | False (не припарковали).

    СОСТАВ, КОТОРЫЙ ВЛАДЕЛЕЦ ВИДЕЛ, — это ТЕКСТ САМОЙ КАРТОЧКИ, и отпечаток снимается с него, а
    не с номера ряда: номер переживает подмену содержимого (тот же id приходит НОВОЙ задачей через
    дни — класс, ради которого заведён `TASK_START_RECLAIM`, :1348). Ответ на припаркованный повод
    сверится с этим отпечатком, и уехавший состав будет спрошен ЗАНОВО, а не применён.

    Ничего не бросает НИ НА ЧЁМ: парковка — страховка поверх закрытия ряда, и падение страховки не
    смеет отменить само закрытие. Не смогли — говорим в лог и закрываем ряд как закрывали."""
    try:
        text = str(task.get("what") or task.get("result") or task.get("task_text") or "").strip()
        out, rec, err = (park_fn or decision_waits.park)(
            DECISION_CLS_APPROVAL, str(tid),
            decision_waits.shape_of([text]),
            _clip(text) or f"карточка задачи #{tid}",
            where="очередь ПК", answer_at=DECISION_ANSWER_AT)
        if err:
            log.warning("парковка повода #%s: хранилище не записано (%s)", tid, err)
        return bool(out) and not err
    except Exception as e:
        log.warning("парковка повода #%s не удалась (%s: %s) — ряд закрываю как прежде",
...
def process_approval_timeouts():
    ...
    for task in [it for it in r.get("items", []) if _lane_ok(it)]:
        ...
        if (_age_sec(task.get("updated")) or 0) > APPROVAL_TTL:
            ...
            parked = _park_expired_approval(tid, task)
            # ЗАКРЫВАЕТСЯ РЯД, А НЕ ВОПРОС (06.09.2026). Срок остаётся коротким по своей прежней
            # причине — незакрытая карточка держит локальную цепь ОТКРЫТОЙ, и её не видит ни один
            # сторож (забор Честертона, :142). Но до этой правки вместе с рядом умирал и ПОВОД:
            # владелец спал, десяток карточек протухал, и то, что он одобрил бы, ответить было
            # уже негде. Теперь повод переезжает в парковку и ждёт ответа сколько нужно.
            # Fail-closed НЕ ТРОНУТ ни на строку: парковка НИЧЕГО не применяет и применить не
            # может (`decision_waits`, инвариант DW_NO_APPLY) — молчание по-прежнему = отказ.
            ...
            kept = (f"ПОВОД НЕ ПОТЕРЯН: он открыт и ждёт ответа — придёт сводкой открытых "
                    f"решений. Ответить: {DECISION_ANSWER_AT}. Применилось за это время НИЧЕГО."
                    if parked else
                    "повод припарковать НЕ УДАЛОСЬ (см. лог) — ответить по нему уже негде")
            msg = fail_result(FAIL_APPROVAL_TIMEOUT,
                              f"подтверждение не получено за {APPROVAL_TTL // 60} мин — ряд "
                              f"закрыт без «да». {kept}", since=_task_started_get(tid),
...
def _thinker_exec(prompt, timeout, tag):
    ...
    # `test_profile_choice.TestBothSpawnBranchesAsk` (с 25.09 — реестр учёток, `accounts_registry.py`).
    _pchoice = accounts.apply_builders(env, REPO)
    ...
    try:
        ...
        return None
    if p.returncode != 0:
        # СВИДЕТЕЛЬСТВО ВЫХОДА ДУМАТЕЛЯ (класс 21.09.2026). До этой правки stderr был захвачен
        # `capture_output=True` и ВЫБРОШЕН ЦЕЛИКОМ: в лог уходило ровно «думатель exit=N —
        # fail-safe», без единого слова о причине. Цена измерена переписью 69n: из шести рядов
        # exec_error за 14–21.09 думатель падал exit=1 в ЧЕТЫРЁХ, и по всем четырём диагноз не
        # устанавливался вовсе. Здесь важен не только текст, но и РАЗВЕДЕНИЕ: думатель, ушедший
        # в тот же лимит подписки, что и исполнитель, — это «нам не дали», а не «думатель
        # сломался», и следующий шаг у этих двух разный. Секреты вырезаны внутри `evidence`.
        kind, _said = exit_evidence.classify(p.returncode, p.stdout, p.stderr)
        log.warning("%s: думатель НЕ ОТРАБОТАЛ (fail-safe) — %s", tag,
                    exit_evidence.evidence(p.returncode, p.stdout, p.stderr))
        if exit_evidence.is_external(kind):
            log.warning("%s: думателю РАБОТАТЬ НЕ ДАЛИ (%s) — круг самопочинки сгорел не на "
                        "задаче: %s", tag, kind, exit_evidence.OUT_NAMES.get(kind, kind))
        return None
...
# 🚧 в наборе с 21.09.2026 по той же причине, что и 📡: внешнее ограничение думателем не
# чинится по построению. Замер класса: из шести рядов exec_error за 14–21.09 думатель звался и
# падал exit=1 в четырёх — он уходил в ТОТ ЖЕ отказ, что и исполнитель, и круг сгорал впустую.
NO_HEAL_PREFIXES = (_REJECT_PREFIX, TIMEOUT_MARK, MANUAL_MARK, NET_MARK, LIMIT_MARK)
...
_ORCH_RUNTIME = ("pc_orchestrator.py", "gate_selective.py", "task_metrics.py",
                 ...
                 "git_serial_pc.py",
                 # 06.09.2026: ОТКРЫТЫЕ РЕШЕНИЯ ВЛАДЕЛЬЦА. Верхний импорт, и цена грязи здесь своя:
                 # `decision_waits.py` — единственное место, где повод переживает свой ряд, поэтому
                 # незакоммиченная правка меняет не работу задачи, а ТО, ЧТО ВООБЩЕ ОСТАНЕТСЯ от
                 # ночного вопроса к утру. Ошибка в одну сторону теряет повод молча (ровно тот
                 # класс, ради выхода из которого модуль заведён), в другую — ничего не даёт
                 # применить лишнего: применения в модуле нет физически (инвариант DW_NO_APPLY).
                 # Модуль чистый (io/json/os/time), замыкание не растит.
                 "decision_waits.py",
                 # 06.09.2026: МАРКЕР ГАРДА ПРОТИВ АДРЕСА РЕЗУЛЬТАТА. Верхний импорт, и цена грязи
                 # здесь своя и обоюдоострая: `mark_vs_address_pc.py` решает, ПАРКОВАТЬ ли ряд,
                 # чей продукт уже прочитан, — то есть распоряжается и судьбой сделанной работы, и
                 # громкостью отказа гарда. Ошибка в одну сторону возвращает полосе измеренный
                 # класс (40 парковок из 43 поверх лежащего продукта, 8 из них переделкой), в
                 # другую — гасит владельцу извещение об отказе; прав не добавляет ни одна:
                 # решения гарда модуль не касается физически. Он чистый ДО ПУСТОТЫ — импортов в
                 # нём нет вовсе (инвариант MARK_VS_ADDR_PURE), замыкание не растит ни на файл.
                 "mark_vs_address_pc.py",
                 # 21.09.2026: СВИДЕТЕЛЬСТВО ВЫХОДА процесса. Верхний импорт, и цена грязи здесь
                 # своя: незакоммиченная правка `exit_evidence.py` меняет не работу задачи, а то,
                 # ПРОЧТЁТ ли владелец «нам не дали работать» или снова «мы сломались», и не
...
_ORCH_LAZY_UNCOVERED = ("suggest.py", "reviewer.py", "pc_agent.py", "moderation_ipc.py",
                        ...
                        # 09.09.2026: ЛОТОК информационных заявок — два новых листа того же
                        # куста (`review_intake_run` → `zayavki_lotok_run` → `zayavki_lotok_pc`).
                        # Ленивыми они не объявлены и объявлять их такими не надо: ступень B
                        # тянет их ВЕРХНИМ импортом, а сама ступень B ленива у демона — значит
                        # лист приезжает вместе с ней. Своих зависимостей у пары ровно одна, и
                        # та уже покрыта верхним импортом демона (`zayavki_route_pc`).
                        # ЧЕМ ОПАСНА ИХ ГРЯЗЬ: эта пара решает, СОЗДАВАТЬ ЛИ РЯД, ждущий ответа
                        # владельца. Незакоммиченный WIP здесь тихо меняет не вид сообщения, а
                        # то, увидит ли владелец вопрос вообще, — и увидеть это можно будет
                        # только по тишине, то есть по отсутствию, которое никто не замечает.
                        "zayavki_lotok_run.py", "zayavki_lotok_pc.py",
                        ...
                        # 06.09.2026: БАЗА УРОКОВ СТАЛА ИСТОЧНИКОМ ОТВЕТА — `import lesson_store`
                        # в `suggest.load_playbook`, тот же вход в куст, что у price_gate,
                        # model_name и season_gate. Своих листьев приносит РОВНО ОДИН —
                        # `anonymize_corpus` (им хранилище неотключаемо чистит персональное перед
                        # записью урока); `io_utf8` и `parse_outcome` уже несёт САМ демон верхними
                        # импортами (`_ORCH_RUNTIME`), то есть накрыты строже остатка.
                        # ЧЕМ ОПАСНА ИХ ГРЯЗЬ — двумя разными способами, и оба тихие:
                        #   • `lesson_store` решает, КАКИЕ уроки бот считает действующими. WIP
                        #     здесь меняет секцию «Выученные правила» КАЖДОГО промпта — клиент
                        #     получит ответ по правилам, которых владелец не утверждал, и увидят
                        #     это не тестом, а перепиской;
                        #   • `anonymize_corpus` держит чистку персонального. Недоставленный или
                        ...
                        "zayavki_route_pc.py",
                        # 12.09.2026 (задание 51-b): ПОДБОР ЖИВЫХ ПРИМЕРОВ — куст за `import
                        # style_examples` в suggest.py, тот же вход, что у model_name и season_gate.
                        # Своих новых листьев РОВНО ОДИН: сам модуль (его единственная зависимость
                        # `model_name` стои́т в остатке выше, а список сверяется МНОЖЕСТВАМИ —
                        # дублировать нельзя).
                        # ЧЕМ ОПАСНА ЕГО ГРЯЗЬ. Этот файл решает, ЧТО ГОЛОВА ВИДИТ ПЕРЕД ОТВЕТОМ
                        # КЛИЕНТУ. Недоставленный на диск демона — `_style_block` ловит отказ и
                        # отдаёт ручные пары, то есть контур возвращается к июльскому образцу молча
                        # (это и есть выбранное направление отказа, и оно безопасно). Доставленный
                        # НАПОЛОВИНУ опаснее и тише: старый порог при новом законе оценки — или
                        # наоборот — даёт ПРИМЕРЫ НЕ ПРО ТО, а внешне исправный ответ клиенту
                        # выглядит так же, как верный. Ошибку здесь увидит не тест, а клиент.
                        "style_examples.py",
                        # 15.09.2026 (задание 62-g): ПОЛЗУНКИ СЕЗОННОЙ СКИДКИ — куст за ленивым
                        # `import polzunki_pc_run` в `maybe_polzunki`. Своих новых листьев ровно
                        # ДВА: сам прибор и его руки. Всё прочее уже в остатке выше и повторять
                        # его здесь НЕЛЬЗЯ (список сверяется МНОЖЕСТВАМИ): `price_freshness_run` —
                        # ЧУЖОЙ боевой съёмщик девяти ручек, которым прибор и снимает живые числа;
                        # `price_freshness`, `queue_snapshot_pc`, `brain_writer` приезжают за ним
                        # тем же кустом цены, что и у `price_gate`.
                        # ЧЕМ ОПАСНА ЕГО ГРЯЗЬ, названо прямо, и опасность тут ТИХАЯ: этот файл
                        # решает, УВИДИТ ЛИ ВЛАДЕЛЕЦ СДВИНУТУЮ РУЧКУ. Недоставленный на диск
                        # демона — `maybe_polzunki` ловит отказ импорта и молчит, то есть полоса
                        # возвращается к состоянию до 15.09 (сдвиг живёт незамеченным — ровно
                        # измеренные 11 суток). Доставленный НАПОЛОВИНУ опаснее: старая карта
                        # предписаний при новом разборе ответа двери даёт ВОПРОС НЕ ПРО ТО, а
                        # внешне он читается так же, как верный, и человек по нему двинет ручку.
                        "polzunki_pc_run.py", "polzunki_pc.py",
                        # 22.09.2026: ФОРМА СЛОВА «урок включи N» — новый лист куста `pc_agent`
                        # (верхний импорт агента, а сам агент стои́т в остатке выше). Своих
                        # листьев не приносит НИ ОДНОГО: из мира у него только `re`.
                        # ЧЕМ ОПАСНА ЕГО ГРЯЗЬ: этот файл держит ОДНО определение формы слова и
                        # ОДИН образец, который квитанция экзамена диктует владельцу дословно.
                        # Недоставленный на диск демона — агент падает на верхнем импорте, то
                        # есть замолкает дверь ВСЕХ команд темы 205 разом; доставленный
                        # наполовину (новый образец при старом разборе) — владелец говорит ровно
                        # то, что ему написали, и слышит «не разобрал команду», а урок остаётся
                        # выключенным молча.
                        "lesson_word_forms.py")
...
#     сверка коммита — БЕЗ префикса, короткая метка раскрывается git'ом или сверка отказывает).
# ЗАМОРОЗКА СНИМАЕТ ВТОРОЕ ОСНОВАНИЕ ЦЕЛИКОМ (05.09.2026): пока лежит `pc_orchestrator.contour_frozen`,
# ворота открывает ТОЛЬКО поимённое «да» владельца — ни зелёный вердикт, ни любое будущее третье
# основание. До этого дня заморозка решала лишь АДРЕС отказа, а зелёная запись выкатывала коммит на
# живых ботов за 8 секунд (замер 26.08 по логу демона). Замок стои́т в двух местах и не зря: в
# `client_contour.release_reason` основание не возникает, а в `_client_block` не проезжает.
...
# ─── РАСКРЫВАТЕЛЬ КОРОТКОГО ХЕША (класс 05.09.2026, сверка вердикта по префиксу) ───
# ЗАЧЕМ ЗДЕСЬ, А НЕ В `client_contour`: тот модуль ЧИСТЫЙ по построению — «ни git, ни subprocess,
# ни сети» (его шапка), и признак клиентского контура обязан считаться от файлов, а не от процесса
# git. Поэтому судья вердикта (`client_contour.same_commit`) раскрывателя не имеет и без него
# ОТКАЗЫВАЕТ, а живую руку git ему подаёт демон — ровно один вызов на сверку, и только когда две
# метки сошлись префиксом, но разошлись длиной (в бою это «запись 40 hex против `head[:9]`»).
def _git_expand_commit(commit, git_out=None):
    """Короткая метка → ПОЛНЫЙ 40-hex этого коммита. → str ('' — раскрыть не смогли).

    `rev-parse --verify <c>^{commit}`: суффикс обязателен по той же причине, что у
    `_gate_commit_known` — без него метка резолвится в любой объект, включая дерево и блоб.
    Неоднозначный префикс git отвергает сам, и это ровно то, что нам нужно: неоднозначность здесь
    значит «сверять нечем», а не «сойдёт»."""
    c = str(commit or "").strip().lower()
    if not re.match(r"^[0-9a-f]{7,40}$", c):
        return ""
    try:
        out = (git_out or _git_out)(["rev-parse", "--verify", c + "^{commit}"])
    except Exception:                     # noqa: BLE001 — «не смог» это НЕ «совпало»
        return ""
    out = str(out or "").strip().lower()
    return out if re.match(r"^[0-9a-f]{40}$", out) else ""


def _release_reason(commit):
    """Основание пропуска ворот — БОЕВОЙ вызов (дефолт `_client_block.reason_fn`).

    Отличается от `client_contour.release_reason` ровно одним: подаёт живую руку git на сверку
    коммита. Отдельная функция, а не lambda в сигнатуре, чтобы инъекция тестов подменяла ОДНО
    имя и чтобы раскрыватель нельзя было забыть на новой дороге вызова."""
    return client_contour.release_reason(commit, expand=_git_expand_commit)


def _trainer_status(commit):
    """Причина «почему тренажёр (не) открыл» для карточки — тем же раскрывателем, что и решение.
    Разойдись эти два источника — владелец получил бы причину не про тот отказ (класс 30.07)."""
    return client_contour.trainer_status(commit, expand=_git_expand_commit)
...
def _client_block(kinds, commit, paths, where, subject=None, notifier=None, cowork=None,
                  ...
                  route_fn=None, asked_fn=None, commit_known_fn=None, frozen_release_fn=None):
    ...
    if not held:
        st.pop(kk, None)
        return []
    reason = (reason_fn or _release_reason)(commit)
    # ВТОРОЙ ЗАМОК ЗАМОРОЗКИ, и он не дублирует первый (класс 05.09.2026). Первый живёт в
    # `client_contour.release_reason` — там основание под заморозкой просто НЕ ВОЗНИКАЕТ. Этот
    # стои́т на РЕЗУЛЬТАТЕ и потому переживает будущее: какое бы третье основание ни завели и
    # каким бы `reason_fn` ни подменили, под заморозкой мимо него проедет ровно одно слово —
    # поимённое «да» владельца. Задание требует «ни зелёным вердиктом, ни чем-либо ещё», а
    # перечислять «что-либо ещё» по именам значит протухнуть на первом новом основании.
    if reason and reason != "owner":
        _frz, _why = (frozen_release_fn or client_contour.freeze_holds_release)()
        if _frz:
            log.error("ворота контура (%s): основание «%s» на коммите %s НЕ открывает ворота — %s",
                      where, reason, commit, _why)
            (cowork or _cowork)(
                "ворота клиентского контура ДЕРЖАТ %s для %s при основании «%s»: %s"
                % (commit, kk, reason, _why))
            reason = None
    if reason:
        st.pop(kk, None)
    ...
    if st.get(kk) != key:
        ...
        # АДРЕС отказа (21.08.2026, правило переписано 05.09.2026): карточка владельцу или строка в
        # ленту. Ворота этим решением НЕ тронуты — они уже отказали выше и вернут `held` в обоих
        # исходах; заморозка глушит ВОПРОС («можно выкатить?»), которого под заморозкой всё равно
        # никто не задаёт. С 05.09 глушится ЛЮБОЙ такой отказ, а не только повтор формы: постоянное
        # решение владельца «пока контур заморожен — про выкатку детей не спрашивать вовсе». Прежняя
        # ветка «отказ новой формы остаётся карточкой» и рождала все 4 карточки суток 05.09 — форма
        # включает состав клиентских файлов, а он растёт с каждой задачей ночи.
        ...
        # ЧЕГО ЖДЁМ — говорим ПРАВДУ, а не заученную пару (05.09.2026). Под заморозкой зелёный
        # тренажёр ворот больше не открывает, и строка «жду „да“ владельца или зелёный тренажёр»
        # звала бы владельца снять вердикт, который ничего не изменит.
        _wait = ("жду поимённого «да» владельца (контур заморожен: зелёный тренажёр ворот не "
                 "открывает)" if (frozen_release_fn or client_contour.freeze_holds_release)()[0]
                 else "жду «да» владельца или зелёный тренажёр")
        (cowork or _cowork)(
            "ворота клиентского контура: применение %s к %s ОСТАНОВЛЕНО (%s) — клиентские файлы: "
            "%s; %s%s"
            % (commit, kk, where, ", ".join(held), _wait,
        ...
        (notifier or _notify_gate_card)(commit, client_contour.card_text(
            ...
            trainer_note=(trainer_fn or _trainer_status)(commit)))
...
def _notify_deploy_happened(kind, commit, pids, where, notifier=None, frozen_fn=None):
    ...
    сюда доходят ТОЛЬКО применения, которым основание уже открыло дорогу, — заморозка ворот не
    открывает ни одной веткой. С 05.09.2026 таких оснований под заморозкой ровно ОДНО (поимённое
    «да» владельца): зелёный вердикт тренажёра ворот больше не открывает — `freeze_holds_release`.
    ...
    (notifier or _notify)(
        ...
        "Ворота открыло ПОИМЁННОЕ «да» владельца на этот коммит — единственное основание, которое "
        "под заморозкой действует: зелёный вердикт тренажёра ворот не открывает (05.09.2026)."
...
# ПОТОЛОК СУТОК — ЛОВУШКА НА РАЗГОН, А НЕ ДЕЛЁЖ МОЩНОСТИ (8 → 40, 04.09.2026).
# ЭТО НАСТОЯЩИЙ ПОТОЛОК БОЯ: демон передаёт его ящику параметром `budget=`, и
# `shtab_box.DAILY_BUDGET` в бою не спрашивается вовсе — но ИМ печатает число сводка
# контура. Два экземпляра обязаны двигаться ВМЕСТЕ, иначе полоса поведёт себя по
...
# ------------------- ПОЛЗУНКИ ГЛОБАЛЬНОЙ СКИДКИ (задание 62-g, 15.09.2026) ----
# ЗАЧЕМ. Сдвинутый ползунок сезонной скидки жил незамеченным 11 суток — замер: I3 «Моты 2»
# съехал с 0.15 на 0.0 между 02.09 и 13.09, и увидел это только заход, посланный смотреть.
# Прибор сверяет живые ручки с правилами сезона и ОДИН раз на смену картины кладёт вопрос
# владельцу в витрину полосы. Ползунков он не трогает: переставляет человек.
#
# ПОЧЕМУ ВЫЗЫВАЮЩИЙ ЗДЕСЬ, А НЕ «ТО ЖЕ РАСПИСАНИЕ, ЧТО У СТОРОЖА СВЕЖЕСТИ» (пункт 4 задания
# исполнен вслух, а не обойдён). Расписания у сторожа свежести НЕТ ни одного: `price_gate.allow`
# зовётся ЛЕНИВО с пути ответа клиенту (`suggest`), а окном 30 мин он бережёт клиента от девяти
# GET, а не ходит по часам. Задачи Планировщика у ценовых приборов полосы тоже нет —
# `TurboBabyPriceQuench` не создана (schtasks на этой машине красная операция, замер 12.09,
# docs/artifacts/2026-09-12-ПЛАНИРОВЩИК-tick-ceny-1209.md). Поэтому вызывающий взят ЖИВОЙ и
# доказанный — виток демона, тот самый, которым уже ходят витрина и ступень G.
#
# ТРОТТЛИНГ ДВОЙНОЙ, как у соседей, и этажи разные: внешний (POLZUNKI_PC_SEC) бережёт ВИТОК от
# лишнего чтения диска, внутренний (пол `POLZUNKI_EVERY_MIN` внутри прибора) бережёт МОСТ от
# лишних девяти GET. Убери внутренний — и каждый заглядыш стоил бы полосе двух минут двери.
POLZUNKI_PC_SEC = float(os.getenv("POLZUNKI_PC_SEC", "1800") or "1800")   # как часто ЗАГЛЯДЫВАТЬ, с
_POLZUNKI_LAST = [0.0]          # метка загляда В ПАМЯТИ: рестарт демона = свежий взгляд, не потеря


def _polzunki_on():
    """Прибор включён? Дефолт — ВКЛЮЧЕНО; рубильник один и живёт в самом приборе."""
    return (os.environ.get("POLZUNKI_PC_OFF") or "").strip() != "1"


def maybe_polzunki(now=None, runner=None):
    """Один заглядыш прибора ползунков за тик. → отчёт | None.

    Наружу не шлёт ничего ни одной веткой: выход прибора — строка в состоянии, которую
    показывает витрина, плюс строка журнала. Ползунков и ячеек не правит.
    """
    if not _polzunki_on():
        return None
    now = float(now if now is not None else time.time())
    if _POLZUNKI_LAST[0] and (now - _POLZUNKI_LAST[0]) < POLZUNKI_PC_SEC:
        return None
    _POLZUNKI_LAST[0] = now
    try:
        import polzunki_pc_run

        report = (runner or polzunki_pc_run.tick)(root=REPO, now=now)
    except Exception as e:
        log.warning("ползунки: заглядыш упал (fail-safe, следующая попытка через тик): %s", e)
        return None
    if not report.get("acted"):
        log.debug("ползунки: %s", report.get("why") or "сигнала нет")
        return report
    log.info("ползунки: %s — %s", report.get("state"), report.get("why"))
    if report.get("line"):
        _cowork(report["line"])
    return report
...
def _main_loop():
    ...
    while not _stopped():
        try:
            ...
            maybe_polzunki()          # ползунки скидки: живые ручки против правил сезона, вопрос в витрину (POLZUNKI_PC_SEC)
...
# ПАСПОРТ ПОРОГА: терпит=ВЕСЬ виток демона делит один срок тишины продукта
#   | замер=ЕСТЬ, и он ДВУХСЛОЙНЫЙ. (1) 12.08: 1005 пауз витка своей полосы — медиана 173 ·
#   p95 314 · p99 461 · MAX 605с; 1800с = 2.98× худшей законной паузы и выше потолка одного
#   вызова моста (≈28 мин). (2) 03-05.09, ПЕРЕПРОВЕРКА по живому случаю: 39 сборок пакета
#   ревьюеру, медиана 1841с при пороге 1800с, max 1916с — то есть порог оказался НИЖЕ МЕДИАНЫ
#   сборки, и 25 сборок из 39 позвали владельца зря. ЭТО ПЕРВЫЙ ИЗ ТРЁХ СЛУЧАЕВ КЛАССА за
#   сутки 05-06.09 (второй — PRICE_GATE_PROBE_SEC, третий — разведка ценового пути). Порог
#   после перепроверки НЕ ДВИНУТ ни на секунду СОЗНАТЕЛЬНО: поднять его значило бы купить
#   тишину сборки ценой слепоты ко всякому другому зависанию; вместо этого сборка научена
#   ОБЪЯВЛЯТЬ себя (STAGE_START_FILE ниже) | снят=2026-09-05
#   | делится=все ступени витка | предел-единицы=ЕСТЬ, но не одна ручка, а по ступеням:
#   объявленный заход судится WD_BUSY_ALLOW, объявленная длинная ступень — СВОИМ потолком,
#   приезжающим вместе с объявлением. Необъявленная ступень своего предела не имеет — она и
#   есть предмет этого порога | род=латентность
#   | артефакт=docs/artifacts/2026-09-05-терминал-необратим-сторож-не-хоронит-закрытое.md
