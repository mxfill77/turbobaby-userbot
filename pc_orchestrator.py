...
REVIZOR_PREAMBLE = (
    ...
    "  ж) анкета на первом сообщении — клиент ПЕРВЫМ же СОДЕРЖАТЕЛЬНЫМ сообщением дал модель И даты, а "
    "БОТ в ОТВЕТ шлёт анкету/список вопросов вместо тарифа (надо было котировать, а не допрашивать). "
    "НЕ путай с автоприветствием: если «анкета» — это САМ статичный автогритинг Telegram Business "
    "(секция выше), а не ответ бота, находку НЕ заводи (action=noise).\n"
