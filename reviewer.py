...
# «ИЛИ»-соединитель — явная альтернатива; если есть рядом, это НЕ находка
_OR_RE = re.compile(r"\b(?:или|либо|or\b)", re.IGNORECASE | re.UNICODE)
...
def _depcheck(text: str) -> str | None:
    ...
    Скоуп: ищем совместное присутствие денежного маркера И слова «паспорт»
    в пределах 150 символов. Если между ними есть «или/либо» — это нормальное
    предложение выбора, не находка. Если есть «и/плюс/а также» — находка.
    """
    if not _PASSPORT_RE.search(text):
        return None
    # Для каждой позиции «паспорт» ищем деньги в ±150 символах
    for pm in _PASSPORT_RE.finditer(text):
        ps, pe = pm.start(), pm.end()
        window = text[max(0, ps - 150): pe + 150]
        if not _MONEY_RE.search(window):
            continue
        # Есть и деньги, и паспорт в окне. Находка только при AND-соединителе.
        if _OR_RE.search(window):
            # Явное «или/либо/or» в том же окне → нормальное предложение выбора
            return None
        if _AND_RE.search(window):
