...
ЭТОТ ДОВОД ПРОТУХ 07.09.2026, А ВЕТКА — НЕТ, и путать эти две вещи дорого. Решением владельца
`07.09.2026-1` (плюс его ответ «полной» о ступени срока) `price_source` НАУЧИЛСЯ считать срок
через границу: `term_total` берёт множитель периода У КАЖДЫХ СУТОК, а ступень — одну и от
полной длительности. Абзац выше описывает состояние ДО этой правки и оставлен как история.
Что из этого следует, дословно:
  · СЧИТАТЬ ТЕПЕРЬ ЕСТЬ ЧЕМ — фраза «считать такой срок правилом нечем» больше не факт;
  · ВЕТКА ЭТИМ НЕ ТРОНУТА НИ ОДНОЙ СТРОКОЙ. Открывать ли ворота — решение ВЛАДЕЛЬЦА, а не
    следствие того, что счёт появился; задача 07.09 меняла счёт и прямо запрещала трогать
    условия. Сегодня на точечной котировке пересечение по-прежнему уходит человеком: замер
    07.09 — 292 окна из корпуса (365 дат × сроки 7 и 31) возвращаются ДО счёта, то есть
    разбивка на этой дороге не меняет НИ ОДНОГО бата;
  · ТЕКСТ ЗАПИСКИ КЛИЕНТУ (`_NOTE_RU`/`_NOTE_EN`) ТОЖЕ ОСТАВЛЕН КАК ЕСТЬ, и он тоже содержит
    протухший довод («правило цены считает период по дате НАЧАЛА»). Правка клиентского текста —
    это решение о том, ЧТО СЛЫШИТ КЛИЕНТ, и она ждёт слова владельца вместе с воротами;
  · ГДЕ РАЗБИВКА УЖЕ РАБОТАЕТ — прайс-сетка парка: ворот на ней нет сознательно (см. ниже),
    и её числа с 07.09 считаются по дням.
