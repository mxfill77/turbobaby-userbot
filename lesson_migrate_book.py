...
ДВА ПОЛЯ ИЗ ШЕСТИ У ПЕРЕНОСИМОГО ПРАВИЛА ИСТОЧНИКА НЕ ИМЕЮТ, и это названо, а не залатано.
...
def migrate(book_path=None, side_path=None, snap_dir=None, store_path=None, apply=False):
    ...
    for r in plan.rules:
        ...
        numbers.append(lesson_store.add(
            ...
            why=MIGRATION_WHY, who=MIGRATION_WHO, when=snap.stamp, path=store_path))
