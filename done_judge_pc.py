...
# ЯКОРЬ МАРКЕРА, А НЕ ПОИСК (17.09.2026, задание 62-s). До этой правки маркер искался ГДЕ УГОДНО,
# и «неизвестно» проверялось первым: наш таймаут, процитировавший маркер в черновике, и «не
# доказано» с цитатой в докладе получали исход «неизвестно» — льготу серии за одну цитату. Задача,
# разбирающая этот класс, такую цитату иметь обязана.
#
# Правило то же, что у различителя чужого отказа (62-p, `shtab_box_signals`): машинный знак
# читается ТАМ, КУДА ЕГО СТАВИТ МАШИНА, а не там, где его может напечатать текст. У судьи «там» —
# ГОЛОВА ЕГО ЧАСТИ: `fail_result` ставит маркер первым символом, а сборщики выше по течению кладут
# над ним ТОЛЬКО машинные строки — шапку закрытия (`close_msg_pc.prepend`: «СПРАШИВАЛИ/ВЫШЛО/
# ДАЛЬШЕ», отделена пустой строкой) и строки самопочинки. Поэтому НЕ «первый символ итога» (живой
# итог в очереди начинается шапкой, и такой якорь убил бы неизвестность судьи на ВСЕХ живых
# рядах), а два условия, которых цитата не производит:
#   1. маркер стои́т В НАЧАЛЕ СТРОКИ. Черновик доклада демон кладёт одной строкой ПОСЛЕ своего
#      знака (⏱ + «🧾 ЧЕРНОВИК…»), шапка — строками с постоянными метками, цель задания — хвостом
#      строки «СПРАШИВАЛИ:»; ни одна из них строку маркером не начинает;
#   2. маркер стои́т ДО ЧАСТИ ДЕМОНА (`[причина=`). Судья зовётся только на `done` и части демона
#      не пишет; всё, что лежит после неё, — подробности провала, включая МНОГОСТРОЧНЫЙ вывод
#      ребёнка (`claude exit=N: <stdout>`), где строка может начаться с чего угодно.
# Решает ПЕРВОЕ такое вхождение: доклад исполнителя лежит НИЖЕ маркера судьи и первым не бывает.
# Литерал части демона заимствован (= `pc_orchestrator._DAEMON_MARK`), равенство сторожит тест.
_DAEMON_HEAD = "[причина="
_HEAD_RE = re.compile(r"^[ \t]*(%s)"
                      % "|".join(re.escape(PREFIX[w]) for w in _MARK_ORDER), re.MULTILINE)


def _judge_head(text):
    """Текст закрытия → ГОЛОВА ЧАСТИ СУДЬИ (с маркера до конца зоны) | "" (маркера на месте нет).

    Разбор условий — в шапке раздела выше. Судить обязаны по ПОЛНОМУ тексту: якорь держит цитату
    сам, обрезка для этого не нужна и прежде держала её лишь случайно (слепок очереди до 62-s)."""
    s = "" if text is None else str(text)
    at = s.find(_DAEMON_HEAD)
    zone = s if at < 0 else s[:at]
    got = _HEAD_RE.search(zone)
    return zone[got.start(1):] if got else ""


def outcome_of(text):
    """Текст закрытия → слово исхода судьи | `None` («маркера судьи на его месте нет»).
    ...
    Маркер засчитывается ТОЛЬКО НА СВОЁМ МЕСТЕ (:func:`_judge_head`); цитата в чужом тексте
    исхода не покупает.
    ...
    head = _judge_head(text)
    for word in _MARK_ORDER:
        if head.startswith(PREFIX[word]):
            return word
    return None
...
def reason_of(text):
    ...
    Читается ТЕМ ЖЕ ЯКОРЕМ, что и исход (:func:`_judge_head`): причина из цитаты в докладе была
    бы словами исполнителя, выданными за слова судьи."""
    match = _WHY_RE.match(_judge_head(text))
