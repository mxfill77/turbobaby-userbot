...
class TestUnsureFrequency(Base):
    """ЧАСТОТА КЛАССА «ИСХОД НЕ СВЕРЕН» ЗА СУТКИ (решение Штаба 19.09.2026, п. 3).

    ПОЧЕМУ ЭТО НЕ ПОВТОР СОСЕДНЕЙ ПРОВЕРКИ. Счётчик `исход не сверен N` отвечает про
    МОМЕНТ СНЯТИЯ, а состояние живёт минутами — в обычные сутки он честно печатает ноль, и
    класс невидим целиком: замер 19.09 (67m) даёт окно ложного нуля в 554 с, не оставившее
    в суточной статистике ни одного следа. Здесь проверяется ВТОРОЕ число — сколько рядов
    за сутки ПОБЫВАЛИ несверенными, — без которого частоту класса нечем измерить."""

    def test_след_ставится_в_минуту_рождения_несверенного_ряда(self):
        closed = q.merge_closed({"553": {"goal": "ЦЕЛЬ: X"}}, {}, {}, T0, 86400)
        row_ = closed[q.closed_id("553", "ЦЕЛЬ: X")]
        self.assertIs(row_.get(q.F_WAS_UNSURE), True)
        self.assertIn("исход побывал несверенным у 1 рядов", q.render_body([], closed, None, T0))

    def test_след_переживает_сверку_а_состояние_исчезает(self):
        """ГЛАВНОЕ ЗДЕСЬ: после сверки «исход не сверен» честно ноль, а частота — единица.

        Ровно этот случай и был невидим: ряд #106 к ближайшему снятию слепка уже нёс
        `done`, и суточные числа говорили «исход не сверен 0», то есть «такого не бывает»."""
        closed = q.merge_closed({"553": {"goal": "ЦЕЛЬ: X"}}, {}, {}, T0, 86400)
        closed = q.apply_failed(closed, [], T0 + 60, 86400)
        self.assertEqual(closed[q.closed_id("553", "ЦЕЛЬ: X")]["outcome"], "done")
        text = q.render_body([], closed, T0 + 60, T0 + 60)
        self.assertIn("исход не сверен 0", text)
        self.assertIn("исход побывал несверенным у 1 рядов", text)

    def test_след_переживает_и_запись_поверх_у_упавшего_ряда(self):
        """СОСЕДНЯЯ ВЕТКА, без которой правка половинчата: строка, найденная среди упавших,
        собирается в `apply_failed` ЗАНОВО и кладётся поверх прежней целиком."""
        closed = q.merge_closed({"553": {"goal": "ЦЕЛЬ: X"}}, {}, {}, T0, 86400)
        closed = q.apply_failed(closed, [{"id": "553", "since": T0, "goal": "ЦЕЛЬ: X",
                                          "result": "упало: claude exit=1"}], T0 + 60, 86400)
        row_ = closed[q.closed_id("553", "ЦЕЛЬ: X")]
        self.assertEqual(row_["outcome"], "failed")
        self.assertIs(row_.get(q.F_WAS_UNSURE), True, "след потерян ровно там, где ставился")

    def test_число_печатается_и_нулём(self):
        """ОТРИЦАТЕЛЬНЫЙ: ряд, пришедший сразу с прочитанным исходом, следа не получает, а
        число всё равно звучит — величина, видимая только ненулевой, читается как «такого не
        бывает» (та же дверь, что у «НЕИЗВЕСТНО»)."""
        closed = q.apply_failed({}, [{"id": "553", "since": T0, "goal": "ЦЕЛЬ: X",
                                      "result": "упало: claude exit=1"}], T0, 86400)
        row_ = closed[q.closed_id("553", "ЦЕЛЬ: X")]
        self.assertIsNone(row_.get(q.F_WAS_UNSURE))
        self.assertIn("исход побывал несверенным у 0 рядов", q.render_body([], closed, T0, T0))

    def test_счёт_именно_ЗА_СУТКИ_а_не_навсегда(self):
        """Окно суток режет реестр — вместе со следом. Иначе число росло бы вечно и меряло
        бы не частоту, а возраст полосы."""
        closed = q.merge_closed({"553": {"goal": "ЦЕЛЬ: X"}}, {}, {}, T0, 86400)
        later = q.merge_closed({}, {}, closed, T0 + 90000, 86400)
        self.assertEqual(later, {})
        self.assertIn("исход побывал несверенным у 0 рядов",
                      q.render_body([], later, T0 + 90000, T0 + 90000))
