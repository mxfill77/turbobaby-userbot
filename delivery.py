...
# Таймаут одного HTTP-запроса при разворачивании короткой ссылки (HEAD-редиректы + GET тела).
# 5с (было 8): чат-бот не должен подвешивать клиента ради гео — валидный goo.gl-редирект и
# тело place-страницы (~200 КБ) укладываются в 5с с запасом, а при медленной сети быстрее уходим
# в честный фолбэк-вопрос. Координаты уже в URL → сеть не дёргаем вовсе. env-override сохранён.
MAPS_TIMEOUT = int(os.getenv("MAPS_RESOLVE_TIMEOUT", "5") or "5")
