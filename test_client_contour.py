...
class TestGranicaProcessa(unittest.TestCase):
    ...
    def test_vyshel_rovno_odin_i_eto_raner(self):
        """ЧИСЛОМ ДО И ПОСЛЕ, обоими замыканиями: живыми воротами (`cut=()`, 82 → 81) и умолчанием
        признака (42 → 41). Вышло РОВНО одно имя — `trainer_run.py`, и ни одного имени данных.

        ЧИСЛА ЗДЕСЬ — ЗЕРКАЛО ДЕРЕВА, И ОНИ ДВИЖУТСЯ. Предмет теста — РАЗНОСТЬ (что именно вышло за
        границу), а размеры замыкания стоя́т рядом лишь как опора для неё; но растёт замыкание от
        ЛЮБОГО нового модуля клиентской полосы, и тогда краснеет тест, ничего про новый модуль не
        знающий. Замер 12.09.2026 развёл два слагаемых, чтобы следующий читатель не искал вину не
        там: `cut=()` отстал на ТРИ ещё ДО этой правки (78 → 81 — модули, заведённые после
        07.09: `exam_show.py`, `wa_bridge.py`, `zayavki_lotok_pc.py` и соседи), а обе ветки
        подросли ещё на ОДИН из-за `style_examples.py` (задание 51-b): подбор живых примеров
        зовётся из `suggest`, то есть честно стои́т на клиентской полосе. Ветка `cut=умолчание`
        до 51-b была ЗЕЛЁНОЙ (41 → 40 замерено) — её единица целиком наша."""
        for cut, (was_f, now_f) in ((), (82, 81)), (None, (42, 41)):
